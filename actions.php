<?php
require_once dirname(__FILE__).'/assets/includes/cors.inc.php';
// Definisci gli origins permessi
...
// Gestione CORS per chiamate AJAX
...
if (in_array($origin, $allowed_origins)) {
    ...
    header("Access-Control-Allow-Headers: Content-Type, X-Requested-With");
    header("Access-Control-Allow-Credentials: true");
} else if (strpos($origin, 'fotoartpuzzle.it') !== false) {
    // Permetti qualsiasi sottodominio di fotoartpuzzle.it
    header("Access-Control-Allow-Origin: " . $origin);
    header("Access-Control-Allow-Methods: GET, POST, OPTIONS");
    header("Access-Control-Allow-Headers: Content-Type, X-Requested-With");
    header("Access-Control-Allow-Credentials: true");
...
{    
    ...
    $selectedLanguage = isset($_SESSION['selectedLanguageSession']) ? $_SESSION['selectedLanguageSession'] : 'italian';
    if (empty($selectedLanguage)) {
        $selectedLanguage = isset($config['settings']['lang_file_mgr']) ? $config['settings']['lang_file_mgr'] : 'italian';
    }
    
    if($countryID > 0) {
        try {
            // Query per ottenere gli stati
            $query = "SELECT * FROM {$dbinfo[pre]}states WHERE country_id = '{$countryID}' AND active = 1 AND deleted = 0 ORDER BY name ASC";
            $stateResult = mysqli_query($db, $query);
            
            if($stateResult) {
                $stateCount = mysqli_num_rows($stateResult);
                
                if($stateCount > 0) {
                    $stateArray = array();
                    
                    while($state = mysqli_fetch_array($stateResult)) {
                        // Determina il nome dello stato in base alla lingua
                        $stateName = '';
                        
                        // Prova con la lingua italiana prima
                        if (!empty($state['name_italian'])) {
                            $stateName = $state['name_italian'];
                        }
                        // Fallback su inglese
                        elseif (!empty($state['name_english'])) {
                            $stateName = $state['name_english'];
                        }
                        // Fallback finale su name
                        elseif (!empty($state['name'])) {
                            $stateName = $state['name'];
                        }
                        else {
                            $stateName = 'Stato ID: ' . $state['state_id'];
                        }
                        
                        // Escape delle virgolette per JSON
                        $stateName = str_replace('"', '\\"', trim($stateName));
                        $stateID = $state['state_id'];
                        
                        // Aggiungi all'array degli stati
                        $stateArray[] = '"' . $stateID . '" : "' . $stateName . '"';
                    }
                    
                    // Costruisci la stringa JSON degli stati
                    if(!empty($stateArray)) {
                        $states = implode(',', $stateArray);
                        $errorCode = $countryID;
                    }
                } else {
                    // Nessuno stato trovato per questo paese
                    $errorCode = $countryID;
                    $errorMessage = "Nessuno stato trovato per il paese selezionato";
                }
            } else {
                // Errore nella query
                $errorCode = "1";
                $errorMessage = "Errore nella query del database: " . mysqli_error($db);
            }
        } catch(Exception $e) {
            $errorCode = "1";
            $errorMessage = "Errore interno del server: " . $e->getMessage();
        }
    } else {
        $errorCode = "1";
        $errorMessage = "ID paese non valido";
    }
    
    // Output JSON
    header('Content-Type: application/json; charset=utf-8');
    echo '{"states": {' . $states . '},"errorCode": "' . $errorCode . '","errorMessage": "' . $errorMessage . '"}';
    exit;
break;
			case "checkEmail":
