...
class metadata {
	...
	private function prepIPTCString($iptcstring, $characterset) {
		...
		if($characterset == 'utf-8')
			$characterset = 'UTF-8';
		if($langset['lang_charset'] == 'utf-8')
			$outputset = 'UTF-8';
		else
			$outputset = $langset['lang_charset'];
		
		if ($characterset == $outputset) return $iptcstring;
		$iptcstring = $psUTF8->convert($iptcstring, $characterset, $outputset);
	...
	public function getIPTC()
	{
		...
		if(isset($info['APP13']))
		{
			...
			$characterset = $this->getIPTCTag('1#090', $iptcparsed); // Get character set
			
			if(!$characterset)
			{
				$characterset = $langset['lang_charset'];
			}
			...
			if(!$characterset) // Make sure after all that it is set to something
				$characterset = 'utf-8';
		...
		if(is_array($iptcparsed))
		{
			...
			*/
			$this->iptc['iptcCharset']			= $characterset;			
			$this->iptc['keywords'] 			= $keywords;
			$this->iptc['description'] 			= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#120'][0], $characterset));
			$this->iptc['title'] 				= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#005'][0], $characterset));
			$this->iptc['instructions'] 		= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#040'][0], $characterset));
			$this->iptc['date_created'] 		= $iptcparsed['2#055'][0];
			$this->iptc['author'] 				= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#080'][0], $characterset));
			$this->iptc['creator_title'] 		= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#085'][0], $characterset));
			$this->iptc['city'] 				= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#090'][0], $characterset));
			$this->iptc['state'] 				= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#095'][0], $characterset));
			$this->iptc['country'] 				= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#101'][0], $characterset));
			$this->iptc['job_identifier'] 		= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#103'][0], $characterset)); // ALSO TRANSMISSION REFERENCE
			$this->iptc['headline'] 			= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#105'][0], $characterset));
			$this->iptc['provider'] 			= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#110'][0], $characterset)); // ALSO CREDIT
			$this->iptc['source'] 				= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#115'][0], $characterset));
			$this->iptc['description_writer'] 	= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#122'][0], $characterset));
			$this->iptc['urgency'] 				= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#010'][0], $characterset));
			$this->iptc['copyright_notice']		= $this->cleanMetadata($this->prepIPTCString($iptcparsed['2#116'][0], $characterset));
		}
		return $this->iptc;
	}
...
"Ｂ"=>"ｂ","Ａ"=>"ａ","Å"=>"å","K"=>"k","Ω"=>"ω","Ώ"=>"ώ",
"Ὼ"=>"ὼ","Ό"=>"ό","Ὸ"=>"ὸ","Ῥ"=>"ῥ","Ύ"=>"ύ","Ὺ"=>"ὺ",
"Ῡ"=>"ῡ","Ῠ"=>"� ","Ί"=>"ί","Ὶ"=>"ὶ","Ῑ"=>"ῑ","Ῐ"=>"ῐ",
"Ή"=>"ή","Ὴ"=>"ὴ","Έ"=>"έ","Ὲ"=>"ὲ","Ά"=>"ά","Ὰ"=>"ὰ",
...
"ὸ"=>"Ὸ","ί"=>"Ί","ὶ"=>"Ὶ","ή"=>"Ή","ὴ"=>"Ὴ","έ"=>"Έ",
"ὲ"=>"Ὲ","ά"=>"Ά","ὰ"=>"Ὰ","ὧ"=>"Ὧ","ὦ"=>"Ὦ","ὥ"=>"Ὥ",
