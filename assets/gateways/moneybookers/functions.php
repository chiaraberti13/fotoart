<?php
	switch($gatewayMode)
	{
		...
		case "mgrForm":
			...
			$input[] = gateway_input('email',$data['email'],$lang['mbookers_f_email'],$lang['mbookers_f_email_d'],'textbox',1);
			$input[] = gateway_input('testmode',$data['testmode'],$lang['mbookers_f_testmode'],$lang['mbookers_f_testmode_d'],'checkbox',0);
			$input[] = gateway_input('testemail',$data['testemail'],$lang['mbookers_f_testemail'],$lang['mbookers_f_testemail_d'],'textbox',0);
		break;
		...
		case "publicForm":
			...
			*/
			...
			$formData['return_url'] 						= "{$config[settings][site_url]}/assets/gateways/moneybookers/ipn.php"; // Page to notify on payment approval
			$formData['return_url_text'] 				= $lang['mbookers_f_completeOrder'];
			$formData['return_url_target']			= 3;
			$formData['cancel_url']							= "{$config[settings][site_url]}/assets/gateways/moneybookers/ipn.php"; // Page to notify on payment approval
			$formData['cancel_url_target']			= 3;
			$formData['status_url']							= "{$config[settings][site_url]}/assets/gateways/moneybookers/ipn.php"; // Page to notify on payment approval
