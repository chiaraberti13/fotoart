<?php
	...
	*/
	...
	$gatewaySetting = getGatewayInfoFromDB('robokassa'); // Get the gateway settings from the db
	...
	*/
	...
	$ipnValue['paymentTotal'] 		= $_GET['OutSum']; 				// Total amount of payment (*REQUIRED*)
	switch($_GET['OutSum']) 									// Convert status of the payment to usable number values 0=pending, 1=completed, 2=unpaid, 3=bill later, 4=failed (*REQUIRED*)
	{
		case ($_GET['OutSum'] > 0):
	...
	$robokassaOrderResult = mysqli_query($db,"SELECT uorder_id FROM {$dbinfo[pre]}orders WHERE order_id = '{$_GET[InvId]}'"); 
  $robokassaOrder = mysqli_fetch_assoc($robokassaOrderResult);
     
  $ipnValue['orderID'] 			= $robokassaOrder['uorder_id']; 			// The order ID number (*REQUIRED*)
	...
	*/
	...
	if($_GET['OutSum'] > 0){
		...
		$return = "{$config[settings][site_url]}/pay.return.php?orderID=".$robokassaOrder['uorder_id'];
