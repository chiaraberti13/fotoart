<?php
	...
	*/
    ...
    // SMTP con schema corretto in base alla porta
    $host   = $config['settings']['smtp_host'];
    $port   = (int)$config['settings']['smtp_port'];
    $scheme = ($port === 465) ? 'ssl' : (($port === 587) ? 'tls' : null);

    $transport = Swift_SmtpTransport::newInstance($host, $port, $scheme)
        ->setUsername($config['settings']['smtp_username'])
        ->setPassword($config['settings']['smtp_password'])
        ->setTimeout(15);

    // SOLO PER TEST su server legacy con CA vecchie (rimuovi in produzione):
    /*
