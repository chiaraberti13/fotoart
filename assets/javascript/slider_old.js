...
function moveSlider(evnt)
{
	...
	if (mouseover) { // Only if slider is dragged
		...
		// BLOW IS CUSTOM CODE ADDED BY KTOOLS.NET
		custom_slide = display.getAttribute('slide_action') // SEE IF THERE IS AN ACTION ASSIGNED TO SLIDING
		if(custom_slide){
			slide_action(custom_slide);
		}
		//scale(); // THIS IS WHERE I ADDED MY SCALE LOGIC
		return false
	}
	return
}
...
function slide(evnt)
{
	...
	//preview = display.getAttribute('preview') // See if a preview function should be called / KTOOLS
...
function sliderMouseUp()
{
	if (mouseover) {
		...
		// BLOW IS CUSTOM CODE ADDED BY KTOOLS.NET
		custom_drop = display.getAttribute('drop_action') // SEE IF THERE IS AN ACTION ASSIGNED TO DROPPING THE SLIDER
		if(custom_drop){
			//slide_action(custom_slide);
			drop_action(custom_drop);
		}
