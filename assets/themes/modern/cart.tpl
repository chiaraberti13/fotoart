					...
					<div class="container cartContainer" style="clear: both;">
						...
						<div class="row">
							<div class="col-md-2 cartThumbColumn">
								...
								{elseif $bPP}
									<img src="{$cartItem.photoPuzzleImg}" class="thumb" style="width: 100px;">
								{else}
							...
							<div class="col-md-5">
								...
								{else}
									<h2><span class="cartItemEditLink">{$cartItem.itemDetails.name}</span></h2>
								{/if}
								...
								{elseif $cartItem.has_options && $bPP}
									<div>
										{foreach $cartItem.photoPuzzleGruppoOpzione as $k => $gruppoOpzione}
											<span>{$lang.$gruppoOpzione}: {$cartItem.photoPuzzleOpzione[$k]}</span>
										{/foreach}
										{*{$cartItem.photoPuzzleGruppoOpzione|@var_dump}*}
										<!--<span>{$lang.$cartItem.photoPuzzleGruppoOpzione}</span>-->
										<!--<div style="display: none" id="optionsBox{$cartItem.oi_id}" class="optionsBox"></div>-->
									</div>
								{/if}
							</div>
							...
							<div class="col-md-2 cartActionsColumn">
								{if $cartItem.item_type == 'package' or $cartItem.item_type == 'print' or $cartItem.item_type == 'product'}
									{if !$bPP}
										<input type="button" href="{$cartItem.itemDetails.cartEditLink}" class="btn btn-xs btn-success cartItemEditLink" value="{$lang.edit}"><br>
									{/if}
								{/if}
				...
				<div class="col-md-4">
					...
					<div class="cartTotalColumn">
						...
						<div class="cartTotalList promotionsBox">
							<p>{$lang.discountCode} <input type="text" name="couponCode" id="couponCode" class="form-control"><div style="padding-top: 1px;"> <input type="button" value="{$lang.apply}" id="applyCouponButton" style=" float: right;" class="btn btn-xs btn-primary"></div></p>
							{if $cartCouponsArray}
								<ul>
