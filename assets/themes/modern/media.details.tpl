		...
		<div class="container">
			...
			{else}				
				...
				<hr>
					<div class="row">
					<div class="col-md-12">
		             {* <h1>{if $crumbs}{foreach $crumbs as $key => $crumb}<a href="{$crumb.linkto}">{$crumb.name}</a> {if !$crumb@last} &raquo; {/if}{/foreach}{if $media.details.title.value} &raquo; {$media.details.title.value} - {$media.details.description.value}{/if}</h1><hr>{/if} *}
						<h4> {$media.details.description.value} </h4>
					</div>
				</div>
				...
				<div class="row">					
					<div class="col-md-8 mediaPreviewCol">					
						...
						{else}
							...
							{else}
								<div class="previewContainer"><img src="{if $media.sampleCachedLink}{$media.sampleCachedLink}{else}{mediaImage mediaID=$media.encryptedID type=sample folderID=$media.encryptedFID seo=$media.seoName}{/if}" title="{$media.details.description.value}" alt="{$media.details.description.value}" class="img-responsive"></div>
							{/if}
						{/if}
					...
					<div class="col-md-12">
		             {* <h1>{if $crumbs}{foreach $crumbs as $key => $crumb}<a href="{$crumb.linkto}">{$crumb.name}</a> {if !$crumb@last} &raquo; {/if}{/foreach}{if $media.details.title.value} &raquo; {$media.details.title.value} - {$media.details.description.value}{/if}</h1><hr>{/if} *}
						<h4>{$media.details.title.value}</h4><hr>
					</div>
				</div>
							...
							{if $productRows}
								<div class="mediaPurchaseContainers" id="purchaseProducts">
									...
									{foreach $products as $product}
										{*{$lang|@var_dump}*}
										{if $product['item_code'] == "PPZL"}
											{if $ppzl == FALSE}
												{$ppzl = TRUE}
												<div class="purchaseRow">
												{*{if $product.photo}<img src="{productShot itemID=$product.prod_id itemType=prod photoID=$product.photo.id size=125}">{/if}*}
												<h2><a href="{$product.linktopuzzle}">{$lang.fotopuzzleName}</a></h2>
												<p class="purchaseListDescription">{$lang.fotopuzzleDesc|truncate:200}</p>
												{*
					...
					<div style="margin-top: 4px; overflow: auto; clear: both; margin-bottom: 4px;">
							...
							{if $media.showLightbox}<p class="btn btn-info btn-primary addToLightboxButton" lightboxItemID="{$media.lightboxItemID}" mediaID="{$media.media_id}"> {$lang.lightbox}</p>{/if}
							{if $config.settings.email_friend}<p class="btn btn-info btn-primary emailToFriend" mediaID="{$useMediaID}"> {$lang.email}</p>{/if}										
					...
					<ul class="mediaDetailsList">
						{content id=99}
						<hr>
					<center>	<p class="btn btn-warning btn-primary"><a href="{$product.linktopuzzle}">{$lang.fotopuzzleName}</a></p> </center><br>
									<center>			<p class="purchaseListDescription">{$lang.fotopuzzleDesc|truncate:200}</p></center>
						<hr>						
							...
							<div id="mediaExtraInfoDG" style="margin-bottom: 30px;">
								...
								{if $config.settings.related_media}
									<div class="dataGroupContainer" id="similarMediaDGC">
										<div id="mediaSimilarPhotos" mediaID="{$useMediaID}" galleryID="{$useGalleryID}" galleryMode="{$galleryMode}" style="overflow: auto; min-height: 60px;"></div>
									</div>
								{/if}
								{if $commentSystem}
...
{* Featured Galleries Area *}
					...
					<center>	<h3>{$lang.viewGalleries}</h3>		</center>				
						<div id="galleryListContainer">
							{foreach $subGalleriesData as $subGallery}
								<div class="galleryContainer" style="width: {$config.settings.gallery_thumb_size}px">
										...
										<a href="{$galleriesData.$subGallery.linkto}"></p>
										<p class="galleryDetails">{if $galleriesData.$subGallery.password}<img src="{$imgPath}/lock.png" class="lock">{/if}<a href="{$galleriesData.$subGallery.linkto}">{$galleriesData.$subGallery.name}</a></p>
									</div>
