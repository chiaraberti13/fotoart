	...
	<div id="container">
		...
		<div id="contentContainer" class="center">
			...
			<div class="content">
				...
				{else}
					{if $crumbs}<h1>{foreach $crumbs as $key => $crumb}<a href="{$crumb.linkto}">{$crumb.name}</a> {if !$crumb@last} &raquo; {/if}{/foreach}{if $media.details.title.value} &raquo; {$media.details.title.value}{/if}</h1>{/if}
							...
							<div class="divTableRow">
								<div class="divTableCell" id="mediaPreviewImage">
									...
									<div style="margin-top: 4px;">
										{if $galleryMode}
											<div class="prevNext">
												{if $backButton}<p class="mediaPreviewContainerIcon"><img src="{$imgPath}/back.icon.png" alt="{$lang.backUpper}" title="{$lang.backUpper}" id="backButton" href="{$backButton.linkto}"></p>{/if}												
												{if $prevButtonID}<p class="mediaPreviewContainerIcon"><img src="{$imgPath}/prev.icon.png" alt="{$lang.prevUpper}" title="{$lang.prevUpper}" id="prevButton" href="{linkto page="media.details.php?mediaID={$prevButtonID}"}"></p>{/if}
												{if $nextButtonID}<p class="mediaPreviewContainerIcon"><img src="{$imgPath}/next.icon.png" alt="{$lang.nextUpper}" title="{$lang.nextUpper}" id="nextButton" href="{linkto page="media.details.php?mediaID={$nextButtonID}"}"></p>{/if}
											</div>
										{/if}
										<p style="float: left;">{$config.settings.sn_code}</p>
										{if $media.showLightbox}<p class="mediaPreviewContainerIcon addToLightboxButton" lightboxItemID="{$media.lightboxItemID}" mediaID="{$media.media_id}"><img src="{$imgPath}/lightbox.icon.{$media.inLightbox}.png" id="addToLightboxButton{$media.media_id}" inLightbox="{$media.inLightbox}" alt="{$lang.lightbox}" title="{$lang.lightbox}"></p>{/if}
										{if $config.settings.email_friend}<p class="mediaPreviewContainerIcon emailToFriend" mediaID="{$useMediaID}"><img src="{$imgPath}/email.icon.0.png" alt="{$lang.email}" title="{$lang.email}"></p>{/if}										
										{if $packageRows}<p class="mediaPreviewContainerIcon assignToPackageButton" mediaID="{$media.media_id}"><img src="{$imgPath}/package.icon.0.png" alt="{$lang.assignToPackage}" title="{$lang.assignToPackage}"></p>{/if}
								...
								<div class="divTableCell" style="width: 100%">
									<ul class="mediaDetailsList">
										{foreach $media.details as $detail}
											{if $detail.value != ''}
												<li class="mediaDetail{$detail@key}">
													...
													{elseif $detail@key == 'colorPalette'}
														{foreach $detail.value as $colorKey => $color}
															<p class="colorSwatch" style="background-color: #{$color.hex}" title='#{$color.hex}'></p>
														{/foreach}
										...
										{if $media.showRating}
											<li style="padding-top: 15px; clear: both;">
												<p class="{if $media.allowRating}starRating{/if}" mediaID="{$media.media_id}">
													{foreach $media.rating.stars as $stars}<img src="{$imgPath}/star.{$stars}.png" class="ratingStar" originalStatus="{$stars}" style="margin-top: -3px;">{/foreach}
										...
										{if $media.latitude and $media.longitude}
											<li style="padding-top: 15px; clear: both;">
												...
												<p>
													...
													<img src='http://maps.googleapis.com/maps/api/staticmap?center={$media.latitude},{$media.longitude}&zoom={$config.settings.gpszoom}&size={$config.settings.gpswidth}x{$config.settings.gpsheight}&maptype={$config.settings.gpsmaptype}&sensor=false&markers=color:{$config.settings.gpscolor}%7C{$media.latitude},{$media.longitude}'>
													</a>
												</p>
											</li>
										{/if}
					...
					{if $digitalRows}
						<div class="mediaPurchaseContainers" id="purchaseDigitalDownloads" style="display: block">
							<!--{$digitalRows} purchaseDigitalDownloads <input type="button" value="Download" onclick="workbox({ 'page' : 'digital.php' });">-->
								...
								{foreach $digitalsArray as $digital}
									<div class="divTableRow">
										<div class="divTableCell" style="width: 100%;">
											...
											{if $config.settings.display_license}<p class="purchaseListDescription">{$lang.license}: {$digital.licenseLang} {if $digital.attachlicense > 0}<a href="{$baseURL}/content.php?id={$digital.attachlicense}">{$lang.moreInfoPlus}</a>{/if}</p>{/if}
											{if $digital.width or $digital.height}<p class="purchaseListDescription">Resolution: {$digital.width} x {$digital.height} {$lang.px} {if $digital.widthIC or $digital.heightIC}<em>( {$digital.widthIC} x {$digital.heightIC} @ {$config.dpiCalc} {$lang.dpi} )</em>{/if}</p>{/if}
											<!--{$digital.quantity}<br>-->
											{if $digital.format}<p class="purchaseListDescription">{$lang.mediaLabelFormat}: {$digital.format}</p>{/if}
											{if $digital.dsp_type == 'video'}
												<p class="purchaseListDescription">
													{if $digital.fps}{$lang.mediaLabelFPS}: {$digital.fps}<br>{/if}
													{if $digital.running_time}{$lang.mediaLabelRunningTime}: {$digital.running_time} {$lang.seconds}<br>{/if}
												</p>
											{/if}
											<p class="purchaseListDescription">{$digital.description|truncate:200}</p>
											{if $digital.license == 'rf' or $digital.license == 'ex' or $digital.license == 'eu'}<p class="purchaseListPrice">{if $digital.price}<span class="price">{$digital.price.display}</span>{if $digital.price.taxInc} <span class="taxIncMessage">({$lang.taxIncMessage})</span>{/if}{/if}{if $digital.credits and $digital.price} {$lang.priceCreditSep} {/if}{if $digital.credits}<span class="price">{$digital.credits} <sup>{$lang.mediaLabelCredits}</sup></span>{/if}</p>{/if}
										</div>
										<div class="divTableCell"><a href="{$digital.linkto}" class="colorButtonLink workboxLink workboxLinkAttach">{$lang.downloadUpper}</a></div>
									</div>
								{/foreach}
							</div>	
						</div>
						...
						<div class="mediaPurchaseContainers" id="purchasePrints">
							<div class="divTable" style="width: 100%">
								{foreach $prints as $print}
									<div class="divTableRow">
										<div class="divTableCell" style="vertical-align: top">{if $print.photo}<img src="{productShot itemID=$print.print_id itemType=print photoID=$print.photo.id size=75 crop=50}">{/if}</div>
										<div class="divTableCell" style="width: 100%;">
											...
											<p class="purchaseListPrice">{if $print.price}<span class="price">{$print.price.display}</span>{if $print.price.taxInc} <span class="taxIncMessage">({$lang.taxIncMessage})</span>{/if}{/if}{if $print.credits and $print.price} {$lang.priceCreditSep} {/if}{if $print.credits}<span class="price">{$print.credits} <sup>{$lang.mediaLabelCredits}</sup></span>{/if}</p>
										</div>
										{if $cartStatus}<div class="divTableCell"><a href="{$print.addToCartLink}" class="colorButtonLink{if !$print.directToCart} workboxLink workboxLinkAttach{elseif $config.settings.minicart} miniCartLink{/if}" rel="nofollow">{$lang.addToCart}</a></div>{/if}
									</div>
								{/foreach}
							</div>
						</div>
						...
						<div class="mediaPurchaseContainers" id="purchaseProducts">						
							<div class="divTable" style="width: 100%">
								{foreach $products as $product}
									<div class="divTableRow">
										<div class="divTableCell" style="vertical-align: top">{if $product.photo}<img src="{productShot itemID=$product.prod_id itemType=prod photoID=$product.photo.id size=75 crop=50}">{/if}</div>
										<div class="divTableCell" style="width: 100%;">
											...
											<p class="purchaseListPrice">{if $product.price}<span class="price">{$product.price.display}</span>{if $product.price.taxInc} <span class="taxIncMessage">({$lang.taxIncMessage})</span>{/if}{/if}{if $product.credits and $product.price} {$lang.priceCreditSep} {/if}{if $product.credits}<span class="price">{$product.credits} <sup>{$lang.mediaLabelCredits}</sup></span>{/if}</p>
										</div>
										{if $cartStatus}<div class="divTableCell"><a href="{$product.addToCartLink}" class="colorButtonLink{if !$product.directToCart} workboxLink workboxLinkAttach{elseif $config.settings.minicart} miniCartLink{/if}" rel="nofollow">{$lang.addToCart}</a></div>{/if}
									</div>
								{/foreach}
							</div>						
						</div>
						...
						<div class="mediaPurchaseContainers" id="purchaseCollections">						
							...
							<div class="divTable" style="width: 100%">
								{foreach $collections as $collection}
									<div class="divTableRow">
										<div class="divTableCell" style="vertical-align: top">{if $collection.photo}<img src="{productShot itemID=$collection.coll_id itemType=coll photoID=$collection.photo.id size=75 crop=50}">{/if}</div>
										<div class="divTableCell" style="width: 100%;">
											...
											<p class="purchaseListPrice">{if $collection.price}<span class="price">{$collection.price.display}</span>{if $collection.price.taxInc} <span class="taxIncMessage">({$lang.taxIncMessage})</span>{/if}{/if}{if $collection.credits and $collection.price} {$lang.priceCreditSep} {/if}{if $collection.credits}<span class="price">{$collection.credits} <sup>{$lang.mediaLabelCredits}</sup></span>{/if}</p>
										</div>
										<div class="divTableCell"><a href="{$collection.viewCollectionLink}" class="buttonLink">{$lang.viewCollection}</a>{if $cartStatus} <a href="{$collection.addToCartLink}" class="colorButtonLink {if $config.settings.minicart}miniCartLink{/if}" rel="nofollow">{$lang.addToCart}</a>{/if}</div>
									</div>
								{/foreach}
							</div>						
						</div>
								...
								{foreach $packages as $package}
									<div class="divTableRow">
										...
										<div class="divTableCell" style="width: 100%;">
											...
											<p class="purchaseListPrice">{if $package.price}<span class="price">{$package.price.display}</span>{if $package.price.taxInc} <span class="taxIncMessage">({$lang.taxIncMessage})</span>{/if}{/if}{if $package.credits and $package.price} {$lang.priceCreditSep} {/if}{if $package.credits}<span class="price">{$package.credits} <sup>{$lang.mediaLabelCredits}</sup></span>{/if}</p>
										</div>
										{if $cartStatus}<div class="divTableCell"><a href="{$package.linkto}" class="colorButtonLink workboxLink workboxLinkAttach" rel="nofollow">{$lang.addToCart}</a></div>{/if}
									</div>
								{/foreach}
							</div>
						</div>
							...
							{if $member.allowCommenting}
								<form action="" id="newCommentForm" method="post">
									...
									<div id="newCommentContainer">
										{$lang.addNewComment}<br>								
										<textarea class="commentTextarea" name="newComment" id="newComment"></textarea><br>
										<input type="submit" value="{$lang.submit}" style="float: right;">
									</div>
								</form>
							{/if}
							...
							{if $member.allowTagging}
								<div style="float: right; color: #666; font-size: 11px; float: right; margin-top: 10px;">
									...
									<form action="" id="newTagForm" method="post">
										...
										<input type="submit" value="{$lang.submit}" style="height: 24px; margin-top: -4px; vertical-align: middle">
									</form>
								</div>
							{/if}
					...
					<div class="mediaPurchaseContainers" id="shareMedia">
						<div class="divTable" style="width: 100%">
							<div class="divTableRow">
								<div class="divTableCell" style="width: 100%;">
									<h2>{$lang.bbcode}</h2>
									<p class="purchaseListDescription"><textarea name="bbcode" rows="4" cols="100" style="width:709px">[url={$fullURL}][img]{mediaImage mediaID=$media.encryptedID type=sample folderID=$media.encryptedFID seo=$media.seoName}[/img][/url]</textarea></p>
								</div>
							...
							<div class="divTableRow">
								<div class="divTableCell" style="width: 100%;padding-top:10px">
									<h2>{$lang.html}</h2>
									<p class="purchaseListDescription"><textarea name="html" rows="4" cols="100" style="width:709px"><a href="{$fullURL}" title="{$media.title}" alt="{$media.title}"><img src="{mediaImage mediaID=$media.encryptedID type=sample folderID=$media.encryptedFID seo=$media.seoName}" title="{$media.title}" alt="{$media.title}" border="0"></a></textarea></p>
								</div>
								<div class="divTableCell"></div>
							</div>
							<div class="divTableRow">
								<div class="divTableCell" style="width: 100%;padding-top:10px">
									<h2>{$lang.link}</h2>
									<p class="purchaseListDescription"><input type="textbox" name="linkto" value="{$fullURL}" style="width:726px"></p>
								</div>
								<div class="divTableCell"></div>
							</div>
						</div>
