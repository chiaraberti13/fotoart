<div id="memberNav">
	<div class="subNavFeaturedBox">
		<p style="line-height: 1.5;">
			{*if $member.avatar}{/if*}
			{if $member.membershipDetails.avatar}<img src="{memberAvatar memID=$member.mem_id size=50}" class="memberAvatar" id="memNavAvatar">{/if}
			{$lang.loggedInAs}:<br>
	...
	{if $creditSystem}
		<div class="subNavFeaturedBox yourCredits">
			<div>{$lang.yourCredits}</div>
			<div class="myCredits">{if $member.credits}{$member.credits}{else}0{/if}</div>
			<div style="float: right; margin-bottom: 20px;"><a href="{linkto page="featured.php?mode=credits"}" class="buttonLink">{$lang.purchaseCredits}</a></div>
		</div>
		<hr>
	{/if}
