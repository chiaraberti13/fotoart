	...
	{	
		...
		*/
		if($mode == 'add')
		{
			...
			$bPP = is_photo_puzzle($id); //FP
			if($bPP) {

				$sesskey = tep_session_id();
				$aCurrent = getCurrent($sesskey);
				...
				$smarty->assign('photo_puzzle_img', $img);
			}
			...
			//if($mediaID && !$bPP) // Has a media ID check the owner
			{
			...
			$insertId = ($bPP ? $photoPuzzleId : $mediaID);
			mysqli_query($db,
				"
					...
					*/
					...
					case "product":
						...
						if($owner) // Add commission record
						{
							...
							if($product['commission_type'] == 1) // 1 = Percentage | 2 = Dollar Value
							{
		...
		{	
			...
			*/
			...
			while($cartItem = mysqli_fetch_assoc($cartItemsResult))
			{
				...
				if($cartItem['item_type'] == 'product') {//FP

					$bPP = is_photo_puzzle($cartItem['item_id']); 
					if($bPP) {
						$query = tep_db_query("SELECT * FROM ".TABLE_PHOTO_PUZZLE." WHERE photo_puzzle_id = ".$cartItem['asset_id']."");
						...
						$img = WEB_DIR.$aCurrent['sesskey'].'/'.$aCurrent['photo_puzzle_id'].'/boxes/'.$boxFile;
						$cartItemsArray[$cartItem['oi_id']]['photoPuzzleImg'] = $img;
						
						require_once BASE_PATH.'/assets/classes/invoicetools.php';
						$invoice = new invoiceTools;
						$invoiceOptions = $invoice->getOnlyItemOptions('product', $cartItem['oi_id']);
						foreach ($invoiceOptions[$cartItem['oi_id']]['optionsArray'] as $k => $aOpzione) {
							$cartItemsArray[$cartItem['oi_id']]['photoPuzzleGruppoOpzione'][] = $aOpzione['groupName'];
							$cartItemsArray[$cartItem['oi_id']]['photoPuzzleOpzione'][] = $aOpzione['name'];
						}
						//var_dump($cartItem['oi_id']);
						//var_dump($invoiceOptions['oi_id']['optionsArray']['groupName']);
						//$cartItemsArray[$cartItem['oi_id']]['photoPuzzleGruppoOpzione'] = $invoiceOptions['groupName'];
						//$cartItemsArray[$cartItem['oi_id']]['photoPuzzleOpzione'] = $invoiceOptions['name'];
						//print "<PRE>";
						//print_r(($invoiceOptions));
						//print_r(());
						//print "</PRE>";
					}
				}
