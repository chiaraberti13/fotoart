	...
	*/
	...
	try
	{
		...
		foreach ( $physicalInvoiceItems as $k => $array ){
			$invoiceOptions = $invoice->getOnlyItemOptions('product', $k);
			foreach ($invoiceOptions[$k]['optionsArray'] as $i => $aOpzione) {
				$physicalInvoiceItems[$k]['photoPuzzleGruppoOpzione'][] = $aOpzione['groupName'];
				$physicalInvoiceItems[$k]['photoPuzzleOpzione'][] = $aOpzione['name'];
			}
