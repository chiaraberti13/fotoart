		...
		//{	
			...
			if($orderID)
			{
				foreach($invoiceItems as $invoiceItemKey => $invoiceItem)
				{
					...
					elseif ($bPP){
						$invoiceItems[$invoiceItemKey]['thumbnail'] = "<img src='".$img."' style='width: 45px;'/>";
						//var_dump($invoiceItems[$invoiceItemKey]);
						foreach($invoiceItems[$invoiceItemKey]['photoPuzzleGruppoOpzione'] as $grpOpzioneKey => $opzione) {
							
							$invoiceItems[$invoiceItemKey]['name'] = $invoiceItems[$invoiceItemKey]['name'].' - '.$lang[$opzione].': '.$invoiceItems[$invoiceItemKey]['photoPuzzleOpzione'][$grpOpzioneKey];
						}
