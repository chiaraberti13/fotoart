		...
		switch($_REQUEST['action']){
			...
			case "save_edit":							
				...
				if($_FILES['uploadedfile']['name'] != ""){
					...
					$sql = "INSERT INTO {$dbinfo[pre]}content_files (content_id,file_name,type) VALUES ('$saveid','$fname','agreements')";
				...
				# UPDATE ACTIVITY LOG
				save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_agreements'],1,$mgrlang['gen_b_ed'] . " > <strong>$name</strong>");
				
				header("location: mgr.agreements.php?mes=edit"); exit;
			break;
			# SAVE NEW ITEM
			case "save_new":
				# CONVERT POST & GET ARRAYS TO LOCAL VALUES AND CLEAN DATA				
				...
				# UPDATE ACTIVITY LOG
				save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_agreements'],1,$mgrlang['gen_b_new'] . " > <strong>$name</strong>");
				
				header("location: mgr.agreements.php?mes=new"); exit;
			break;
		}		
?>
		...
		function form_sumbit(){
			...
			<?php
				if($_SESSION['admin_user']['admin_id'] == "DEMO"){
					...
					$action_link = ($_GET['edit'] == "new") ? "mgr.agreements.edit.php?action=save_new" : "mgr.agreements.edit.php?action=save_edit";
	...
	<div id="container">
        ...
        ?>
            ...
            <div id="title_bar">
                ...
                <p><strong><?php echo ($ptitle = ($_GET['edit'] == "new") ? $mgrlang['agree_new_header'] : $mgrlang['agree_edit_header']); ?></strong><br /><span><?php echo ($pdescr = ($_GET['edit'] == "new") ? $mgrlang['agree_new_message'] : $mgrlang['agree_edit_message']); ?></span></p>
            ...
            <div id="content" style="padding: 0px;">                
                ...
                <div id="tab1_group" class="group" style="display: block;">
					...
					?>
                        <div class="<?php fs_row_color(); ?>" id="name_div">
                            ...
                            <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                                ...
                                <span><?php echo $mgrlang['gen_code_link_d']; ?></span>
                            </p>
                            <div style="float: left; margin-top: 10px;"><textarea style="width: 300px; height: 50px;"><a href="{$baseURL}/content.php?id=<?php echo $content->content_id; ?>">{content id=<?php echo $content->content_id; ?> titleOnly=1}</a></textarea></div>
                        </div>
                    <?php
						}
					?>
					...
					<div class="<?php fs_row_color(); ?>" id="name_div">
                        ...
                        <p class="input_label" onclick="support_popup('<?php echo $supportPageID; ?>');"><?php echo $mgrlang['agree_f_name']; ?>:<br />
                        <span class="input_label_subtext"><?php echo $mgrlang['agree_f_name_d']; ?></span></p>
...
<textarea style="width: 375px; height: 135px; background-color: #fdf9d0; border: 0; padding: 10px; border: 1px solid #CCC"><?php include 'mgr.clips.php'; ?></textarea>
                    ...
                    <div class="<?php fs_row_color(); ?>">
                        ...
                        <p for="x" onclick="support_popup('<?php echo $supportPageID; ?>');">
                            ...
                            <span class="input_label_subtext"><?php echo $mgrlang['agree_f_content_d']; ?></span>
                        </p>
                    ...
                    <div class="<?php fs_row_color(); ?>">
						...
						<?php
						#GET FILES LIST
						$file_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}content_files WHERE content_id = '$_GET[edit]' AND type = 'agreements' ORDER BY file_name ASC");
            ...
            <div id="save_bar">
                <input type="button" value="<?php echo $mgrlang['gen_b_cancel']; ?>" onclick="cancel_edit('mgr.agreements.php');" /><input type="submit" value="<?php echo $mgrlang['gen_b_save']; ?>" />
            </div>
            </form>
