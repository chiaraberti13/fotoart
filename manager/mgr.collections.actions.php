	...
	###################################################################
		...
		switch($_REQUEST['mode'])
		{
			...
			case "galleries":
				...
				# PULL COLLECTION DETAILS FROM DB	
				if($_GET['id'] != "new"){
					$coll_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}collections WHERE coll_id = '$_GET[id]'");
			...
			case "display_ip_list":
				//sleep(1);
				$ip_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}item_photos WHERE item_id = '$_GET[id]' AND mgrarea = 'coll' ORDER BY ip_id DESC");
				$ip_rows = mysqli_num_rows($ip_result);
				while($ip = mysqli_fetch_object($ip_result))
				{
					$coll = zerofill($_GET['id'],4);
					$ip_id = zerofill($ip->ip_id,4);				
					$src_img = "coll".$coll."_ip".$ip_id."_small.jpg";				
					...
					*/
					...
					echo "<div class='ip_div' id='ip_$ip->ip_id'>";
						...
						echo "<div class='ip_div_inner' style='width: ".$newsize[0]."px;'><img src='mgr.collections.actions.php?mode=display_ip&coll=$_GET[id]&ip_id=$ip->ip_id' /></div>";
			...
			break;
			case "delete_ip":
				delete_item_photo('coll',$_GET['coll'],$_GET['ip_id']);
				echo "<script>load_ip();</script>";
			break;
			# SET ACTIVE STATUS
			case "ac":
				$coll_result = mysqli_query($db,"SELECT active,item_name FROM {$dbinfo[pre]}collections where coll_id = '$_REQUEST[id]'");
				$coll = mysqli_fetch_object($coll_result);
				
				# FLIP THE VALUE
				$new_value = (empty($coll->active) ? 1 : 0);	
							
				$sql = "UPDATE {$dbinfo[pre]}collections SET active='$new_value' where coll_id = '$_REQUEST[id]'";
				$result = mysqli_query($db,$sql);
				
				$save_type = ($new_value==1) ? $mgrlang['gen_active'] : $mgrlang['gen_inactive'];
				
				# UPDATE ACTIVITY LOG
				save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_collections'],1,$save_type . " > <strong>$coll->item_name ($_REQUEST[id])</strong>");

				echo "<a href=\"javascript:switch_status('ac','$_REQUEST[id]');\"><img src=\"images/mgr.small.check." . $new_value . ".png\" border=\"0\" /></a>";
			break;
			# SHOW DETAILS
			case "details":
				$coll_result = mysqli_query($db,"SELECT notes,description FROM {$dbinfo[pre]}collections where coll_id = '$_REQUEST[id]'");
				$coll = mysqli_fetch_object($coll_result);
				
				echo "<div style='padding: 20px;'>";
					if($coll->notes){ echo "<strong>{$mgrlang[gen_notes]}:</strong> $coll->notes<br /><br />"; }
					if($coll->description){ echo "<strong>{$mgrlang[gen_description]}:</strong> $coll->description"; }
					if(!$coll->notes and !$coll->description){ echo $mgrlang['gen_no_notes']; }
				echo "</div>";
			break;
		}	
?>
