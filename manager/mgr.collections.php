<?php
	...
	###################################################################
		...
		switch($_REQUEST['action']){
			case "save_groups":				
				save_groups($page,'collections','coll_id');				
			break;			
			case "save_sort":
				save_sort('collections','coll_id');
			break;
			case "clear_sort":
				clear_sort('collections');
			break;
			case "save_status":
				save_status($page,'collections','coll_id');
			break;
			# DELETE
			case "del":
				if(!empty($_REQUEST['items'])){
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT item_name,coll_id FROM {$dbinfo[pre]}collections WHERE coll_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result)){
						$log_titles.= "$log->item_name ($log->coll_id), ";
						
						# MARK AS DELETED
						@$sql = "UPDATE {$dbinfo[pre]}collections SET deleted='1' WHERE coll_id = '$log->coll_id'";
						@$result = mysqli_query($db,$sql);
						
						# REMOVE ITEM GALLERIES
						mysqli_query($db,"DELETE FROM {$dbinfo[pre]}item_galleries WHERE item_id = '$log->coll_id' AND mgrarea = '$page'");
						
						# REMOVE ANY collection SHOTS
						$ip_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}item_photos WHERE item_id = '$log->coll_id' AND mgrarea = 'coll'");
						...
						while($ip = mysqli_fetch_object($ip_result))
						{
							...
							$collzf = zerofill($log->coll_id,4);
							
							# DELETE SMALL, MED, ORG ITEM PHOTOS
							if(file_exists("../assets/item_photos/coll".$collzf."_ip".$ipidzf."_org.jpg"))
							{
								unlink("../assets/item_photos/coll".$collzf."_ip".$ipidzf."_org.jpg");
							}
							if(file_exists("../assets/item_photos/coll".$collzf."_ip".$ipidzf."_med.jpg"))
							{
								unlink("../assets/item_photos/coll".$collzf."_ip".$ipidzf."_med.jpg");
							}
							if(file_exists("../assets/item_photos/coll".$collzf."_ip".$ipidzf."_small.jpg"))
							{
								unlink("../assets/item_photos/coll".$collzf."_ip".$ipidzf."_small.jpg");
							}
						}
					}
					
					# REMOVE MEDIA ENTRIES
					mysqli_query($db,"DELETE FROM {$dbinfo[pre]}media_collections WHERE coll_id IN ($delete_array)");
					
					# REMOVE PACKAGE ITEMS ENTRIES
					mysqli_query($db,"DELETE FROM {$dbinfo[pre]}package_items WHERE item_type = 'coll' AND item_id IN ($delete_array)");
					...
					# UPDATE ACTIVITY LOG
					save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_collections'],1,$mgrlang['gen_b_del'] . " > <strong>$log_titles</strong>");
		...
		$sortprefix="collections";
		$datasorts = new data_sorting;
		$datasorts->prefix = $sortprefix;
		$datasorts->clear_sorts($_GET['ep']);
		$id_field_name = "coll_id";			
		require_once('mgr.datasort.logic.php');	

		# IF THIS IS AN ENTRY PAGE OR collectiongroups IS BLANK RESET THE collectiongroups SESSION	
		if($_GET['ep'] or empty($_SESSION['collectiongroups'])){
			$_SESSION['collectiongroups'] = array('all');
		}			
		# SEE IF ANY GROUPS HAVE BEEN PASSED
		if($_REQUEST['setgroups']){
			if(is_array($_REQUEST['setgroups'])){
				$_SESSION['collectiongroups'] = $_REQUEST['setgroups'];
			} else {				
				$_SESSION['collectiongroups'] = array($_REQUEST['setgroups']);
			}
		}	
		
		# GET THE TOTAL NUMBER OF ROWS
		if(in_array("all",$_SESSION['collectiongroups']))
		{
			$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(coll_id) FROM {$dbinfo[pre]}collections WHERE deleted='0'"));
		}
		else
		{
			$coll_result2 = "SELECT COUNT(coll_id) FROM {$dbinfo[pre]}collections LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}collections.coll_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['collectiongroups']).") AND {$dbinfo[pre]}collections.deleted='0'";
			$r_rows = mysqli_result_patch(mysqli_query($db,$coll_result2));
		}
		//echo "exit:".$r_rows; exit;
		...
		# SELECT ITEMS
		if(in_array("all",$_SESSION['collectiongroups'])){
			$collection_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}collections WHERE deleted = '0' ORDER BY $listby $listtype");
		} else {
			$collection_result = mysqli_query($db,"SELECT coll_id,sortorder,active,item_name,item_name_".$config['settings']['lang_file_mgr']." FROM {$dbinfo[pre]}collections LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}collections.coll_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['collectiongroups']).") AND {$dbinfo[pre]}collections.deleted = '0' GROUP BY {$dbinfo[pre]}collections.coll_id ORDER BY $listby $listtype");
		}
		$collection_rows = mysqli_num_rows($collection_result);
?>
...
<head>
	...
	<script language="javascript">
		...
		Event.observe(window, 'load', function()
			...
			if($('abutton_add_new')!=null)
			{
				$('abutton_add_new').observe('click', function()
					{
						window.location.href='mgr.collections.edit.php?edit=new';
					});
			...
			<?php
				...
				if(in_array('all',$_SESSION['collectiongroups']) or $_GET['setgroups'])
				{
		...
		function load_group_selector()
		{
			...
			var loadpage = "mgr.groups.actions.php?mode=grouplist&mgrarea=<?php echo $page; ?>&ingroups=<?php if(in_array('all',$_SESSION['collectiongroups'])){ echo 0; } else { echo 1; } ?>&exitpage=<?php echo $_SERVER['PHP_SELF']; ?>&sessname=collectiongroups";
		...
		function switch_status(item_type,item_id){
			...
			else
			{
				...
				var loadpage = "mgr.collections.actions.php?mode=" + item_type + "&id=" + item_id;
				var pars = "";
		...
		function clear_sorts(){
			location.href='<?php echo $_SERVER['PHP_SELF']; ?>?ep=1&action=clear_sort&listby=coll_id&listtype=asc';
		}
...
<body>
	...
	<div id="container">
        ...
        <div id="content_container">
            ...
            <!-- TITLE BAR AREA -->
            <?php
				$collection_group_result = mysqli_query($db,"SELECT gr_id,flagtype,name FROM {$dbinfo[pre]}groups WHERE mgrarea = '$page' ORDER BY name");
				$collection_group_rows = mysqli_num_rows($collection_group_result);
			?>
            <!-- ACTIONS BAR AREA -->
            <div id="actions_bar">	
            	<div class="sec_bar">
                    <img src="./images/mgr.badge.collection.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_collections']; ?></span> &nbsp; 
                </div>                
                ...
                <div style="padding-top: 9px; margin-right: 20px;"> 
                    ...
                    <select align="absmiddle" id="actionsdd" onchange="do_actions();">
                        ...
                        <?php if($collection_group_rows and $collection_rows and in_array("pro",$installed_addons)){ ?><option value="assign_groups">&nbsp; <?php echo $mgrlang['gen_au_itg']; ?></option><?php } ?>
                        <option value="set_status">&nbsp; <?php echo $mgrlang['gen_tostatus']; ?></option>
                        <?php if($collection_rows >= 2){ ?><option value="sort_items">&nbsp; <?php echo $mgrlang['gen_sort']; ?></option><?php } ?>
                    </select>
                   	<?php
						}
					?>
			...
			<form name="grouplist" id="grouplist" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
				...
				<div style="<?php if(in_array('all',$_SESSION['collectiongroups'])){ echo "display: none;"; } else { echo "display: block;"; } ?>" class="options_area" id="group_selector"></div>
			</form>
            <!-- START CONTENT -->
            <?php
                ...
                <div id="content" style="border-top: 0px;">						
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
							...
							<?php $header_name = "coll_id";	if($listby == $header_name){ $sel = "_sel"; } else { $sel = ""; }	$hlink = ""; $hlink = $_SERVER['PHP_SELF'] . "?listby=" . $header_name . "&listtype="; if($listtype == "asc" && $listby == $header_name){ $hlink .= "desc"; } else { $hlink .= "asc"; } ?><td class="cth<?php echo $sel; ?>" onclick="window.location.href='<?php echo $hlink; ?>'" align="center" width="100">	<div class="content_table_header<?php echo $sel; ?>"><div><a href="<?php echo $hlink ?>"><?php echo $mgrlang['gen_t_id']; ?></a></div></div></td>
                        ...
                        <?php								
                            ...
                            while($collection = mysqli_fetch_object($collection_result)){
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onMouseOver="cellover(this,'#<?php echo $color_fade; ?>',32);" onMouseOut="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"></a><?php if($collection->sortorder) { echo $collection->sortorder;} else { echo "--"; } ?></td>
                                <td align="center"><a name="row_<?php echo $collection->coll_id; ?>"></a><?php echo $collection->coll_id; ?></td>
                                <td onclick="window.location.href='mgr.collections.edit.php?edit=<?php echo $collection->coll_id; ?>'">
                                	<div style="float: left;"><a href="mgr.collections.edit.php?edit=<?php echo $collection->coll_id; ?>" class="editlink" onMouseOver="show_details_win('details_<?php echo $collection->coll_id; ?>','mgr.collections.actions.php?mode=details&id=<?php echo $collection->coll_id; ?>');" onMouseOut="hide_gdetails('details_<?php echo $collection->coll_id; ?>');"><?php echo $collection->item_name; ?></a></div>
                                	<div style="float: left; overflow: visible;">
										<div id="details_<?php echo $collection->coll_id; ?>" class="galdet_win" style="display: none;">Loading Details</div>
									</div>
                                </td>
                                <td align="center"><div id="ac<?php echo $collection->coll_id; ?>"><a href="javascript:switch_status('ac','<?php echo $collection->coll_id; ?>');"><img src="images/mgr.small.check.<?php echo $collection->active; ?>.png" border="0" /></a></div></td>
                                <td align="center" valign="middle" nowrap>
                                    <a href="mgr.collections.edit.php?edit=<?php echo $collection->coll_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a>
                                    <a href="javascript:deleterec(<?php echo $collection->coll_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a> 
                                <td align="center" valign="middle" nowrap>
                                    <input type="checkbox" name="items[]" value="<?php echo $collection->coll_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
                            }
