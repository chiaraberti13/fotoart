<?php
	...
	###################################################################
		...
		switch($_REQUEST['action'])
		{
			# SET ACTIVE STATUS
			case "ac":
				$country_result = mysqli_query($db,"SELECT active FROM {$dbinfo[pre]}countries where country_id = '$_REQUEST[id]'");
				$country = mysqli_fetch_object($country_result);
				
				# FLIP THE VALUE
				$new_value = (empty($country->active) ? 1 : 0);	
							
				$sql = "UPDATE {$dbinfo[pre]}countries SET active='$new_value' where country_id = '$_REQUEST[id]'";
