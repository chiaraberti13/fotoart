	...
	###################################################################
		...
		{		
			case "save_groups":				
				...
				save_groups($page,'countries','country_id');				
			break;
			case "del":
				if(!empty($_REQUEST['items']))
				{
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT name,country_id FROM {$dbinfo[pre]}countries WHERE country_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result))
					{
						$log_titles.= "$log->name ($log->country_id), ";
					}
					...
					# FIND AND DELETE STATE TAXES
					$states_result = mysqli_query($db,"SELECT state_id FROM {$dbinfo[pre]}states WHERE country_id IN ($delete_array)");
					while($states = mysqli_fetch_object($states_result))
					{
						# DELETE STATE TAXES
						@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}taxes WHERE region_type = '2' AND region_id = '$states->state_id'");
						
						# FIND STATE ZIP CODES
						$zipcode_result = mysqli_query($db,"SELECT zipcode_id FROM {$dbinfo[pre]}zipcodes WHERE state_id = '$states->state_id'");
						...
						# DELETE STATE ZIPS
						@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}zipcodes WHERE state_id = '$states->state_id'");
					}
					
					# DELETE STATES
					//@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}states WHERE country_id IN ($delete_array)");
					# MARK AS DELETED
					@$sql = "UPDATE {$dbinfo[pre]}states SET deleted='1' WHERE country_id IN ($delete_array)";
					@$result = mysqli_query($db,$sql);
					
					# FIND AND DELETE COUNTRY ZIP TAXES
					$zipcode_result = mysqli_query($db,"SELECT country_id FROM {$dbinfo[pre]}zipcodes WHERE country_id IN ($delete_array)");
					while($zipcode = mysqli_fetch_object($zipcode_result))
					{
						# DELETE ZIP TAXES
						@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}taxes WHERE region_type = '3' AND region_id = '$zipcode->zipcode_id'");
					...
					# DELETE COUNTRY ZIPS
					//@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}zipcodes WHERE country_id IN ($delete_array)");
					# MARK AS DELETED
					@$sql = "UPDATE {$dbinfo[pre]}zipcodes SET deleted='1' WHERE country_id IN ($delete_array)";
					@$result = mysqli_query($db,$sql);
					
					# DELETE COUNTRY TAXES
					@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}taxes WHERE region_type = '1' AND region_id IN ($delete_array)");
					
					# DELETE COUNTRIES
					//@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}countries WHERE country_id IN ($delete_array)");
					# MARK AS DELETED
					@$sql = "UPDATE {$dbinfo[pre]}countries SET deleted='1' WHERE country_id IN ($delete_array)";
					@$result = mysqli_query($db,$sql);
					...
					# UPDATE ACTIVITY LOG
					save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_countries'],1,$mgrlang['gen_b_del'] . " > <strong>$log_titles</strong>");
			...
			case "save_status":
				save_status($page,'countries','country_id');
			break;
		}
		...
		$sortprefix="countries";
		$datasorts = new data_sorting;
		$datasorts->prefix = $sortprefix;
		$datasorts->clear_sorts($_GET['ep']);
		$id_field_name = "country_id";		
		require_once('mgr.datasort.logic.php');	
		
		
		# IF THIS IS AN ENTRY PAGE OR countriesgroups IS BLANK RESET THE countriesgroups SESSION	
		if($_GET['ep'] or empty($_SESSION['countriesgroups']))
		{
			$_SESSION['countriesgroups'] = array('all');
		}			
		...
		if($_REQUEST['setgroups'])
		{
			...
			{
				$_SESSION['countriesgroups'] = $_REQUEST['setgroups'];
			}
			else
			{				
				$_SESSION['countriesgroups'] = array($_REQUEST['setgroups']);
			}
		}
		
		# GET THE TOTAL NUMBER OF ROWS
		if(in_array("all",$_SESSION['countriesgroups']))
		{
			$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(country_id) FROM {$dbinfo[pre]}countries WHERE deleted='0'"));
		}
		else
		{
			$country_result2 = "SELECT COUNT(country_id) FROM {$dbinfo[pre]}countries LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}countries.country_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['countriesgroups']).") AND deleted='0'";
			$r_rows = mysqli_result_patch(mysqli_query($db,$country_result2));
		}
		
		//$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(country_id) FROM {$dbinfo[pre]}countries"));
		...
		if(in_array("all",$_SESSION['countriesgroups']))
		{
			$country_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}countries WHERE deleted='0' ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
		}
		else
		{				
			$country_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}countries LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}countries.country_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['countriesgroups']).") AND deleted='0' GROUP BY {$dbinfo[pre]}countries.country_id ORDER BY $listby $listtype LIMIT $startrecord,$perpage"); 				
		}
	...
	<meta http-equiv=refresh content="<?php echo $config['timeout']; ?>; url=mgr.login.php?notice=timed_out" />
		...
		function switch_status(item_type,item_id)
		{
			...
			else
			{
				...
				var loadpage = "mgr.countries.actions.php?action=" + item_type + "&id=" + item_id;
				var pars = "";
		...
		Event.observe(window, 'load', function()
			...
			if($('abutton_add_new')!=null)
			{
				$('abutton_add_new').observe('click', function()
					{
						window.location.href='mgr.countries.edit.php?edit=new';
					});
		...
		function load_group_selector()
		{
			...
			var loadpage = "mgr.groups.actions.php?mode=grouplist&mgrarea=<?php echo $page; ?>&ingroups=<?php if(in_array('all',$_SESSION['countriesgroups'])){ echo 0; } else { echo 1; } ?>&exitpage=<?php echo $_SERVER['PHP_SELF']; ?>&sessname=countriesgroups";
...
<body>
	...
	<div id="container">
        ...
        <div id="content_container">
            ...
            <div id="actions_bar">							
                <div class="sec_bar">
                    <img src="./images/mgr.badge.countries.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_countries']; ?></span> &nbsp; 
                </div>							
                ...
                <div style="padding-top: 9px; margin-right: 20px;">
                    ...
                    <select align="absmiddle" id="actionsdd" onchange="do_actions();">
                        ...
                        <?php if($country_group_rows and $r_rows and in_array("pro",$installed_addons)){ ?><option value="assign_groups">&nbsp; <?php echo $mgrlang['gen_au_itg']; ?></option><?php } ?>
                    </select>
                   	<?php
						}
					?>
			...
			<form name="grouplist" id="grouplist" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
				...
				<div style="<?php if(in_array('all',$_SESSION['countriesgroups'])){ echo "display: none;"; } else { echo "display: block;"; } ?>" class="options_area" id="group_selector"></div>
			</form>             
            <!-- START CONTENT -->
            <?php
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <?php								
                            ...
                            while($country = mysqli_fetch_object($country_result))
							{
								...
								{
                                    $title = $country->{"name_" . $config['settings']['lang_file_mgr']};
                                }
								else
								{
                                    $title = $country->name;
                                ...
                                # IF THE TITLE IS TOO LONG CROP IT / ALSO TAKE OUT ALL HTML IF THERE IS ANY
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onmouseover="cellover(this,'#<?php echo $color_fade; ?>',32);" onmouseout="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"><a name="row_<?php echo $country->country_id; ?>"></a><?php echo $country->country_id; ?></td>
                                <td align="center"><a name="row_<?php echo $country->country_id; ?>"></a><?php echo $country->code2; ?></td>
                                <td onclick="window.location.href='mgr.countries.edit.php?edit=<?php echo $country->country_id; ?>'"><a href="mgr.countries.edit.php?edit=<?php echo $country->country_id; ?>" class="editlink"><?php echo $title; ?></a>&nbsp;</td>
                                <td align="center"><div id="ac<?php echo $country->country_id; ?>"><a href="javascript:switch_status('ac','<?php echo $country->country_id; ?>');"><img src="images/mgr.small.check.<?php echo $country->active; ?>.png" border="0" /></a></div></td>
                                <td align="center" valign="middle" nowrap>
                                    <a href="mgr.countries.edit.php?edit=<?php echo $country->country_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a> 
                                    <a href="javascript:deleterec(<?php echo $country->country_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a>
                                </td>
                                <td align="center" valign="middle" nowrap>                                                
                                    <input type="checkbox" name="items[]" value="<?php echo $country->country_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
                            }
