<?php
	...
	###################################################################
		...
		if($_GET['edit'] != "new" and !$_REQUEST['action']){
			$credit_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}credits WHERE credit_id = '$_GET[edit]'");
			$credit_rows = mysqli_num_rows($credit_result);
			$credit = mysqli_fetch_object($credit_result);
			
			$_SESSION['item_id'] = $credit->credit_id;
			$_SESSION['mgrarea'] = 'credit';
			
			$downitems = explode(",",$credit->downitems);
		}
		
		if($_GET['edit'] == "new")
		{
			# DELETE ANY ORPHANED ITEM PHOTOS
			delete_orphaned_item_photos('credit');
		...
		if($_REQUEST['action'] == "save_edit" or $_REQUEST['action'] == "save_new" ){
			...
			if($downitems){ $downitems = implode(",",$downitems); }
			...
			if($_REQUEST['action'] == "save_edit"){
				...
				# UPDATE THE DATABASE
				$sql = "UPDATE {$dbinfo[pre]}credits SET 
							name='$name',
				...
				# UPDATE ACTIVITY LOG
				save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_credits'],1,$mgrlang['gen_b_ed'] . " > <strong>$name</strong>");
			}
			
			if($_REQUEST['action'] == "save_new"){
				
				$ucredit_id = create_unique2();
				...
				$sql = "INSERT INTO {$dbinfo[pre]}credits (
						name,
						description,
						ucredit_id,
						item_code,
				...
				# RENAME ORPHANED ITEM PHOTOS UNDER THIS credits
				save_new_item_photos('credit',$saveid);
				...
				# UPDATE ACTIVITY LOG
				save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_credits'],1,$mgrlang['gen_b_new'] . " > <strong>$name</strong>");
			}				
			header("location: mgr.credits.php"); exit;
		}
?>
...
<head>
	...
	<title><?php echo $manager_page_title . " : " . $mgrlang['subnav_credits']; ?></title>
	...
	<script language="javascript">	
		function form_sumbit(){
			...
			<?php
				if($_SESSION['admin_user']['admin_id'] == "DEMO"){
					...
					$action_link = ($_GET['edit'] == "new") ? "mgr.credits.edit.php?action=save_new" : "mgr.credits.edit.php?action=save_edit";
				
					# CHECK FIELD AND OUTPUT MESSAGE - ID, LANGUAGE, BRING TO FRONT, CUSTOM MESSAGE
					js_validate_field("name","credits_f_name",1);
			?>
		...
		?>
		uploadobj.id = '<?php echo $credit->credit_id; ?>'; // THE ID OF THE ITEM
		...
		function load_ip()
		{
			...
			var myAjax = new Ajax.Updater('ip_div', 'mgr.credits.actions.php', {method: 'get', parameters: pars});
		}
		...
		function do_delete_ip(ip_id)
		{
			...
			setTimeout(function(){
					var pars = 'pmode=delete_ip&credit=<?php echo $_SESSION['item_id']; ?>&ip_id='+ip_id;
					//var myAjax = new Ajax.Updater('ip_div', 'mgr.prints.actions.php', {method: 'get', parameters: pars, evalScripts: true});
					new Ajax.Request('mgr.credits.actions.php', {method: 'get', parameters: pars, onSuccess: function() {
						$('ip_'+ip_id).remove();
					} });					
				},500);
		}
...
<body>
	...
	<div id="container">
        ...
        ?>
            ...
            <!-- TITLE BAR AREA -->
            <div id="title_bar">
                <img src="./images/mgr.badge.credits.png" class="badge" />
                <p><strong><?php echo ($ptitle = ($_GET['edit'] == "new") ? $mgrlang['credits_new_header'] : $mgrlang['credits_edit_header']); ?></strong><br /><span><?php echo ($pdescr = ($_GET['edit'] == "new") ? $mgrlang['credits_new_message'] : $mgrlang['credits_edit_message']); ?></span></p>
            ...
            <div id="content" style="padding: 0px;">
				...
				<?php
					# PULL GROUPS
					$credit_group_result = mysqli_query($db,"SELECT gr_id,flagtype,name FROM {$dbinfo[pre]}groups WHERE mgrarea = '$page' ORDER BY name");
					$credit_group_rows = mysqli_num_rows($credit_group_result);
				?>
                <div id="button_bar">
                    <div class="subsubon" onclick="bringtofront('1');" id="tab1"><?php echo $mgrlang['credits_tab1']; ?></div>
                    <div class="subsuboff" onclick="bringtofront('4');load_ip();" id="tab4"><?php echo $mgrlang['gen_tab_prod_shots']; ?></div>
                    <?php if($credit_group_rows and in_array("pro",$installed_addons)){ ?><div class="subsuboff" onclick="bringtofront('2');" id="tab2"><?php echo $mgrlang['credits_tab2']; ?></div><?php } ?>
					<?php if(in_array("proXXX",$installed_addons)){ ?><div class="subsuboff" onclick="bringtofront('6');loadDiscountsBox();" id="tab6"><?php echo $mgrlang['gen_tab_discounts']; ?></div><?php } ?>
                ...
                <div id="tab1_group" class="group" style="display: block;">
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        ...
                        <p class="input_label" onclick="support_popup('<?php echo $supportPageID; ?>');"><?php echo $mgrlang['credits_f_name']; ?>:<br />
                        <span class="input_label_credittext"><?php echo $mgrlang['credits_f_name_d']; ?></span></p>
                        <div class="additional_langs">
                            <input type="text" name="name" id="name" style="width: 300px;" maxlength="100" value="<?php echo @stripslashes($credit->name); ?>" />
                            <?php
                            ...
                            ?>
                                ...
                                <?php
									...
									{
                                ?>
                                    <li><input type="text" name="name_<?php echo $value; ?>" id="name_<?php echo $value; ?>" style="width: 300px;" maxlength="100" value="<?php echo @stripslashes($credit->{"name" . "_" . $value}); ?>" />&nbsp;&nbsp;<span class="mtag_dblue" style="color: #FFF"><?php echo ucfirst($value); ?></span></li>
                            <?php
                                    }
                                    echo "</ul></div>";
                                }
                    ...
                    <div class="<?php fs_row_color(); ?>">
                        ...
                        <p for="active" onclick="support_popup('<?php echo $supportPageID; ?>');">
                            ...
                            <span class="input_label_credittext"><?php echo $mgrlang['gen_description_d']; ?></span>
                        </p>
                        <div class="additional_langs">
                            <textarea name="description" id="description" style="width: 300px; height: 50px; vertical-align: middle"><?php echo @stripslashes($credit->description); ?></textarea>
                            <?php
                                if(in_array('multilang',$installed_addons))
								{
                            ?>
                                ...
                                <?php
									...
									{
                                ?>
                                    <li><textarea name="description_<?php echo $value; ?>" style="width: 300px; height: 50px; vertical-align: middle"><?php echo @stripslashes($credit->{"description" . "_" . $value}); ?></textarea>&nbsp;&nbsp;<span class="mtag_dblue" style="color: #FFF"><?php echo ucfirst($value); ?></span></li>
                            <?php
                                    }
                                    echo "</ul></div>";
                                }
                    ...
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        ...
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                            <?php echo $mgrlang['credits_f_price']; ?>:<br />
                            <span><?php echo $mgrlang['credits_f_price_d']; ?></span>
                        </p>
                        <div style="float: left;">
                            <input type="text" name="price" id="price" style="width: 80px;" maxlength="50" onblur="update_input_cur('price');" value="<?php if($credit->price > 0){ echo @$cleanvalues->currency_display($credit->price); } ?>" /> <?php $cleanvalues->example_currency_text(5.00,6.50); ?>
                    ...
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        ...
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                            ...
                            <span><?php echo $mgrlang['credits_f_amount_d']; ?></span>
                        </p>
                        <div style="float: left;">
                            <input type="text" name="credits" id="credits" style="width: 80px;" maxlength="50" value="<?php echo $credit->credits; ?>" />
                        </div>
                    </div>
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        ...
                        <input type="checkbox" name="taxable" value="1" style="padding: 0; margin: 6px 0 0 0;" <?php if($credit->taxable or $_GET['edit'] == 'new'){ echo "checked"; } ?> />
                    </div>
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                        ...
                        <input type="checkbox" name="active" value="1" style="padding: 0; margin: 6px 0 0 0;" <?php if($credit->active or $_GET['edit'] == 'new'){ echo "checked"; } ?> />
                    </div>
            	</div>
                
                <?php
            	if($credit_group_rows)
				{
						$row_color = 0;
				?>
					<div id="tab2_group" class="group"> 
						<div class="<?php fs_row_color(); ?>" id="name_div">
							<img src="images/mgr.ast.off.gif" class="ast" />
							<p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
								<?php echo $mgrlang['credits_f_groups']; ?>:<br />
								<span><?php echo $mgrlang['credits_f_groups_d']; ?></span>
							</p>
							<?php
								$plangroups = array();
								# FIND THE GROUPS THAT THIS ITEM IS IN
								$credit_groupids_result = mysqli_query($db,"SELECT group_id FROM {$dbinfo[pre]}groupids WHERE mgrarea = '$page' AND item_id = '$credit->credit_id' AND item_id != 0");
								while($credit_groupids = mysqli_fetch_object($credit_groupids_result))
								{
									$plangroups[] = $credit_groupids->group_id;
								}
								echo "<ul style='float: left; margin: 0px; list-style-type:none; padding: 0;'>";
								while($credit_group = mysqli_fetch_object($credit_group_result))
								{
									echo "<li><input type='checkbox' id='grp_$credit_group->gr_id' class='permcheckbox' name='setgroups[]' value='$credit_group->gr_id' "; if(in_array($credit_group->gr_id,$plangroups)){ echo "checked "; } echo "/> "; if($credit_group->flagtype == 'icon.none.gif'){ echo "<img src='./images/mini_icons/$credit_group->flagtype' align='absmiddle' width='1' height='16' /> "; } else { echo "<img src='./images/mini_icons/$credit_group->flagtype' align='absmiddle' /> "; } echo "<label for='grp_$credit_group->gr_id'>" . substr($credit_group->name,0,30)."</label></li>";
								}
								echo "</ul>";
							?>
						</div>
                ...
                <div id="tab3_group" class="group">
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        ...
                        <input type="text" name="item_code" id="item_code" style="width: 80px;" maxlength="50" value="<?php echo @stripslashes($credit->item_code); ?>" />
                    </div>
                    <?php
					...
					?>
                        ...
                        <?php
							if($_GET['edit'] != 'new' and $credit->everyone == '0'){
						...
						?>
                        <div style="float: left; padding-right: 10px; font-weight: bold; <?php if($credit->everyone == 1 or $_GET['edit'] == 'new'){ echo "display: block;"; } else { echo "display: none;"; }  ?>" id="perm_div_a"><?php echo $mgrlang['gen_wb_everyone']; ?></div>
						<div style="float: left; padding-right: 10px; font-weight: bold; <?php if($credit->everyone == 0 and $_GET['edit'] != 'new'){ echo "display: block;"; } else { echo "display: none;"; } ?>" id="perm_div_b"><?php echo $mgrlang['gen_wb_limited']; ?></div>	
                        <input type="hidden" name="perm" id="perm<?php echo $credit->credit_id; ?>" value="<?php echo $perm_value; ?>" />
                        <div style="float: left;"><a href="javascript:workbox2({page: 'mgr.workbox.php',pars: 'box=permissions_selector&style=everyone&inputbox=perm<?php echo $credit->credit_id; ?>&multiple=1'});" class="actionlink"><img src="images/mgr.people.icon.png" border="0" id="perm_icon" align="middle" /></a></div>
                    </div> 
                    ...
                    <div class="<?php fs_row_color(); ?>">
                        ...
                        <textarea name="notes" id="notes" style="width: 290px; height: 50px; vertical-align: middle"><?php echo @stripslashes($credit->notes); ?></textarea>
                    </div>
                </div>
                ...
                <div id="tab5_group" class="group">
                    <div class="<?php fs_row_color(); ?>">
                        <?php
							if($config['settings']['hpcredits'] == 0){
						?>
                        ...
                        <input type="checkbox" name="homepage" value="1" style="padding: 0; margin: 6px 0 0 0;" <?php if($credit->homepage){ echo "checked"; } ?> />
                    </div>
                    <div class="<?php fs_row_color(); ?>">
                        <?php
							if($config['settings']['creditpage'] == 0){
						?>
                            <div style="position: absolute; margin: 0 0 0 300px; vertical-align: middle">
                        ...
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                        	...
                        	<span><?php echo $mgrlang['credits_f_advertise_d']; ?></span>
                        </p>
                        <input type="checkbox" name="featured" value="1" style="padding: 0; margin: 6px 0 0 0;" <?php if($credit->featured){ echo "checked"; } ?> />
                    </div>
                </div>
            ...
            <div id="save_bar">
                <input type="button" value="<?php echo $mgrlang['gen_b_cancel']; ?>" onclick="cancel_edit('mgr.credits.php');" /><input type="submit" value="<?php echo $mgrlang['gen_b_save']; ?>" />
            </div>
            </form>
