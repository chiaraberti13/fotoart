<?php
	...
	###################################################################
		...
		switch($_REQUEST['action']){
			case "save_groups":				
				save_groups($page,'digital_sizes','ds_id');	
			break;			
			case "save_sort":
				save_sort('digital_sizes','ds_id');
			break;
			case "clear_sort":
				clear_sort('digital_sizes');
			break;
			# DELETE
			case "del":
				if(!empty($_REQUEST['items'])){
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT name,ds_id FROM {$dbinfo[pre]}digital_sizes WHERE ds_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result)){
						$log_titles.= "$log->name ($log->ds_id), ";
					...
					mysqli_query($db,"DELETE FROM {$dbinfo[pre]}media_digital_sizes WHERE ds_id IN ($delete_array)");
					
					// [todo] need to delete media attached to digital sizes
					...
					mysqli_query($db,"DELETE FROM {$dbinfo[pre]}item_galleries WHERE item_id IN ($delete_array) AND mgrarea = 'digital_sp'");
					
					# UPDATE DIGITAL SIZES TO BE DELETED
					$sql = "UPDATE {$dbinfo[pre]}digital_sizes SET deleted = '1' WHERE ds_id IN ($delete_array)";
					$result = mysqli_query($db,$sql);
					
					# UPDATE ACTIVITY LOG
					save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_digital_sp'],1,$mgrlang['gen_b_del'] . " > <strong>$log_titles</strong>");
		...
		# IF THIS IS AN ENTRY PAGE OR dspgroups IS BLANK RESET THE dspgroups SESSION	
		if($_GET['ep'] or empty($_SESSION['dspgroups'])){
			$_SESSION['dspgroups'] = array('all');
		}			
		# SEE IF ANY GROUPS HAVE BEEN PASSED
		if($_REQUEST['setgroups']){
			if(is_array($_REQUEST['setgroups'])){
				$_SESSION['dspgroups'] = $_REQUEST['setgroups'];
			} else {				
				$_SESSION['dspgroups'] = array($_REQUEST['setgroups']);
			}
		}	
		
		# GET THE TOTAL NUMBER OF ROWS
		if(in_array("all",$_SESSION['dspgroups']))
		{
			$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(ds_id) FROM {$dbinfo[pre]}digital_sizes WHERE deleted='0'"));
		}
		else
		{
			$dsp_result2 = "SELECT COUNT(ds_id) FROM {$dbinfo[pre]}digital_sizes LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}digital_sizes.ds_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['dspgroups']).") AND {$dbinfo[pre]}digital_sizes.deleted='0'";
			$r_rows = mysqli_result_patch(mysqli_query($db,$dsp_result2));
		}
		...
		# SELECT ITEMS
		if(in_array("all",$_SESSION['dspgroups'])){
			$digital_sp_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}digital_sizes WHERE deleted = '0' ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
		} else {
			$digital_sp_result = mysqli_query($db,"SELECT everyone,sortorder,ds_id,active,deleted,name FROM {$dbinfo[pre]}digital_sizes LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}digital_sizes.ds_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['dspgroups']).") AND {$dbinfo[pre]}digital_sizes.deleted = '0' GROUP BY {$dbinfo[pre]}digital_sizes.ds_id ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
		}
...
<head>
	...
	<script language="javascript">
		...
		function clear_sorts(){
			location.href='<?php echo $_SERVER['PHP_SELF']; ?>?ep=1&action=clear_sort&listby=ds_id&listtype=asc';
		}
		...
		Event.observe(window, 'load', function()
			...
			if($('abutton_add_new')!=null)
			{
				$('abutton_add_new').observe('click', function()
					{
						window.location.href='mgr.digital.sp.edit.php?edit=new';
					});
		...
		function load_group_selector()
		{
			...
			var loadpage = "mgr.groups.actions.php?mode=grouplist&mgrarea=<?php echo $page; ?>&ingroups=<?php if(in_array('all',$_SESSION['dspgroups'])){ echo 0; } else { echo 1; } ?>&exitpage=<?php echo $_SERVER['PHP_SELF']; ?>&sessname=dspgroups";
...
<body>
	...
	<div id="container">
        ...
        ?>
            ...
            <div id="actions_bar">	
            	<div class="sec_bar">
                    <img src="./images/mgr.badge.profiles.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_digital_sp']; ?></span> &nbsp; 
                </div>                
                ...
                <div style="padding-top: 9px; margin-right: 20px;">
                    ...
                    <select align="absmiddle" id="actionsdd" onchange="do_actions();">
                        ...
                        <?php if($dsp_group_rows and $digital_sp_rows and in_array("pro",$installed_addons)){ ?><option value="assign_groups">&nbsp; <?php echo $mgrlang['gen_au_itg']; ?></option><?php } ?>
                        <?php if($digital_sp_rows >= 2){ ?><option value="sort_items">&nbsp; <?php echo $mgrlang['gen_sort']; ?></option><?php } ?>
                    </select>
                   	<?php
						}
					?>
			...
			<form name="grouplist" id="grouplist" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
				...
				<div style="<?php if(in_array('all',$_SESSION['dspgroups'])){ echo "display: none;"; } else { echo "display: block;"; } ?>" class="options_area" id="group_selector"></div>
			</form>
            <!-- START CONTENT -->
            <?php
                ...
                <div id="content" style="border-top: 0px;">						
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
							...
							<?php $header_name = "ds_id";	if($listby == $header_name){ $sel = "_sel"; } else { $sel = ""; }	$hlink = ""; $hlink = $_SERVER['PHP_SELF'] . "?listby=" . $header_name . "&listtype="; if($listtype == "asc" && $listby == $header_name){ $hlink .= "desc"; } else { $hlink .= "asc"; } ?><td class="cth<?php echo $sel; ?>" onclick="window.location.href='<?php echo $hlink; ?>'" align="center" width="100">	<div class="content_table_header<?php echo $sel; ?>"><div><a href="<?php echo $hlink ?>"><?php echo $mgrlang['gen_t_id']; ?></a></div></div></td>
                        ...
                        <?php								
                            ...
                            while($digital_sp = mysqli_fetch_object($digital_sp_result)){
                                ...
                                if($digital_sp->{"name_" . $config['settings']['lang_file_mgr']}){
                                    $name = $digital_sp->{"name_" . $config['settings']['lang_file_mgr']};
                                } else {
                                    $name = $digital_sp->name;
                                }
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onMouseOver="cellover(this,'#<?php echo $color_fade; ?>',32);" onMouseOut="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"></a><?php if($digital_sp->sortorder) { echo $digital_sp->sortorder;} else { echo "--"; } ?></td>
                                <td align="center"><a name="row_<?php echo $digital_sp->ds_id; ?>"></a><?php echo $digital_sp->ds_id; ?></td>
                                <td onclick="window.location.href='mgr.digital.sp.edit.php?edit=<?php echo $digital_sp->ds_id; ?>'"><a href="mgr.digital.sp.edit.php?edit=<?php echo $digital_sp->ds_id; ?>" class="editlink"><?php echo $name; ?></a>&nbsp;</td>
                                <td align="center" valign="middle" nowrap>
                                    <a href="mgr.digital.sp.edit.php?edit=<?php echo $digital_sp->ds_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a>
                                    <a href="javascript:deleterec(<?php echo $digital_sp->ds_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a> 
                                </td>
                                <td align="center" valign="middle" nowrap>
                                    <input type="checkbox" name="items[]" value="<?php echo $digital_sp->ds_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
                            }
