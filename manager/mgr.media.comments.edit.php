<?php
	...
	###################################################################
		...
		if($_GET['edit'] != "new" and !$_REQUEST['action']){
			$mc_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}media_comments WHERE mc_id = '$_GET[edit]'");
			$mc_rows = mysqli_num_rows($mc_result);
			$mc = mysqli_fetch_object($mc_result);
			
			# SEE IF THIS WAS POSTED BY A MEMBER OR A VISITOR
			if($mc->member_id)
			{
				# FIND MEMBER DETAILS
				$member_result = mysqli_query($db,"SELECT f_name,l_name,email,mem_id FROM {$dbinfo[pre]}members WHERE mem_id = '$mc->member_id'");
		...
		switch($_REQUEST['action']){
			...
			case "save_edit":							
				...
				# UPDATE THE DATABASE
				$sql = "UPDATE {$dbinfo[pre]}media_comments SET 
							comment='$comment',
							posted='$save_date',
							status='$status'
							where mc_id  = '$saveid'";
				...
				# UPDATE ACTIVITY LOG
				save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_media_comments'],1,$mgrlang['gen_b_ed'] . " > <strong>$mgrlang[media_comments_f_com] ($saveid)</strong>");
				...
				header("location: mgr.media.comments.php?mes=edit"); exit;
			break;
		}		
?>
		...
		function form_sumbit(){
			...
			<?php
				if($_SESSION['admin_user']['admin_id'] == "DEMO"){
					...
					$action_link = ($_GET['edit'] == "new") ? "mgr.media.comments.edit.php?action=save_new" : "mgr.media.comments.edit.php?action=save_edit";
...
<body>
	...
	<div id="container">
        ...
        ?>
            ...
            <!-- TITLE BAR AREA -->
            <div id="title_bar">
                <img src="./images/mgr.badge.media.comments.png" class="badge" />
                <p><strong><?php echo $mgrlang['media_comments_edit_header']; ?></strong><br /><span><?php echo $mgrlang['media_comments_edit_message']; ?></span></p>
                ...
                <div id="tab1_group" class="group" style="display: block;">
                    <?php
						if($mc->media_id)
						{
							try
							{
								$media = new mediaTools($mc->media_id);
					...
					<div class="<?php fs_row_color(); ?>">
                        ...
                        <span class="input_label_subtext"><?php echo $mgrlang['media_comments_f_med_d']; ?></span></p>
                        <p style="width: 300px;">
							<?php
								if($mediaStatus == 1)
								{
                    ...
                    <div class="<?php fs_row_color(); ?>" id="comment_div">
                        ...
                        <p class="input_label" onclick="support_popup('<?php echo $supportPageID; ?>');"><?php echo $mgrlang['media_comments_f_com']; ?>:<br />
                        <span class="input_label_subtext"><?php echo $mgrlang['media_comments_f_com_d']; ?></span></p>
                        <textarea name="comment" id="comment" style="width: 400px; height: 150px; vertical-align: middle"><?php echo @stripslashes($mc->comment); ?></textarea><?php if(in_array('multilang',$installed_addons)){ ?>&nbsp;<span class="mtag_dblue" style="color: #FFF;"><?php echo ucfirst(strtolower($mc->language)); ?></span><?php } ?>
                    </div>
                    <div class="<?php fs_row_color(); ?>">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p class="input_label" onclick="support_popup('<?php echo $supportPageID; ?>');"><?php echo $mgrlang['media_comments_f_mem']; ?>:<br />
                        <span class="input_label_subtext"><?php echo $mgrlang['media_comments_f_mem_d']; ?></span></p>
                    ...
                    <div class="<?php fs_row_color(); ?>">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p class="input_label" onclick="support_popup('<?php echo $supportPageID; ?>');"><?php echo $mgrlang['media_comments_f_postd']; ?>:<br />
                        <span class="input_label_subtext"><?php echo $mgrlang['media_comments_f_postd_d']; ?></span></p>
                        <?php 
							$form_posted = $ndate->date_to_form($mc->posted);
						?>
                    ...
                    <div class="<?php fs_row_color(); ?>">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p class="input_label" onclick="support_popup('<?php echo $supportPageID; ?>');"><?php echo $mgrlang['media_comments_f_status']; ?>:<br />
                        <span class="input_label_subtext"><?php echo $mgrlang['media_comments_f_status_d']; ?></span></p>
                        <select name="status">
                        	<option value="0" <?php if($mc->status == 0){ echo "selected"; } ?>><?php echo $mgrlang['gen_pending']; ?></option>
                            <option value="1" <?php if($mc->status == 1){ echo "selected"; } ?> ><?php echo $mgrlang['gen_b_approved']; ?></option>
                        </select>
                    </div>
                </div>
            </div>
            <div id="save_bar">
                <input type="button" value="<?php echo $mgrlang['gen_b_cancel']; ?>" onclick="cancel_edit('mgr.media.comments.php');" /><input type="submit" value="<?php echo $mgrlang['gen_b_save']; ?>" />
            </div>
            </form>
