	...
	###################################################################
		...
		{			
			case "save_approved_status":
				save_approved_status($page,'media_ratings','mr_id','status');
			...
			case "del":
				if(!empty($_REQUEST['items']))
				{
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT mr_id,rating FROM {$dbinfo[pre]}media_ratings WHERE mr_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result))
					{
						$rating_short = substr($log->rating,0,24)."...";
						$log_titles.= "$rating_short ($log->mr_id), ";
					}
					...
					# DELETE
					@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}media_ratings WHERE mr_id IN ($delete_array)");
					
					# FIND OUT HOW MANY MORE ARE PENDING
					$_SESSION['pending_media_ratings'] = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(mr_id) FROM {$dbinfo[pre]}media_ratings WHERE status = '0'"));
					
					# UPDATE ACTIVITY LOG
					save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_media_ratings'],1,$mgrlang['gen_b_del'] . " > <strong>$log_titles</strong>");
	...
	<script language="javascript">
		...
		function switch_status_rating(item_id,newstatus){
			...
			else
			{
				...
				hide_sp();
				$('ratingcheck' + item_id).innerHTML = "<img src=\"images/mgr.loader.gif\">";
				var updatecontent = 'ratingcheck' + item_id;
				var loadpage = "mgr.media.ratings.actions.php?mode=ap&id=" + item_id + "&newstatus=" + newstatus;
				var pars = "";
		...
		function write_status(mode,id,curstatus)
		{
			...
			switch(mode)
			{
				case "rating":
					...
					if(curstatus != 0){ content+= "<div class='mtag_pending mtag' onclick=\"switch_status_rating('"+id+"',0);\" onmouseover='clear_sp_timeout();' onmouseout='hide_sp();'><?php echo $mgrlang['gen_pending']; ?></div>"; }
					if(curstatus != 1){ content+= "<div class='mtag_approved mtag' onclick=\"switch_status_rating('"+id+"',1);\" onmouseover='clear_sp_timeout();' onmouseout='hide_sp();'><?php echo $mgrlang['gen_b_approved']; ?></div>"; }
				break;
			}
...
<body>
	...
	<div id="container">
        ...
        <?php
			...
			$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(mr_id) FROM {$dbinfo[pre]}media_ratings"));
			...
			$media_ratings_result = mysqli_query($db,"SELECT mr_id,rating,member_id,media_id,status,posted FROM {$dbinfo[pre]}media_ratings ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
            $media_ratings_rows = mysqli_num_rows($media_ratings_result);
        ?>
            <!-- ACTIONS BAR AREA -->
            <div id="actions_bar">	
            	<div class="sec_bar">
                    <img src="./images/mgr.badge.media.ratings.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_media_ratings']; ?></span> &nbsp; 
                </div>                
                <div style="float: left;"><img src="./images/mgr.actions.bar.div.png" class="action_bar_divider" /></div>
            						
                <div style="float: left; padding-left: 3px;">
                    <?php if(!empty($media_ratings_rows)){ ?>
                ...
                <div style="padding-top: 9px; margin-right: 20px;">
                    <?php
						if($media_ratings_rows)
						{
					?>
                    <select align="absmiddle" id="actionsdd" onchange="do_actions();">
                       	<option value="#"><?php echo $mgrlang['gen_actions']; ?>:</option>
                        <?php if($media_ratings_rows){ ?><option value="set_approved">&nbsp; <?php echo $mgrlang['gen_toapproved']; ?></option><?php } ?>
                    </select>
                   	<?php
						}
					?>
                ...
                <div id="content">
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
                            ...
                            <?php $header_name = "rating";if($listby == $header_name){ $sel = "_sel"; } else { $sel = ""; }	$hlink = ""; $hlink = $_SERVER['PHP_SELF'] . "?listby=" . $header_name . "&listtype="; if($listtype == "asc" && $listby == $header_name){ $hlink .= "desc"; } else { $hlink .= "asc"; } ?><td class="cth<?php echo $sel; ?>" onclick="window.location.href='<?php echo $hlink; ?>'" width="100%"><div class="content_table_header<?php echo $sel; ?>"><div><a href="<?php echo $hlink ?>"><?php echo $mgrlang['media_ratings_t_rating']; ?></a></div></div></td>
                        ...
                        <?php								
                            ...
                            while($media_ratings = mysqli_fetch_object($media_ratings_result))
							{
                            
                                if($media_ratings->media_id)
								{
									try
									{
										$media = new mediaTools($media_ratings->media_id);
								...
								if($media_ratings->member_id)
								{
									# FIND MEMBER DETAILS
									$member_result = mysqli_query($db,"SELECT f_name,l_name,email,mem_id FROM {$dbinfo[pre]}members WHERE mem_id = '$media_ratings->member_id'");
									...
									if($member_rows > 0)
									{
										...
										$mem_id = $media_ratings->member_id;
									}
									else
									{
								...
								//$on_stars = $media_ratings->rating * ($config['RatingStars']/5);
								
								if($config['RatingStars'] == 5)
								{
									$on_stars = $media_ratings->rating/2;
								}
								else
								{
									$on_stars = $media_ratings->rating;
								}
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onMouseOver="cellover(this,'#<?php echo $color_fade; ?>',32);" onMouseOut="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"><a name="row_<?php echo $media_ratings->mr_id; ?>"></a><?php echo $media_ratings->mr_id; ?></td>
                                <td align="center" valign="middle" onclick="window.location.href='mgr.media.ratings.edit.php?edit=<?php echo $media_ratings->mr_id; ?>'">
									<?php
										if($mediaStatus == 1)
										{
									?>
                                ...
                                <td align="left" nowrap="nowrap" valign="middle">
                                    ...
                                    <?php
                                        ...
                                        if($member_rows > 0 and $media_ratings->member_id){
                                    ?>
                                            <a href="<?php if(in_array("members",$_SESSION['admin_user']['permissions'])){ echo "mgr.members.edit.php?edit=$mgrMemberInfo->mem_id"; } else { echo "#"; } ?>" class="editlink" style="margin-right: 10px;" onmouseover="start_mem_panel(<?php echo $media_ratings->mr_id; ?>,<?php echo $mgrMemberInfo->mem_id; ?>);" onmouseout="cancel_mem_panel(<?php echo $media_ratings->mr_id; ?>,<?php echo $mgrMemberInfo->mem_id; ?>);"><?php echo $member_name; ?></a>
                                            </div>
                                            <div style="float: left;"><!-- USED TO GET CORRECT ALIGNMENT - WINDOW AFTER NAME -->
                                                <div id="more_info_<?php echo $mgrMemberInfo->mem_id; ?>-<?php echo $media_ratings->mr_id; ?>" style="display: none;" class="mem_details_win">
                                                    <div class="mem_details_win_inner">
                                                        <img src="images/mgr.detailswin.arrow.gif" style="position: absolute; margin: 16px 0 0 -9px;" />
                                                        <div id="more_info_<?php echo $mgrMemberInfo->mem_id; ?>-<?php echo $media_ratings->mr_id; ?>_content" style="overflow: auto; border: 1px solid #fff"><img src="images/mgr.loader.gif" style="margin: 40px;" /></div>
                                                    </div>
                                                </div>
                                            </div>
                                    <?php
                                ...
                                <td style="padding: 10px 10px 10px 13px;">
									<!--<?php echo $media_ratings->rating; ?>-->
                                    <div id="star_div_<?php echo $media_ratings->mr_id; ?>" class="rating_stars_div">
									<?php
										for($x=1;$x<=$config['RatingStars'];$x++)
										{
											...
											*/
											if($x <= $on_stars){ $star_status = "1"; } else { $star_status = "0"; }
											echo "<img src='images/mgr.icon.star.$star_status.png' class='rating_star' onclick='update_rating_stars($x,$media_ratings->mr_id)' onmouseover='rollover_stars($x,$media_ratings->mr_id)' onmouseout='rollout_stars_delay($media_ratings->mr_id)' initialvalue='$star_status' starnumber='$x' />";	
										}
									?>
                                    </div>
                                </td>
                                <td align="center" nowrap="nowrap"><?php echo $mcdate->showdate($media_ratings->posted); ?></td>
                                <td align="center">
                                    <div style='width: 110px;'><!-- FORCE THE COLUMN WIDTH --></div>
                                    <div class='status_popup' id='rating_sp_<?php echo $media_ratings->mr_id; ?>' style="z-index: <?php echo $zindex-1; ?>; display: none;" onmouseout="hide_sp();" onmouseover="clear_sp_timeout();"></div>
                                    <div id="ratingcheck<?php echo $media_ratings->mr_id; ?>" style="position: absolute; z-index: <?php echo $zindex; ?>; margin-left: 17px; margin-top: -10px">
										<?php
                                            switch($media_ratings->status)
                                            {
                                                case 0: // PENDING
                                        ...
                                        ?>
                                   	  <div class='<?php echo $mtag; ?> mtag' onmouseover="show_sp('rating_sp_<?php echo $media_ratings->mr_id; ?>');write_status('rating','<?php echo $media_ratings->mr_id; ?>',<?php echo $media_ratings->status; ?>)"><?php echo $tag_label; ?></div>
                                    </div>
                                </td>
                                <td align="center" valign="middle" nowrap>
                                    <a href="mgr.media.ratings.edit.php?edit=<?php echo $media_ratings->mr_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a>
                                    <a href="javascript:deleterec(<?php echo $media_ratings->mr_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a>
                                </td>
                                <td align="center" valign="middle" nowrap>
                                    <input type="checkbox" name="items[]" value="<?php echo $media_ratings->mr_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
								$zindex-=2;
