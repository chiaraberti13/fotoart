	...
	###################################################################
		...
		{			
			case "save_approved_status":
				save_approved_status($page,'keywords','key_id','status');
				# NUMBER OF MEDIA TAGS PENDING
			...
			case "del":
				if(!empty($_REQUEST['items']))
				{
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT key_id,keyword FROM {$dbinfo[pre]}keywords WHERE key_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result))
					{
						$comment_short = substr($log->keyword,0,24)."...";
						$log_titles.= "$comment_short ($log->key_id), ";
					}
					...
					# DELETE
					@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}keywords WHERE key_id IN ($delete_array)");
	...
	<script language="javascript">
		...
		function switch_status_tag(item_id,newstatus){
			...
			else
			{
				...
				var loadpage = "mgr.media.tags.actions.php?mode=ap&id=" + item_id + "&newstatus=" + newstatus;
				var pars = "";
		...
		function write_status(mode,id,curstatus)
		{
			...
			switch(mode)
			{
				case "tag":
					div_id = "tag_sp_"+id;
					if(curstatus != 0){ content+= "<div class='mtag_pending mtag' onclick=\"switch_status_tag('"+id+"',0);\" onmouseover='clear_sp_timeout();' onmouseout='hide_sp();'><?php echo $mgrlang['gen_pending']; ?></div>"; }
					if(curstatus != 1){ content+= "<div class='mtag_approved mtag' onclick=\"switch_status_tag('"+id+"',1);\" onmouseover='clear_sp_timeout();' onmouseout='hide_sp();'><?php echo $mgrlang['gen_b_approved']; ?></div>"; }
				break;
			}
...
<body>
	...
	<div id="container">
        ...
        <?php
			...
			$id_field_name = "key_id";			
			require_once('mgr.datasort.logic.php');				
            
			$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(key_id) FROM {$dbinfo[pre]}keywords WHERE memtag = 1"));
			...
			//echo "SELECT * FROM {$dbinfo[pre]}media_tags ORDER BY $listby $listtype LIMIT $startrecord,$perpage"; exit;
			$media_tags_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}keywords WHERE memtag = 1 ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
        ...
        ?>
            ...
            <div id="actions_bar">	
            	<div class="sec_bar">
                    <img src="./images/mgr.badge.media.tags.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_media_tags']; ?></span> &nbsp; 
                </div>                
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <?php								
                            ...
                            while($media_tags = mysqli_fetch_object($media_tags_result))
							{
                                ...
                                if($media_tags->media_id)
								{
									try
									{
										$media = new mediaTools($media_tags->media_id);
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onMouseOver="cellover(this,'#<?php echo $color_fade; ?>',32);" onMouseOut="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"><a name="row_<?php echo $media_tags->key_id; ?>"></a><?php echo $media_tags->key_id; ?></td>
                                <td align="center" valign="middle" onclick="window.location.href='mgr.media.tags.edit.php?edit=<?php echo $media_tags->key_id; ?>'">
									<?php
										if($mediaStatus == 1)
										{
									?>
                                ...
                                <td align="left" nowrap="nowrap">
                                    ...
                                    ?>
                                            <a href="<?php if(in_array("members",$_SESSION['admin_user']['permissions'])){ echo "mgr.members.edit.php?edit=$mgrMemberInfo->mem_id"; } else { echo "#"; } ?>" class="editlink" style="margin-right: 10px;" onmouseover="start_mem_panel(<?php echo $media_tags->key_id; ?>,<?php echo $mgrMemberInfo->mem_id; ?>);" onmouseout="cancel_mem_panel(<?php echo $media_tags->key_id; ?>,<?php echo $mgrMemberInfo->mem_id; ?>);"><?php echo $member_name; ?></a>
                                            </div>
                                            <div style="float: left;"><!-- USED TO GET CORRECT ALIGNMENT - WINDOW AFTER NAME -->
                                                <div id="more_info_<?php echo $mgrMemberInfo->mem_id; ?>-<?php echo $media_tags->key_id; ?>" style="display: none;" class="mem_details_win">
                                                    <div class="mem_details_win_inner">
                                                        <img src="images/mgr.detailswin.arrow.gif" style="position: absolute; margin: 16px 0 0 -9px;" />
                                                        <div id="more_info_<?php echo $mgrMemberInfo->mem_id; ?>-<?php echo $media_tags->key_id; ?>_content" style="overflow: auto; border: 1px solid #fff"><img src="images/mgr.loader.gif" style="margin: 40px;" /></div>
                                                    </div>
                                                </div>
                                            </div>
                                    <?php
                                ...
                                <td onclick="window.location.href='mgr.media.tags.edit.php?edit=<?php echo $media_tags->key_id; ?>'" style="padding: 10px 10px 10px 13px;"><strong><?php echo $media_tags->keyword; ?><?php if(in_array('multilang',$installed_addons)){ ?></strong>&nbsp;&nbsp;<span class="mtag_dblue" style="color: #FFF;"><?php echo ucfirst(strtolower($media_tags->language)); ?></span><?php } ?></td>
                                ...
                                <td align="center">
                                    ...
                                    <div class='status_popup' id='tag_sp_<?php echo $media_tags->key_id; ?>' style="z-index: <?php echo $zindex-1; ?>; display: none;" onmouseout="hide_sp();" onmouseover="clear_sp_timeout();"></div>
                                    <div id="tagcheck<?php echo $media_tags->key_id; ?>" style="position: absolute; z-index: <?php echo $zindex; ?>; margin-left: 17px; margin-top: -10px">
										<?php
                                            switch($media_tags->status)
                                            {
                                                case 0: // PENDING
                                        ...
                                        ?>
                                   	  <div class='<?php echo $mtag; ?> mtag' onmouseover="show_sp('tag_sp_<?php echo $media_tags->key_id; ?>');write_status('tag','<?php echo $media_tags->key_id; ?>',<?php echo $media_tags->status; ?>)"><?php echo $tag_label; ?></div>
                                    </div>
                                </td>
                                <td align="center" valign="middle" nowrap>
                                    <a href="mgr.media.tags.edit.php?edit=<?php echo $media_tags->key_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a>
                                    <a href="javascript:deleterec(<?php echo $media_tags->key_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a>
                                </td>
                                <td align="center" valign="middle" nowrap>
                                    <input type="checkbox" name="items[]" value="<?php echo $media_tags->key_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
								$zindex-=2;
