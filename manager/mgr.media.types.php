	...
	###################################################################
		...
		{			
			case "save_groups":				
				...
				save_groups($page,'media_types','mt_id');				
			break;
			case "save_status":
				save_status($page,'media_types','mt_id');
			break;
			# DELETE
			case "del":
				if(!empty($_REQUEST['items']))
				{
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT name,mt_id FROM {$dbinfo[pre]}media_types WHERE mt_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result))
					{
						$log_titles.= "$log->name ($log->mt_id), ";
					}
					...
					mysqli_query($db,"DELETE FROM {$dbinfo[pre]}media_types_ref WHERE mt_id IN ($delete_array)");
					
					# DELETE GROUPS
					@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}groupids WHERE mgrarea = '$page' AND item_id IN ($delete_array)");
					
					# DELETE
					@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}media_types WHERE mt_id IN ($delete_array)");
					
					# UPDATE ACTIVITY LOG
					save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_media_types'],1,$mgrlang['gen_b_del'] . " > <strong>$log_titles</strong>");
		...
		# IF THIS IS AN ENTRY PAGE OR mtgroups IS BLANK RESET THE mtgroups SESSION	
		if($_GET['ep'] or empty($_SESSION['mtgroups']))
		{
			$_SESSION['mtgroups'] = array('all');
		}			
		...
		if($_REQUEST['setgroups'])
		{
			...
			{
				$_SESSION['mtgroups'] = $_REQUEST['setgroups'];
			}
			else
			{				
				$_SESSION['mtgroups'] = array($_REQUEST['setgroups']);
			}
		}
		
		# GET THE TOTAL NUMBER OF ROWS
		if(in_array("all",$_SESSION['mtgroups']))
		{
			$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(mt_id) FROM {$dbinfo[pre]}media_types"));
		}
		else
		{
			$mt_result2 = "SELECT COUNT(mt_id) FROM {$dbinfo[pre]}media_types LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}media_types.mt_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['mtgroups']).")";
			$r_rows = mysqli_result_patch(mysqli_query($db,$mt_result2));
		}
		...
		if(in_array("all",$_SESSION['mtgroups']))
		{
			$media_type_result = mysqli_query($db,"SELECT mt_id,name,active FROM {$dbinfo[pre]}media_types ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
		}
		else
		{				
			$media_type_result = mysqli_query($db,"SELECT mt_id,name,active FROM {$dbinfo[pre]}media_types LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}media_types.mt_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['mtgroups']).") GROUP BY {$dbinfo[pre]}media_types.mt_id ORDER BY $listby $listtype LIMIT $startrecord,$perpage"); 				
		}
		//echo $media_type_result; exit;
		$media_type_rows = mysqli_num_rows($media_type_result);
...
<head>
	...
	<script language="javascript">
		...
		{
			location.href='<?php echo $_SERVER['PHP_SELF']; ?>?ep=1&action=clear_sort&listby=mt_id&listtype=asc';
		}
		...
		Event.observe(window, 'load', function()
			...
			if($('abutton_add_new')!=null)
			{
				$('abutton_add_new').observe('click', function()
					{
						window.location.href='mgr.media.types.edit.php?edit=new';
					});
		...
		function load_group_selector()
		{
			...
			var loadpage = "mgr.groups.actions.php?mode=grouplist&mgrarea=<?php echo $page; ?>&ingroups=<?php if(in_array('all',$_SESSION['mtgroups'])){ echo 0; } else { echo 1; } ?>&exitpage=<?php echo $_SERVER['PHP_SELF']; ?>&sessname=mtgroups";
		...
		function switch_status(item_type,item_id){
			...
			else
			{
				...
				var loadpage = "mgr.media.types.actions.php?mode=" + item_type + "&id=" + item_id;
				var pars = "";
...
<body>
	...
	<div id="container">
        ...
        ?>
            <!-- ACTIONS BAR AREA -->
            <?php
				$media_type_group_result = mysqli_query($db,"SELECT gr_id,flagtype,name FROM {$dbinfo[pre]}groups WHERE mgrarea = '$page' ORDER BY name");
				$media_type_group_rows = mysqli_num_rows($media_type_group_result);
			?>
            <div id="actions_bar">	
            	<div class="sec_bar">
                    <img src="./images/mgr.badge.mediatypes.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_media_types']; ?></span> &nbsp; 
                </div>                
                ...
                <div style="padding-top: 9px; margin-right: 20px;">
                    <?php
						if($media_type_rows and $media_type_group_rows)
						{
					?>
                    <select align="absmiddle" id="actionsdd" onchange="do_actions();">
                        ...
                        <?php if($media_type_group_rows and $media_type_rows and in_array("pro",$installed_addons)){ ?><option value="assign_groups">&nbsp; <?php echo $mgrlang['gen_au_itg']; ?></option><?php } ?>
                    </select>
                   	<?php
						}
					?>
			...
			<form name="grouplist" id="grouplist" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
				...
				<div style="<?php if(in_array('all',$_SESSION['mtgroups'])){ echo "display: none;"; } else { echo "display: block;"; } ?>" class="options_area" id="group_selector"></div>
			</form>
                ...
                <div id="content">
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <?php								
                            ...
                            while($media_type = mysqli_fetch_object($media_type_result))
							{
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onMouseOver="cellover(this,'#<?php echo $color_fade; ?>',32);" onMouseOut="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"><a name="row_<?php echo $media_type->mt_id; ?>"></a><?php echo $media_type->mt_id; ?></td>
                                <td onclick="window.location.href='mgr.media.types.edit.php?edit=<?php echo $media_type->mt_id; ?>'"><a href="mgr.media.types.edit.php?edit=<?php echo $media_type->mt_id; ?>" class="editlink"><?php echo $media_type->name; ?></a></td>
                                <td align="center"><div id="ac<?php echo $media_type->mt_id; ?>"><a href="javascript:switch_status('ac','<?php echo $media_type->mt_id; ?>');"><img src="images/mgr.small.check.<?php echo $media_type->active; ?>.png" border="0" /></a></div></td>
                                <td align="center" valign="middle" nowrap>
                                    <a href="mgr.media.types.edit.php?edit=<?php echo $media_type->mt_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a>
                                    <a href="javascript:deleterec(<?php echo $media_type->mt_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a>
                                </td>
                                <td align="center" valign="middle" nowrap>
                                    <input type="checkbox" name="items[]" value="<?php echo $media_type->mt_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
                            }
