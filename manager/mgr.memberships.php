<?php
	...
	###################################################################
		...
		{			
			case "save_groups":				
				//echo $_GET['selected_items']; exit;
				save_groups($page,'memberships','ms_id');				
			break;			
			case "save_sort":
				//echo $_GET['sort_list_vals']; exit;
				save_sort('memberships','ms_id');
			break;
			case "clear_sort":
				clear_sort('memberships');
			break;
			case "save_status":
				save_status($page,'memberships','ms_id');
			break;
			# DELETE
			case "del":
				if(!empty($_REQUEST['items'])){
					...
					if(in_array('1',$items))
						unset($items[array_search('1',$items)]);
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT name,ms_id FROM {$dbinfo[pre]}memberships WHERE ms_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result)){
						$log_titles.= "$log->name ($log->ms_id), ";
						
						# UPDATE MEMBERS
						@$sql = "UPDATE {$dbinfo[pre]}members SET membership='1' WHERE membership = '$log->ms_id'";
						@$result = mysqli_query($db,$sql);
						
					}
					
					# DELETE MEMBERSHIPS
					@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}memberships WHERE ms_id IN ($delete_array)");
					...
					# UPDATE ACTIVITY LOG
					save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_memberships'],1,$mgrlang['gen_b_del'] . " > <strong>$log_titles</strong>");
		...
		if(@in_array("all",$_SESSION['msgroups']))
		{
			$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(ms_id) FROM {$dbinfo[pre]}memberships WHERE deleted='0'"));
		}
		else
		{
			if($_SESSION['msgroups'])
				$msGroupsFlat = implode(",",$_SESSION['msgroups']);
			else
				$msGroupsFlat = 0;
			$ms_result2 = "SELECT COUNT(ms_id) FROM {$dbinfo[pre]}memberships LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}memberships.ms_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN ({$msGroupsFlat}) AND {$dbinfo[pre]}memberships.deleted='0'";
			$r_rows = mysqli_result_patch(mysqli_query($db,$ms_result2));
		}
		...
		# IF THIS IS AN ENTRY PAGE OR msgroups IS BLANK RESET THE msgroups SESSION	
		if($_GET['ep'] or empty($_SESSION['msgroups']))
		{
			$_SESSION['msgroups'] = array('all');
		}			
		...
		if($_REQUEST['setgroups'])
		{
			...
			{
				$_SESSION['msgroups'] = $_REQUEST['setgroups'];
			}
			else
			{				
				$_SESSION['msgroups'] = array($_REQUEST['setgroups']);
			}
		}		
		...
		# SELECT ITEMS
		if(in_array("all",$_SESSION['msgroups'])){
			$membership_result = mysqli_query($db,"SELECT ms_id,name,msgroups,flagtype FROM {$dbinfo[pre]}memberships ORDER BY $listby $listtype");
		} else {				
			$membership_result = "SELECT ms_id,name,msgroups,flagtype FROM {$dbinfo[pre]}memberships WHERE";
			$x=1;
			foreach($_SESSION['msgroups'] as $value){
				$membership_result.= " msgroups LIKE '%,$value,%'";
				if($x < count($_SESSION['msgroups'])){
					$membership_result.= " OR";
				}
				$x++;
			} 
			$membership_result.= " ORDER BY $listby $listtype";				
			//FIND_IN_SET
			//$membership_result = "SELECT ms_id,name,msgroups,flagtype FROM {$dbinfo[pre]}memberships WHERE FIND_IN_SET('61',msgroups) ORDER BY $listby $listtype";
			$membership_result = mysqli_query($db,$membership_result);
		}
		*/
		
		if($startrecord < 0)
			$startrecord = 0; // Fix for negative start records
		
		
		# SELECT ITEMS
		if(in_array("all",$_SESSION['msgroups']))
		{
			//echo "SELECT ms_id,name,flagtype,sortorder,active FROM {$dbinfo[pre]}memberships WHERE deleted='0' ORDER BY $listby $listtype LIMIT $startrecord,$perpage"; exit; // testing
			$membership_result = mysqli_query($db,"SELECT ms_id,name,flagtype,sortorder,active FROM {$dbinfo[pre]}memberships WHERE deleted='0' ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
		}
		else
		{				
			$membership_result = mysqli_query($db,"SELECT sortorder,ms_id,flagtype,name,active FROM {$dbinfo[pre]}memberships LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}memberships.ms_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['msgroups']).") AND memberships.deleted='0' GROUP BY {$dbinfo[pre]}memberships.ms_id ORDER BY $listby $listtype LIMIT $startrecord,$perpage"); 				
		}
		//echo $membership_result; exit;
...
<head>
	...
	<script language="javascript">
		...
		{
			location.href='<?php echo $_SERVER['PHP_SELF']; ?>?ep=1&action=clear_sort&listby=ms_id&listtype=asc';
		}
		...
		Event.observe(window, 'load', function()
			...
			if($('abutton_add_new')!=null)
			{
				$('abutton_add_new').observe('click', function()
					{
						window.location.href='mgr.memberships.edit.php?edit=new';
					});
		...
		function load_group_selector()
		{
			...
			var loadpage = "mgr.groups.actions.php?mode=grouplist&mgrarea=<?php echo $page; ?>&ingroups=<?php if(in_array('all',$_SESSION['msgroups'])){ echo 0; } else { echo 1; } ?>&exitpage=<?php echo $_SERVER['PHP_SELF']; ?>&sessname=msgroups";
		...
		function switch_status(item_type,item_id){
			...
			else
			{
				...
				var loadpage = "mgr.memberships.actions.php?mode=" + item_type + "&id=" + item_id;
				var pars = "";
...
<body>
	...
	<div id="container">
        ...
        ?>
            ...
            <div id="actions_bar">	
            	<div class="sec_bar">
                    <img src="./images/mgr.badge.membership.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_memberships']; ?></span> &nbsp; 
                </div>                
                ...
                <div style="padding-top: 9px; margin-right: 20px;">
                    ...
                    <select align="absmiddle" id="actionsdd" onchange="do_actions();">
                        ...
                        <?php if($membership_group_rows and $membership_rows and in_array("pro",$installed_addons)){ ?><option value="assign_groups">&nbsp; <?php echo $mgrlang['gen_au_itg']; ?></option><?php } ?>
                        <option value="set_status">&nbsp; <?php echo $mgrlang['gen_tostatus']; ?></option>
                        <?php if($membership_rows >= 2){ ?><option value="sort_items">&nbsp; <?php echo $mgrlang['gen_sort']; ?></option><?php } ?>
                    </select>
                   	<?php
						}
					?>
			...
			<form name="grouplist" id="grouplist" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
				...
				<div style="<?php if(in_array('all',$_SESSION['msgroups'])){ echo "display: none;"; } else { echo "display: block;"; } ?>" class="options_area" id="group_selector"></div>
			</form>
            ...
            <?php
                ...
                if(!empty($membership_rows))
				{
					if($membership_rows > 10 and $perpage > 10)
					{
                ...
                <div id="content">
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
							...
							<?php $header_name = "ms_id";	if($listby == $header_name){ $sel = "_sel"; } else { $sel = ""; }	$hlink = ""; $hlink = $_SERVER['PHP_SELF'] . "?listby=" . $header_name . "&listtype="; if($listtype == "asc" && $listby == $header_name){ $hlink .= "desc"; } else { $hlink .= "asc"; } ?><td class="cth<?php echo $sel; ?>" onclick="window.location.href='<?php echo $hlink; ?>'" align="center" width="100">	<div class="content_table_header<?php echo $sel; ?>"><div><a href="<?php echo $hlink ?>"><?php echo $mgrlang['gen_t_id']; ?></a></div></div></td>
                        ...
                        <?php								
                            ...
                            while($membership = mysqli_fetch_object($membership_result))
							{
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onMouseOver="cellover(this,'#<?php echo $color_fade; ?>',32);" onMouseOut="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"><?php if($membership->sortorder) { echo $membership->sortorder;} else { echo "--"; } ?></td>
                                <td align="center"><a name="row_<?php echo $membership->ms_id; ?>"></a><?php echo $membership->ms_id; ?></td>
                                <td onclick="window.location.href='mgr.memberships.edit.php?edit=<?php echo $membership->ms_id; ?>'">
                                	
                                	<div style="float: left;"><img src="images/mini_icons/<?php echo $membership->flagtype; ?>" align="absmiddle" style="margin-right: 5px;" /> <a href="mgr.memberships.edit.php?edit=<?php echo $membership->ms_id; ?>" class="editlink" onMouseOver="show_details_win('details_<?php echo $membership->ms_id; ?>','mgr.memberships.actions.php?mode=details&id=<?php echo $membership->ms_id; ?>');" onMouseOut="hide_gdetails('details_<?php echo $membership->ms_id; ?>');"><?php echo $membership->name; ?></a>&nbsp;<?php if($membership->ms_id == '1'){ echo " <span style='color: #888888; font-style:italic'>($mgrlang[membership_default])</span>"; } ?></div>
                                	<div style="float: left; overflow: visible;">
										<div id="details_<?php echo $membership->ms_id; ?>" class="galdet_win" style="display: none;"><?php echo $mgrlang['gen_loading']; ?></div>
									</div>
                                </td>
                                <td align="center"><?php if($membership->ms_id == '1'){ ?><img src="images/mgr.small.check.<?php echo $membership->active; ?>.png" border="0" /><?php } else { ?><div id="ac<?php echo $membership->ms_id; ?>"><a href="javascript:switch_status('ac','<?php echo $membership->ms_id; ?>');"><img src="images/mgr.small.check.<?php echo $membership->active; ?>.png" border="0" /></a></div><?php } ?></td>
                                <td align="center" valign="middle" nowrap>
                                    <a href="mgr.memberships.edit.php?edit=<?php echo $membership->ms_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a>
                                    <?php if($membership->ms_id != '1'){ ?><a href="javascript:deleterec(<?php echo $membership->ms_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a> <?php } ?>
                                </td>
                                <td align="center" valign="middle" nowrap>
                                    <input type="checkbox" name="items[]" value="<?php echo $membership->ms_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
                            }
