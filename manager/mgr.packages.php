<?php
	...
	###################################################################
		...
		switch($_REQUEST['action']){
			case "save_groups":				
				save_groups($page,'packages','pack_id');				
			break;			
			case "save_sort":
				save_sort('packages','pack_id');
			break;
			case "save_status":
				save_status($page,'packages','pack_id');
			break;
			case "clear_sort":
				clear_sort('packages');
			break;
			...
			case "del":
				if(!empty($_REQUEST['items'])){
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT item_name,pack_id FROM {$dbinfo[pre]}packages WHERE pack_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result)){
						$log_titles.= "$log->item_name ($log->pack_id), ";
						
						# MARK AS DELETED
						@$sql = "UPDATE {$dbinfo[pre]}packages SET deleted='1' WHERE pack_id = '$log->pack_id'";
						@$result = mysqli_query($db,$sql);
						
						# REMOVE ITEM GALLERIES
						mysqli_query($db,"DELETE FROM {$dbinfo[pre]}item_galleries WHERE item_id = '$log->pack_id' AND mgrarea = '$page'");
						
						# REMOVE ANY PRODUCT SHOTS
						$ip_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}item_photos WHERE item_id = '$log->pack_id' AND mgrarea = 'pack'");
						...
						while($ip = mysqli_fetch_object($ip_result))
						{
							...
							if(file_exists("../assets/item_photos/pack".$prodzf."_ip".$ipidzf."_org.jpg"))
							{
								unlink("../assets/item_photos/pack".$prodzf."_ip".$ipidzf."_org.jpg");
							}
							if(file_exists("../assets/item_photos/pack".$prodzf."_ip".$ipidzf."_med.jpg"))
							{
								unlink("../assets/item_photos/pack".$prodzf."_ip".$ipidzf."_med.jpg");
							}
							if(file_exists("../assets/item_photos/pack".$prodzf."_ip".$ipidzf."_small.jpg"))
							{
								unlink("../assets/item_photos/pack".$prodzf."_ip".$ipidzf."_small.jpg");
							}
						}
					}
					...
					mysqli_query($db,"DELETE FROM {$dbinfo[pre]}media_packages WHERE pack_id IN ($delete_array)");
					
					# REMOVE PACKAGE ITEMS ENTRIES
					mysqli_query($db,"DELETE FROM {$dbinfo[pre]}package_items WHERE pack_id IN ($delete_array)");
					...
					mysqli_query($db,"DELETE FROM {$dbinfo[pre]}item_galleries WHERE item_id IN ($delete_array) AND mgrarea = 'packages'");
					...
					# UPDATE ACTIVITY LOG
					save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_packages'],1,$mgrlang['gen_b_del'] . " > <strong>$log_titles</strong>");
		...
		# IF THIS IS AN ENTRY PAGE OR packagegroups IS BLANK RESET THE packagegroups SESSION	
		if($_GET['ep'] or empty($_SESSION['packagegroups'])){
			$_SESSION['packagegroups'] = array('all');
		}			
		# SEE IF ANY GROUPS HAVE BEEN PASSED
		if($_REQUEST['setgroups']){
			if(is_array($_REQUEST['setgroups'])){
				$_SESSION['packagegroups'] = $_REQUEST['setgroups'];
			} else {				
				$_SESSION['packagegroups'] = array($_REQUEST['setgroups']);
			}
		}	
		
		# GET THE TOTAL NUMBER OF ROWS
		if(in_array("all",$_SESSION['packagegroups']))
		{
			$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(pack_id) FROM {$dbinfo[pre]}packages WHERE deleted='0'"));
		}
		else
		{
			$package_result2 = "SELECT COUNT(pack_id) FROM {$dbinfo[pre]}packages LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}packages.pack_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['packagegroups']).") AND {$dbinfo[pre]}packages.deleted='0'";
			$r_rows = mysqli_result_patch(mysqli_query($db,$package_result2));
		}
		
		//echo "testing: " . $package_result2; exit;
		...
		# SELECT ITEMS
		if(in_array("all",$_SESSION['packagegroups'])){
			$package_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}packages WHERE deleted = '0' ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
			//echo "tester: " . $listby; exit;
		} else {				
			$package_result = mysqli_query($db,"SELECT pack_id,sortorder,active,item_name FROM {$dbinfo[pre]}packages LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}packages.pack_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['packagegroups']).") AND {$dbinfo[pre]}packages.deleted = '0' GROUP BY {$dbinfo[pre]}packages.pack_id ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
		}
		$package_rows = mysqli_num_rows($package_result);
		//echo "test:" .$package_rows; exit;		
?>
...
<head>
	...
	<script language="javascript">
		...
		function clear_sorts(){
			location.href='<?php echo $_SERVER['PHP_SELF']; ?>?ep=1&action=clear_sort&listby=pack_id&listtype=asc';
		}
		...
		Event.observe(window, 'load', function()
			...
			if($('abutton_add_new')!=null)
			{
				$('abutton_add_new').observe('click', function()
					{
						window.location.href='mgr.package.edit.php?edit=new';
					});
		...
		function load_group_selector()
		{
			...
			var loadpage = "mgr.groups.actions.php?mode=grouplist&mgrarea=<?php echo $page; ?>&ingroups=<?php if(in_array('all',$_SESSION['packagegroups'])){ echo 0; } else { echo 1; } ?>&exitpage=<?php echo $_SERVER['PHP_SELF']; ?>&sessname=packagegroups";
		...
		function switch_status(item_type,item_id){
			...
			else
			{
				...
				var loadpage = "mgr.packages.actions.php?mode=" + item_type + "&id=" + item_id;
				var pars = "";
...
<body>
	...
	<div id="container">
        ...
        ?>
            <!-- ACTIONS BAR AREA -->
            <?php
				$package_group_result = mysqli_query($db,"SELECT gr_id,flagtype,name FROM {$dbinfo[pre]}groups WHERE mgrarea = '$page' ORDER BY name");
				$package_group_rows = mysqli_num_rows($package_group_result);
			?>
            <div id="actions_bar">
            	<div class="sec_bar">
                    <img src="./images/mgr.badge.packages.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_packages']; ?></span> &nbsp; 
                </div>							
                ...
                <div style="padding-top: 9px; margin-right: 20px;">
                    ...
                    <select align="absmiddle" id="actionsdd" onchange="do_actions();">
                        ...
                        <?php if($package_group_rows and $r_rows and in_array("pro",$installed_addons)){ ?><option value="assign_groups">&nbsp; <?php echo $mgrlang['gen_au_itg']; ?></option><?php } ?>
			...
			<form name="grouplist" id="grouplist" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
				...
				<div style="<?php if(in_array('all',$_SESSION['packagegroups'])){ echo "display: none;"; } else { echo "display: block;"; } ?>" class="options_area" id="group_selector"></div>
			</form>            
            <!-- START CONTENT -->
            <?php
                ...
                <div id="content" style="border-top: 0px;">						
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
							...
							<?php $header_name = "pack_id";	if($listby == $header_name){ $sel = "_sel"; } else { $sel = ""; }	$hlink = ""; $hlink = $_SERVER['PHP_SELF'] . "?listby=" . $header_name . "&listtype="; if($listtype == "asc" && $listby == $header_name){ $hlink .= "desc"; } else { $hlink .= "asc"; } ?><td class="cth<?php echo $sel; ?>" onclick="window.location.href='<?php echo $hlink; ?>'" align="center" width="100">	<div class="content_table_header<?php echo $sel; ?>"><div><a href="<?php echo $hlink ?>"><?php echo $mgrlang['gen_t_id']; ?></a></div></div></td>
                        ...
                        <?php								
                            ...
                            while($package = mysqli_fetch_object($package_result)){
                                ...
                                if($package->{"item_name_" . $config['settings']['lang_file_mgr']}){
                                    $name = $package->{"item_name_" . $config['settings']['lang_file_mgr']};
                                } else {
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onMouseOver="cellover(this,'#<?php echo $color_fade; ?>',32);" onMouseOut="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"></a><?php if($package->sortorder) { echo $package->sortorder;} else { echo "--"; } ?></td>
                                <td align="center"><a name="row_<?php echo $package->pack_id; ?>"></a><?php echo $package->pack_id; ?></td>
                                <td onclick="window.location.href='mgr.package.edit.php?edit=<?php echo $package->pack_id; ?>'">
                                	<div style="float: left;"><a href="mgr.package.edit.php?edit=<?php echo $package->pack_id; ?>" class="editlink" onMouseOver="show_details_win('details_<?php echo $package->pack_id; ?>','mgr.packages.actions.php?mode=details&id=<?php echo $package->pack_id; ?>');" onMouseOut="hide_gdetails('details_<?php echo $package->pack_id; ?>');"><?php echo $package->item_name; ?></a></div>
                                	<div style="float: left; overflow: visible;">
										<div id="details_<?php echo $package->pack_id; ?>" class="galdet_win" style="display: none;">Loading Details</div>
									</div>
                                </td>
                                <td align="center"><div id="ac<?php echo $package->pack_id; ?>"><a href="javascript:switch_status('ac','<?php echo $package->pack_id; ?>');"><img src="images/mgr.small.check.<?php echo $package->active; ?>.png" border="0" /></a></div></td>
                                <td align="center" valign="middle" nowrap>
                                    <a href="mgr.package.edit.php?edit=<?php echo $package->pack_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a>
                                    <a href="javascript:deleterec(<?php echo $package->pack_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a> 
                                <td align="center" valign="middle" nowrap>
                                    <input type="checkbox" name="items[]" value="<?php echo $package->pack_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
                            }
