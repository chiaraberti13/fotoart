		...
		switch($_REQUEST['action']){
			...
			case "save_edit":
				...
				if($_FILES['uploadedfile']['name'] != ""){
					...
					$sql = "INSERT INTO {$dbinfo[pre]}content_files (content_id,file_name,type) VALUES ('$saveid','$fname','page')";
				...
				if(in_array('multilang',$installed_addons) and count($active_langs) > 0)
				{
					...
					$addsql = substr($addsql,0,strlen($addsql)-1); // Strip the trailing comma
					$addsql = ",".$addsql;
				}
				
				//echo $addsql."<br><br>";
				//print_r($active_langs);
				...
				# UPDATE ACTIVITY LOG
				save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_page_content'],1,$mgrlang['gen_b_ed'] . " > <strong>$name</strong>");
				
				header("location: mgr.page.content.php?mes=edit"); exit;
			break;
			# SAVE NEW ITEM
			case "save_new":
				# CONVERT POST & GET ARRAYS TO LOCAL VALUES AND CLEAN DATA				
				...
				# UPDATE ACTIVITY LOG
				save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_page_content'],1,$mgrlang['gen_b_new'] . " > <strong>$name</strong>");
				
				header("location: mgr.page.content.php?mes=new"); exit;
			break;
		}		
?>
		...
		function form_sumbit(){
			...
			<?php
				if($_SESSION['admin_user']['admin_id'] == "DEMO"){
					...
					$action_link = ($_GET['edit'] == "new") ? "mgr.page.content.edit.php?action=save_new" : "mgr.page.content.edit.php?action=save_edit";
...
<body>
	...
	<div id="container">
        ...
        ?>
            ...
            <!-- TITLE BAR AREA -->
            <div id="title_bar">
                <img src="./images/mgr.badge.content.png" class="badge" />
                <p><strong><?php echo ($ptitle = ($_GET['edit'] == "new") ? $mgrlang['pc_new_header'] : $mgrlang['pc_edit_header']); ?></strong><br /><span><?php echo ($pdescr = ($_GET['edit'] == "new") ? $mgrlang['pc_new_message'] : $mgrlang['pc_edit_message']); ?></span></p>
            ...
            <div id="content" style="padding: 0px;">                
                ...
                <div id="tab1_group" class="group" style="display: block;">
					...
					?>
                        <div class="<?php fs_row_color(); ?>" id="name_div">
                            ...
                            <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                                <?php echo $mgrlang['gen_code']; ?>:<br />
                                <span><?php echo $mgrlang['gen_code_d']; ?></span>
                            </p>
                            <div style="float: left; margin-top: 10px;">{content id=<?php echo $content->content_id; ?>}</div>
                        </div>
                    <?php
						}
					?>
                    ...
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        ...
                        <p class="input_label" onclick="support_popup('<?php echo $supportPageID; ?>');"><?php echo $mgrlang['gen_content_name']; ?>:<br />
                        <span class="input_label_subtext"><?php echo $mgrlang['gen_content_name_d']; ?></span></p>
					...
					?>
                        <div class="<?php fs_row_color(); ?>" id="name_div">
                            <img src="images/mgr.ast.off.gif" class="ast" />
                            <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                                <?php echo $mgrlang['gen_content_type']; ?>:<br />
                                <span><?php echo $mgrlang['gen_content_type_d']; ?></span>
                            </p>
                            <select name="contenttype" id="contenttype">
                                <option value="2"><?php echo $mgrlang['gen_content_block']; ?></option>
                                <option value="3"><?php echo $mgrlang['gen_content_page']; ?></option>
                            </select>
                        </div>
                    <?php
						}
...
<textarea style="width: 375px; height: 135px; background-color: #fdf9d0; border: 0; padding: 10px; border: 1px solid #CCC"><?php include 'mgr.clips.php'; ?></textarea>
                    ...
                    <div class="<?php fs_row_color(); ?>">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p for="x" onclick="support_popup('<?php echo $supportPageID; ?>');">
                            <?php echo $mgrlang['f_content']; ?>: <br />
                            <span class="input_label_subtext"><?php echo $mgrlang['f_content_d']; ?></span>
                        </p>
					...
					<div class="<?php fs_row_color(); ?>">
						...
						<input name="uploadedfile" type="file" style="font-size: 11; width: 500">
						<?php
						#GET FILES LIST
						$file_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}content_files WHERE content_id = '$_GET[edit]' AND type = 'page' ORDER BY file_name ASC");
					...
					<?php
					if($_GET['edit'] == 'new' OR $content->ca_id == 3){
					?>
					<div class="<?php fs_row_color(); ?>">
						<img src="images/mgr.ast.off.gif" class="ast" />
						<p class="input_label" onclick="support_popup('<?php echo $supportPageID; ?>');"><?php echo $mgrlang['cp_linked']; ?>:<br />
						<span class="input_label_subtext"><?php echo $mgrlang['cp_linked_d']; ?></span></p>
						<input type="text" name="linked" id="linked" style="width: 300px;" value="<?php echo $content->linked; ?>">
					</div>
					<?php
					} else {
					?>
						<input type="hidden" name="linked" id="linked" value="">
					<?php
					}
					?>
					...
					<div class="<?php fs_row_color(); ?>" id="name_div" fsrow='1'>
                        ...
                        <?php if($_GET['edit'] == 'new' or $content->ca_id == '2' or $content->ca_id == '3'){ ?>
							<input type="checkbox" name="active" value="1" style="padding: 0; margin: 6px 0 0 0;" <?php if($content->active or $_GET['edit'] == 'new'){ echo "checked"; } ?> />
						<?php } else { ?>
							<input type="hidden" name="active" value="1" />
							<input type="checkbox" name="activeFaux" value="1" checked="checked" disabled="disabled" />
						<?php } ?>
                    </div>
                </div>
            </div>
            <div id="save_bar">
                <input type="button" value="<?php echo $mgrlang['gen_b_cancel']; ?>" onclick="cancel_edit('mgr.page.content.php');" /><input type="submit" value="<?php echo $mgrlang['gen_b_save']; ?>" />
            </div>
            </form>
