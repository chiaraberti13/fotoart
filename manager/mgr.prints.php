<?php
	...
	###################################################################
		...
		switch($_REQUEST['action']){
			case "save_groups":				
				save_groups($page,'prints','print_id');				
			break;			
			case "save_sort":
				save_sort('prints','print_id');
			break;
			case "clear_sort":
				clear_sort('prints');
			break;
			case "save_status":
				save_status($page,'prints','print_id');
			break;
			# DELETE
			case "del":
				if(!empty($_REQUEST['items'])){
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT item_name,print_id FROM {$dbinfo[pre]}prints WHERE print_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result)){
						$log_titles.= "$log->item_name ($log->print_id), ";
						
						# MARK AS DELETED
						@$sql = "UPDATE {$dbinfo[pre]}prints SET deleted='1' WHERE print_id = '$log->print_id'";
						@$result = mysqli_query($db,$sql);
						
						# REMOVE ANY PRODUCT SHOTS
						$ip_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}item_photos WHERE item_id = '$log->prod_id' AND mgrarea = 'print'");
						...
						while($ip = mysqli_fetch_object($ip_result))
						{
							...
							if(file_exists("../assets/item_photos/print".$printzf."_ip".$ipidzf."_org.jpg"))
							{
								unlink("../assets/item_photos/print".$printzf."_ip".$ipidzf."_org.jpg");
							}
							if(file_exists("../assets/item_photos/print".$printzf."_ip".$ipidzf."_med.jpg"))
							{
								unlink("../assets/item_photos/print".$printzf."_ip".$ipidzf."_med.jpg");
							}
							if(file_exists("../assets/item_photos/print".$printzf."_ip".$ipidzf."_small.jpg"))
							{
								unlink("../assets/item_photos/print".$printzf."_ip".$ipidzf."_small.jpg");
							}
						}
					}
					...
					mysqli_query($db,"DELETE FROM {$dbinfo[pre]}media_prints WHERE print_id IN ($delete_array)");
					
					# REMOVE PACKAGE ITEMS ENTRIES
					mysqli_query($db,"DELETE FROM {$dbinfo[pre]}package_items WHERE item_type = 'print' AND item_id IN ($delete_array)");
					...
					mysqli_query($db,"DELETE FROM {$dbinfo[pre]}item_galleries WHERE item_id IN ($delete_array) AND mgrarea = 'prints'");
					...
					# UPDATE ACTIVITY LOG
					save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_prints'],1,$mgrlang['gen_b_del'] . " > <strong>$log_titles</strong>");
		...
		# IF THIS IS AN ENTRY PAGE OR printgroups IS BLANK RESET THE printgroups SESSION	
		if($_GET['ep'] or empty($_SESSION['printgroups'])){
			$_SESSION['printgroups'] = array('all');
		}			
		# SEE IF ANY GROUPS HAVE BEEN PASSED
		if($_REQUEST['setgroups']){
			if(is_array($_REQUEST['setgroups'])){
				$_SESSION['printgroups'] = $_REQUEST['setgroups'];
			} else {				
				$_SESSION['printgroups'] = array($_REQUEST['setgroups']);
			}
		}
		
		# GET THE TOTAL NUMBER OF ROWS
		if(in_array("all",$_SESSION['printgroups']))
		{
			$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(print_id) FROM {$dbinfo[pre]}prints WHERE deleted='0'"));
		}
		else
		{
			$prints_result2 = "SELECT COUNT(print_id) FROM {$dbinfo[pre]}prints LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}prints.print_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['printgroups']).") AND {$dbinfo[pre]}prints.deleted='0'";
			$r_rows = mysqli_result_patch(mysqli_query($db,$prints_result2));
		}
		...
		# SELECT ITEMS
		if(in_array("all",$_SESSION['printgroups'])){
			$prints_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}prints WHERE deleted='0' ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
		} else {
			$prints_result = mysqli_query($db,"SELECT sortorder,item_name,active,print_id FROM {$dbinfo[pre]}prints LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}prints.print_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['printgroups']).") AND {$dbinfo[pre]}prints.deleted='0' GROUP BY {$dbinfo[pre]}prints.print_id ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
		}
...
<head>
	...
	<script language="javascript">
		Event.observe(window, 'load', function()
			...
			if($('abutton_add_new')!=null)
			{
				$('abutton_add_new').observe('click', function()
					{
						window.location.href='mgr.prints.edit.php?edit=new';
					});
		...
		function load_group_selector()
		{
			...
			var loadpage = "mgr.groups.actions.php?mode=grouplist&mgrarea=<?php echo $page; ?>&ingroups=<?php if(in_array('all',$_SESSION['printgroups'])){ echo 0; } else { echo 1; } ?>&exitpage=<?php echo $_SERVER['PHP_SELF']; ?>&sessname=printgroups";
		...
		function switch_status(item_type,item_id){
			...
			else
			{
				...
				var loadpage = "mgr.prints.actions.php?pmode=" + item_type + "&id=" + item_id;
				var pars = "";
		...
		function clear_sorts(){
			location.href='<?php echo $_SERVER['PHP_SELF']; ?>?ep=1&action=clear_sort&listby=print_id&listtype=asc';
		}
...
<body>
	...
	<div id="container">
        ...
        ?>
            <!-- TITLE BAR AREA -->
            <?php
				$prints_group_result = mysqli_query($db,"SELECT gr_id,flagtype,name FROM {$dbinfo[pre]}groups WHERE mgrarea = '$page' ORDER BY name");
				$prints_group_rows = mysqli_num_rows($prints_group_result);
			?>
            
            <?php
			/*
            <div id="title_bar">
                <img src="./images/mgr.badge..gif" class="badge" />
                <p><strong><?php echo $mgrlang['subnav_prints']; ?></strong><br /><span><?php echo $mgrlang['subnav_prints_d']; ?></span></p>
            </div>
            <!-- ACTIONS BAR AREA -->
            <div id="actions_bar">							
                <div style="float: left; padding-left: 3px;">
                    <div style="float: left;" class="abuttons"><a href="mgr.prints.edit.php?edit=new"><img src="./images/mgr.button.add.news.gif" align="absmiddle" border="0" alt="<?php echo $mgrlang['gen_b_new']; ?>" /></a><br /><?php echo $mgrlang['gen_b_new']; ?></div>
                    <?php if(!empty($prints_rows)){ ?>
                        <div style="float: left;" class="abuttons"><a href="#"><img src="./images/mgr.button.select.all.gif" align="absmiddle" border="0" alt="<?php echo $mgrlang['gen_b_sa']; ?>" onclick="select_all_cb('datalist');" /></a><br /><?php echo $mgrlang['gen_b_sa']; ?></div>
                        <div style="float: left;" class="abuttons"><a href="#"><img src="./images/mgr.button.select.none.gif" align="absmiddle" border="0" alt="<?php echo $mgrlang['gen_b_sn']; ?>" onclick="deselect_all_cb('datalist');" /></a><br /><?php echo $mgrlang['gen_b_sn']; ?></div>	
                        <div style="float: left;" class="abuttons"><a href="javascript:deleterec();"><img src="./images/mgr.button.delete.gif" align="absmiddle" border="0" alt="<?php echo $mgrlang['gen_delete_sel']; ?>" /></a><br /><?php echo $mgrlang['gen_b_del']; ?></div>
                        <!--<div style="float: left;" class="abuttons"><a href="javascript:deleterec();"><img src="./images/mgr.button.group.png" align="absmiddle" border="0" alt="<?php echo $mgrlang['gen_b_grps']; ?>" /></a><br /><?php echo $mgrlang['gen_b_grps']; ?></div>	-->				
                    <?php } ?>
                    <div style="float: left;" class="abuttons"><a href="javascript:sdisplaybool('group_selector');"><img src="./images/mgr.button.group.png" align="absmiddle" border="0" alt="<?php echo $mgrlang['gen_b_grps_alt']; ?>" /></a><br /><?php echo $mgrlang['gen_b_grps']; ?></div>
                </div>                
            </div>
			*/
			?>
            ...
            <div id="actions_bar">	
            	<div class="sec_bar">
                    <img src="./images/mgr.badge.prints.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_prints']; ?></span> &nbsp; 
                </div>                
                ...
                <div style="padding-top: 9px; margin-right: 20px;"> 
                    ...
                    <select style="font-size: 14px;" align="absmiddle" id="actionsdd" onchange="do_actions();">
                        <option value="#" style="background-color: #eeeeee;"><?php echo $mgrlang['gen_actions']; ?>:</option>
                        <option value="assign_groups">&nbsp; <?php echo $mgrlang['gen_au_itg']; ?></option>
                        <option value="sort_items">&nbsp; <?php echo $mgrlang['gen_sort']; ?></option>
                    </select>
                    -->
                    <?php
						if($prints_rows){
                    ...
                    <select align="absmiddle" id="actionsdd" onchange="do_actions();">
                        ...
                        <?php if($prints_group_rows and $prints_rows and in_array("pro",$installed_addons)){ ?><option value="assign_groups">&nbsp; <?php echo $mgrlang['gen_au_itg']; ?></option><?php } ?>
                        <option value="set_status">&nbsp; <?php echo $mgrlang['gen_tostatus']; ?></option>
                        <?php if($prints_rows >= 2){ ?><option value="sort_items">&nbsp; <?php echo $mgrlang['gen_sort']; ?></option><?php } ?>
                    </select>
                   	<?php
						}
					?>
			...
			<form name="grouplist" id="grouplist" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
				...
				<div style="<?php if(in_array('all',$_SESSION['printgroups'])){ echo "display: none;"; } else { echo "display: block;"; } ?>" class="options_area" id="group_selector"></div>
			</form>
            <!-- START CONTENT -->
            <?php
                ...
                <div id="content" style="border-top: 0px;">						
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
							...
							<?php $header_name = "print_id";	if($listby == $header_name){ $sel = "_sel"; } else { $sel = ""; }	$hlink = ""; $hlink = $_SERVER['PHP_SELF'] . "?listby=" . $header_name . "&listtype="; if($listtype == "asc" && $listby == $header_name){ $hlink .= "desc"; } else { $hlink .= "asc"; } ?><td class="cth<?php echo $sel; ?>" onclick="window.location.href='<?php echo $hlink; ?>'" align="center" width="100">	<div class="content_table_header<?php echo $sel; ?>"><div><a href="<?php echo $hlink ?>"><?php echo $mgrlang['gen_t_id']; ?></a></div></div></td>
                        ...
                        <?php								
                            ...
                            while($prints = mysqli_fetch_object($prints_result)){
                                ...
                                if($prints->{"item_name_" . $config['settings']['lang_file_mgr']}){
                                    $name = $prints->{"item_name_" . $config['settings']['lang_file_mgr']};
                                } else {
                                    $name = $prints->item_name;
                                }
                                
                                # IF THE NAME IS TOO LONG CROP IT / ALSO TAKE OUT ALL HTML IF THERE IS ANY
                                if(strlen(strip_tags($name)) > 70){
                                    $name = substr(strip_tags($name),0,67) . "..."; 
                                } else {
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onMouseOver="cellover(this,'#<?php echo $color_fade; ?>',32);" onMouseOut="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"></a><?php if($prints->sortorder) { echo $prints->sortorder;} else { echo "--"; } ?></td>
                                <td align="center"><a name="row_<?php echo $prints->print_id; ?>"></a><?php echo $prints->print_id; ?></td>
                                <td onclick="window.location.href='mgr.prints.edit.php?edit=<?php echo $prints->print_id; ?>'">
									<div style="float: left;"><a href="mgr.prints.edit.php?edit=<?php echo $prints->print_id; ?>" class="editlink" onMouseOver="show_details_win('details_<?php echo $prints->print_id; ?>','mgr.prints.actions.php?pmode=details&id=<?php echo $prints->print_id; ?>');" onMouseOut="hide_gdetails('details_<?php echo $prints->print_id; ?>');"><?php echo $prints->item_name; ?></a></div>
                                	<div style="float: left; overflow: visible;">
										<div id="details_<?php echo $prints->print_id; ?>" class="galdet_win" style="display: none;">Loading Details</div>
									</div>
								</td>
                                <td align="center"><div id="ac<?php echo $prints->print_id; ?>"><a href="javascript:switch_status('ac','<?php echo $prints->print_id; ?>');"><img src="images/mgr.small.check.<?php echo $prints->active; ?>.png" border="0" /></a></div></td>
                                <td align="center" valign="middle" nowrap>
                                    <a href="mgr.prints.edit.php?edit=<?php echo $prints->print_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a>
                                    <a href="javascript:deleterec(<?php echo $prints->print_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a> 
                                <td align="center" valign="middle" nowrap>
                                    <input type="checkbox" name="items[]" value="<?php echo $prints->print_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
                            }
