<?php
	...
	###################################################################
		...
		if($_GET['edit'] != "new" and !$_REQUEST['action']){
			$prod_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}products WHERE prod_id = '$_GET[edit]'");
			$prod_rows = mysqli_num_rows($prod_result);
			$prod = mysqli_fetch_object($prod_result);
			
			$_SESSION['item_id'] = $prod->prod_id;
			$_SESSION['mgrarea'] = 'prod';
		}
		
		if($_GET['edit'] == "new")
		{
			# DELETE ANY ORPHANED ITEM PHOTOS
			delete_orphaned_item_photos('prod');
			
			# DELETE ANY ORPHANED OPTION GROUPS
			delete_orphaned_optiongroups('print');
		...
		if($_REQUEST['action'] == "save_edit" or $_REQUEST['action'] == "save_new" ){
			...
			if($_REQUEST['action'] == "save_edit"){
				...
				# UPDATE THE DATABASE
				$sql = "UPDATE {$dbinfo[pre]}products SET 
							item_name='$item_name',
				...
				# UPDATE ACTIVITY LOG
				save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_products'],1,$mgrlang['gen_b_ed'] . " > <strong>$item_name</strong>");
			}
			
			if($_REQUEST['action'] == "save_new"){
				
				$uprod_id = create_unique2();
				...
				$sql = "INSERT INTO {$dbinfo[pre]}products (
						item_name,
						description,
						uprod_id,
						item_code,
				...
				# RENAME ORPHANED ITEM PHOTOS UNDER THIS PRODUCT
				save_new_item_photos('prod',$saveid);
				...
				# UPDATE ACTIVITY LOG
				save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_products'],1,$mgrlang['gen_b_new'] . " > <strong>$item_name</strong>");
			}				
			header("location: mgr.products.php?mes=edit"); exit;
		}		
?>
...
<head>
    ...
    <!-- FP -->
    <script type="text/javascript" src="../assets/javascript/mgr.photo.puzzle.js"></script>
	...
	<script language="javascript" type="text/javascript">	
		...
		function form_submitter(){
			...
			//var curoption = 0;
			//var optionerror = 0;
			//$$('input.prod_option_name').each(
			//	function (){
			//		if($F($$('input.prod_option_name')[curoption]) == "" || $F($$('input.prod_option_name')[curoption]) ==  null){
			//			optionerror = 1;
			//		}
			//		curoption++;
			//	}				
			//);		
			//if(optionerror == 1){
			//	simple_message_box('<?php echo $mgrlang['products_mes_04']; ?>','');
			//	return false;
			//}
			<?php
				if($_SESSION['admin_user']['admin_id'] == "DEMO"){
					...
					$action_link = ($_GET['edit'] == "new") ? "mgr.products.edit.php?action=save_new" : "mgr.products.edit.php?action=save_edit";
			
					# CHECK FIELD AND OUTPUT MESSAGE - ID, LANGUAGE, BRING TO FRONT, CUSTOM MESSAGE
					js_validate_field("item_name","products_f_name",1);
				}
			?>
		}
		...
		// THE PRODUCT TYPE BOX HAS BEEN CHANGED
		function product_type_change()
		{
			var selecteditem = $('product_type').options[$('product_type').selectedIndex].value;
			...
			{	
				if($('price_calc') != undefined)
				{
					$('price_calc').hide();
					$('price_calc').setValue('norm');
				}
				
				if($('credits_calc') != undefined)
				{
					$('credits_calc').hide();
					$('credits_calc').setValue('norm');
				}
		...
		?>
		uploadobj.id = '<?php echo $prod->prod_id; ?>'; // THE ID OF THE ITEM
				...
				if(ip_length < <?php echo $config['MaxProductShots']; ?>)
				{
					<?php
						if($config['settings']['uploader'] == '4xxx')
							echo "window.open('mgr.plupload.php?itemPhotos=1&mgrarea={$_SESSION[mgrarea]}&id={$prod->prod_id}', 'Plupload', 'width=800,height=380,scrollbars=yes,menubar=no,titlebar=no');";
						else
		...
		function load_gals()
		{
			...
			var myAjax = new Ajax.Updater('gals', 'mgr.products.actions.php', {method: 'get', parameters: pars, onComplete: display_in_all_check});
		}
		...
		function load_ip()
		{
			...
			var myAjax = new Ajax.Updater('ip_div', 'mgr.products.actions.php', {method: 'get', parameters: pars});
		}
...
<body>
	...
	<div id="container">
        ...
        ?>
            ...
            <!-- TITLE BAR AREA -->
            <div id="title_bar">
                <img src="./images/mgr.badge.products.png" class="badge" />
                <p><strong><?php echo ($ptitle = ($_GET['edit'] == "new") ? $mgrlang['products_new_header'] : $mgrlang['products_edit_header']); ?></strong><br /><span><?php echo ($pdescr = ($_GET['edit'] == "new") ? $mgrlang['products_new_message'] : $mgrlang['products_edit_message']); ?></span></p>
            ...
            <div id="content">
                ...
                <?php
					# PULL GROUPS
					$prod_group_result = mysqli_query($db,"SELECT gr_id,flagtype,name FROM {$dbinfo[pre]}groups WHERE mgrarea = '$page' ORDER BY name");
                ...
                <div id="button_bar">
                    ...
                    <div class="subsuboff" onclick="bringtofront('9');" id="tab9"><?php echo $mgrlang['gen_tab_pricing']; ?></div>
                    <div class="subsuboff" onclick="bringtofront('7');" id="tab7"><?php echo $mgrlang['gen_tab_attach']; ?></div>
                    <div class="subsuboff" onclick="bringtofront('8');load_ip();" id="tab8"><?php echo $mgrlang['gen_tab_prod_shots']; ?></div>
                    ...
                    <?php if($prod_group_rows and in_array("pro",$installed_addons)){ ?><div class="subsuboff" onclick="bringtofront('3');" id="tab3"><?php echo $mgrlang['gen_tab_groups']; ?></div><?php } ?>
                    <?php if($config['settings']['cart'] and in_array("contr",$installed_addons)){ ?><div class="subsuboff" onclick="bringtofront('6');" id="tab6" <?php if($prod->product_type != '1'){ echo "style='display: none;'"; } ?>><?php echo $mgrlang['gen_tab_contributors']; ?></div><?php } ?>
                ...
                <div id="tab1_group" class="group" style="display: block; padding-bottom: 5px;">                    
                    <div class="<?php fs_row_color(); ?>" id="name_div" fsrow='1'>
                        ...
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
							<?php echo $mgrlang['products_f_top']; ?>:<br />
                        	<span><?php echo $mgrlang['products_f_top_d']; ?></span>
                        </p>
                        <select style="width: 300px;" id="product_type" name="product_type" onchange="product_type_change();">
                        	<option value="1" <?php if($prod->product_type == '1' or $_GET['edit'] == "new"){ echo "selected"; } ?>><?php echo $mgrlang['products_f_medbased']; ?></option>
                            <option value="2" <?php if($prod->product_type == '2'){ echo "selected"; } ?>><?php echo $mgrlang['products_f_standalone']; ?></option>
                        </select>
                    </div>
                    
                    <div class="<?php fs_row_color(); ?>" id="item_name_div" fsrow='1'>
                        <img src="images/mgr.ast.gif" class="ast" />
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
							<?php echo $mgrlang['products_f_name']; ?>:<br />
                        	<span><?php echo $mgrlang['products_f_name_d']; ?></span>
                        </p>
                        <div class="additional_langs">
                            <input type="text" name="item_name" id="item_name" style="width: 290px;" maxlength="50" value="<?php echo @htmlentities(stripslashes($prod->item_name)); ?>" />
                            <?php
                                ...
                                ?>
									...
									<?php
                                        foreach($active_langs as $value){
                                    ?>
                                        <li><input type="text" name="item_name_<?php echo $value; ?>" id="item_name_<?php echo $value; ?>" style="width: 290px;" maxlength="100" value="<?php echo @htmlentities(stripslashes($prod->{"item_name" . "_" . $value})); ?>" />&nbsp;&nbsp;<span class="mtag_dblue" style="color: #FFF"><?php echo ucfirst($value); ?></span></li>
                                <?php
                                        }
                                        echo "</ul></div>";
                                    }
                    ...
                    <div class="<?php fs_row_color(); ?>" fsrow='1'>
                        ...
                        <div class="additional_langs">
                            <textarea name="description" id="description" style="width: 290px; height: 50px; vertical-align: middle"><?php echo @stripslashes($prod->description); ?></textarea>
                            <?php
                                if(in_array('multilang',$installed_addons))
                                {
                            ?>
                                ...
                                <?php
                                    ...
                                    {
                                ?>
                                    <li><textarea name="description_<?php echo $value; ?>" style="width: 290px; height: 50px; vertical-align: middle"><?php echo @stripslashes($prod->{"description" . "_" . $value}); ?></textarea>&nbsp;&nbsp;<span class="mtag_dblue" style="color: #FFF"><?php echo ucfirst($value); ?></span></li>
                            <?php
                                    }
                                    echo "</ul></div>";
                                }
                    ...
                    <div class="<?php fs_row_color(); ?>" id="quantity_div" <?php if($prod->product_type == 1 or $_GET['edit'] == 'new'){ echo "style='display: none;'"; } ?> fsrow='1'>
                        ...
                        <input type="text" name="quantity" id="quantity" style="width: 90px;" maxlength="50" value="<?php echo @stripslashes($prod->quantity); ?>" />
                    </div>
                    <div class="<?php fs_row_color(); ?>" id="name_div" fsrow='1'>
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                        ...
                        <input type="checkbox" name="active" value="1" style="padding: 0; margin: 6px 0 0 0;" <?php if($prod->active or $_GET['edit'] == 'new'){ echo "checked"; } ?> />
                    </div>
            	</div>                
                ...
                <div id="tab9_group" class="group">
					...
					?>
                        <div class="<?php fs_row_color(); ?>" id="name_div">
                            <img src="images/mgr.ast.off.gif" class="ast" />
                            <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                                <?php echo $mgrlang['products_f_price']; ?>:<br />
                                <span><?php echo $mgrlang['products_f_price_d']; ?></span><br /><br />
                            </p>
                            <?php
                                /*
                            <select id="price_calc" name="price_calc" style="width: 100px;" >
                                <option value="$" <?php if(@$prod->price_calc == '$'){ echo "selected"; } ?>><?php echo $config['settings']['cur_denotation']; ?></option>
                                <option value="+" <?php if(@$prod->price_calc == '+'){ echo "selected"; } ?>><?php echo $mgrlang['products_base_price']; ?> +</option>
                                <option value="-" <?php if(@$prod->price_calc == '-'){ echo "selected"; } ?>><?php echo $mgrlang['products_base_price']; ?> -</option>
                                <option value="x" <?php if(@$prod->price_calc == 'x'){ echo "selected"; } ?>><?php echo $mgrlang['products_base_price']; ?> x</option>
                            </select>
                                */
                            ?>
                            ...
                            <select id="price_calc" name="price_calc" style="width: 140px;">
                                <option value="norm" <?php if(@$prod->price_calc == 'norm'){ echo "selected"; } ?>><?php echo $config['settings']['cur_denotation']; ?></option>
                                <option value="add" <?php if(@$prod->price_calc == 'add'){ echo "selected"; } ?>><?php echo $mgrlang['gen_base_photoprice']; ?> ( + )</option>
                                <option value="sub" <?php if(@$prod->price_calc == 'sub'){ echo "selected"; } ?>><?php echo $mgrlang['gen_base_photoprice']; ?> ( - )</option>
                                <option value="mult" <?php if(@$prod->price_calc == 'mult'){ echo "selected"; } ?>><?php echo $mgrlang['gen_base_photoprice']; ?> ( x )</option>
                            </select>
                            <?php
                                } else {
                            ...
                            ?>                        
                            <input type="text" name="price" id="price" style="width: 90px;" maxlength="50" onblur="update_input_cur('price');" value="<?php if($prod->price > 0){ echo @$cleanvalues->currency_display($prod->price); } ?>" /> <?php $cleanvalues->example_currency_text(5.00,6.50); ?>
                        ...
                        <div class="<?php fs_row_color(); ?>" id="name_div">
                            <img src="images/mgr.ast.off.gif" class="ast" />
                            <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                                <?php echo $mgrlang['products_f_taxable']; ?>:<br />
                                <span><?php echo $mgrlang['products_f_taxable_d']; ?></span>
                            </p>
                            <input type="checkbox" name="taxable" value="1" style="padding: 0; margin: 6px 0 0 0;" <?php if($prod->taxable or $_GET['edit'] == 'new'){ echo "checked"; } ?> />
                        </div> 
                    <?php
						}
						if(($config['settings']['cart'] == 2 or $config['settings']['cart'] == 3) and $config['settings']['credits_prod'])
						{
					?>
						<div class="<?php fs_row_color(); ?>" id="name_div">
							<img src="images/mgr.ast.off.gif" class="ast" />
							<p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
								<?php echo $mgrlang['products_f_credits']; ?>:<br />
								<span><?php echo $mgrlang['products_f_credits_d']; ?></span><br /><br />
							</p>
							<?php
								if($config['settings']['flexpricing'] == 1){
							?>
							<select id="credits_calc" name="credits_calc" style="width: 140px;">
								<option value="norm" <?php if(@$prod->credits_calc == 'norm'){ echo "selected"; } ?>></option>
								<option value="add" <?php if(@$prod->credits_calc == 'add'){ echo "selected"; } ?>><?php echo $mgrlang['gen_base_photocredits']; ?> ( + )</option>
								<option value="sub" <?php if(@$prod->credits_calc == 'sub'){ echo "selected"; } ?>><?php echo $mgrlang['gen_base_photocredits']; ?> ( - )</option>
								<option value="mult" <?php if(@$prod->credits_calc == 'mult'){ echo "selected"; } ?>><?php echo $mgrlang['gen_base_photocredits']; ?> ( x )</option>
							</select>
							<?php
								} else {
							...
							?> 
							<input type="text" name="credits" id="credits" style="width: 90px;" maxlength="50" value="<?php echo $prod->credits; ?>" />
                        ...
                        <div class="<?php fs_row_color(); ?>">
                            ...
                            <?php
                               if($config['settings']['flexpricing'] == 1){
                            ?>
                            <select id="discountc_calc" name="discountc_calc" style="width: 140px;">
                                <option value="norm" <?php if(@$prod->discountc_calc == 'norm'){ echo "selected"; } ?>></option>
                                <option value="add" <?php if(@$prod->discountc_calc == 'add'){ echo "selected"; } ?>><?php echo $mgrlang['gen_base_photocredits']; ?> ( + )</option>
                                <option value="sub" <?php if(@$prod->discountc_calc == 'sub'){ echo "selected"; } ?>><?php echo $mgrlang['gen_base_photocredits']; ?> ( - )</option>
                                <option value="mult" <?php if(@$prod->discountc_calc == 'mult'){ echo "selected"; } ?>><?php echo $mgrlang['gen_base_photocredits']; ?> ( x )</option>
                            </select>
                            <?php
                                } else {
                                    echo "<input type='hidden' name='discountc_calc' value='norm' />";
                                }
                            ?> 
                            <input type="text" name="discountc" id="discountc" style="width: 90px;" maxlength="50" value="<?php echo $cleanvalues->number_display($prod->discountc); ?>" />
                        </div>
				<?php
						*/
						}
				?>            
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                            <?php echo $mgrlang['products_f_multiple']; ?>:<br />
                            <span><?php echo $mgrlang['products_f_multiple_d']; ?></span>
                        </p>
                        <input type="checkbox" name="multiple" value="1" style="padding: 0; margin: 6px 0 0 0;" <?php if($prod->multiple or $_GET['edit'] == 'new'){ echo "checked"; } ?> />
                    </div>                      
                </div>
                ...
                <div id="tab3_group" class="group"> 
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
							<?php echo $mgrlang['products_f_groups']; ?>:<br />
                        	<span><?php echo $mgrlang['products_f_groups_d']; ?></span>
                        </p>
                        <?php
							$plangroups = array();
							# FIND THE GROUPS THAT THIS ITEM IS IN
							$prod_groupids_result = mysqli_query($db,"SELECT group_id FROM {$dbinfo[pre]}groupids WHERE mgrarea = '$page' AND item_id = '$prod->prod_id' AND item_id != 0");
							while($prod_groupids = mysqli_fetch_object($prod_groupids_result)){
								$plangroups[] = $prod_groupids->group_id;
							}
							
							echo "<ul style='float: left; margin: 0px; list-style-type:none; padding: 0;'>";
							while($prod_group = mysqli_fetch_object($prod_group_result)){
								echo "<li><input type='checkbox' id='$prod_group->gr_id' class='permcheckbox' name='setgroups[]' value='$prod_group->gr_id' "; if(in_array($prod_group->gr_id,$plangroups)){ echo "checked "; } echo "/> "; if($prod_group->flagtype == 'icon.none.gif'){ echo "<img src='./images/mini_icons/$prod_group->flagtype' align='absmiddle' width='1' height='16' /> "; } else { echo "<img src='./images/mini_icons/$prod_group->flagtype' align='absmiddle' /> "; } echo "<label for='$prod_group->gr_id'>".substr($prod_group->name,0,30)."</label></li>";
							}
							echo "</ul>";
						?>
                    </div>
                ...
                <div id="tab4_group" class="group">
                    ...
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
							<?php echo $mgrlang['products_f_shipping']; ?>:<br />
                        	<span><?php echo $mgrlang['products_f_shipping_d']; ?></span>
                        </p>
                    ...
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        ...
                        <input type="text" name="weight" id="weight" style="width: 100px;" maxlength="50" value="<?php echo @stripslashes($prod->weight); ?>" />
                    ...
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        ...
                        <input type="text" name="addshipping" id="addshipping" style="width: 100px;" maxlength="50" onblur="update_input_cur('addshipping');" value="<?php if($prod->addshipping > 0){ echo $cleanvalues->currency_display($prod->addshipping); } ?>" /> <?php $cleanvalues->example_currency_text(5.00,6.50); ?>
                    </div>
                </div>
                ...
                <div id="tab5_group" class="group">
                	<div class="<?php fs_row_color(); ?>" id="name_div" fsrow='1'>
                        ...
                        <input type="text" name="item_code" id="item_code" style="width: 90px;" maxlength="50" value="<?php echo @stripslashes($prod->item_code); ?>" />
                    </div>
					<!--
                    <div class="<?php fs_row_color(); ?>" id="name_div">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        ...
                        <select class='select' name='notify_lab' style="width: 298px; margin-top: 12px;">
							...
							<?php
	                        	...
	                        	while($lab = mysqli_fetch_object($lab_result)){
									...
									if($lab->lab_id == $prod->notify_lab){ echo " selected"; }
                    ...
                    <div class="<?php fs_row_color(); ?>">
                        ...
                        <input type="text" name="lab_code" id="lab_code" style="width: 90px;" maxlength="50" value="<?php echo @stripslashes($prod->lab_code); ?>" />
                    </div>
                    -->
                    <?php
					...
					?>
                        ...
                        <?php
							if($_GET['edit'] != 'new' and $prod->everyone == '0'){
						...
						?>
                        <div style="float: left; padding-right: 10px; font-weight: bold; <?php if($prod->everyone == 1 or $_GET['edit'] == 'new'){ echo "display: block;"; } else { echo "display: none;"; }  ?>" id="perm_div_a"><?php echo $mgrlang['gen_wb_everyone']; ?></div>
						<div style="float: left; padding-right: 10px; font-weight: bold; <?php if($prod->everyone == 0 and $_GET['edit'] != 'new'){ echo "display: block;"; } else { echo "display: none;"; } ?>" id="perm_div_b"><?php echo $mgrlang['gen_wb_limited']; ?></div>	
                        <input type="hidden" name="perm" id="perm<?php echo $prod->prod_id; ?>" value="<?php echo $perm_value; ?>" />
                        <div style="float: left;"><a href="javascript:workbox2({page: 'mgr.workbox.php',pars: 'box=permissions_selector&style=everyone&inputbox=perm<?php echo $prod->prod_id; ?>&multiple=1'});" class="actionlink"><img src="images/mgr.people.icon.png" border="0" id="perm_icon" /></a></div>
                    </div>
                    <div class="<?php fs_row_color(); ?>">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                        ...
                        <textarea name="notes" id="notes" style="width: 290px; height: 50px; vertical-align: middle"><?php echo @stripslashes($prod->notes); ?></textarea>
                    </div>
                </div>
                ...
                <div id="tab10_group" class="group">  
                	<div class="<?php fs_row_color(); ?>">
                        ...
                        <input type="checkbox" name="homepage" value="1" style="padding: 0; margin: 6px 0 0 0;" <?php if($prod->homepage){ echo "checked"; } ?> />
                    </div>
                    <div class="<?php fs_row_color(); ?>">
                        <?php
							if($config['settings']['prodpage'] == 0){
						?>
                        ...
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                        	...
                        	<span><?php echo $mgrlang['products_f_adv_d']; ?></span>
                        </p>
                        <input type="checkbox" name="featured" value="1" style="padding: 0; margin: 6px 0 0 0;" <?php if($prod->featured){ echo "checked"; } ?> />
                    </div>
                </div>
                ...
                ?>
                	<div id="tab6_group" class="group">                	
						<div class="<?php fs_row_color(); ?>" fsrow='1'>
                            ...
                            <input type="checkbox" name="contr_sell" value="1" style="padding: 0; margin: 6px 0 0 0;" <?php if($prod->contr_sell){ echo "checked"; } ?> onclick="$('contr_settings').toggle()" />
                        </div>
                        <div id="contr_settings" style="display: <?php if($prod->contr_sell){ echo "blcok"; } else { echo "none"; } ?>;">
                            <div class="<?php fs_row_color(); ?>" fsrow='1'>
                                <img src="images/mgr.ast.off.gif" class="ast" />
                                <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                                ...
                                <select style="width: 200px;" id="commission_type" name="commission_type" onchange="commission_type_change();">
                                    <option value="1" <?php if($prod->commission_type == '1' or $_GET['edit'] == "new"){ echo "selected"; } ?>><?php echo $mgrlang['percentage']; ?></option>
                                    <option value="2" <?php if($prod->commission_type == '2'){ echo "selected"; } ?>><?php echo $mgrlang['dollar_value']; ?></option>
                                </select>
                            </div>
                            <div class="<?php fs_row_color(); ?>" id="com_dollar" fsrow='1' style="<?php if($prod->commission_type == '1' or $_GET['edit'] == "new"){ echo "display: none;"; } ?>">
                                <img src="images/mgr.ast.off.gif" class="ast" />
                                <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                                    <?php echo $mgrlang['gen_com_value']; ?>:<br />
                                    <span><?php echo $mgrlang['gen_com_value_d']; ?></span>
                                </p>
                                <input type="text" name="commission_dollar" id="commission_dollar" onblur="update_input_cur('commission_dollar');" style="width: 70px;" maxlength="50" value="<?php echo @$cleanvalues->currency_display($prod->commission_dollar); ?>" /> <?php $cleanvalues->example_currency_text(5.00,6.50); ?>
                            </div>
                            <div class="<?php fs_row_color(); ?>" fsrow='1' id="com_percentage" style="<?php if($prod->commission_type == '2'){ echo "display: none;"; } ?>">
                                <img src="images/mgr.ast.off.gif" class="ast" />
                                <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                                ...
                                <?php
                                    ...
                                    $commission = ($_GET['edit'] == 'new') ? 0 : $prod->commission;
                            ...
                            ?>
                                <div class="<?php fs_row_color(); ?>" fsrow='1'>
                                    ...
                                    <div style="float: left;  font-size: 11px;"><strong><?php echo $mgrlang['min']; ?></strong><br /><input type="text" name="min_contr_price" id="min_contr_price" onblur="update_input_cur('min_contr_price');" style="width: 70px;" maxlength="50" value="<?php echo @$cleanvalues->currency_display($prod->min_contr_price); ?>" /></div>
                                    <div style="float: left;  font-size: 11px; margin-left: 5px;"><strong><?php echo $mgrlang['max']; ?></strong><br /><input type="text" name="max_contr_price" id="max_contr_price" onblur="update_input_cur('max_contr_price');" style="width: 70px;" maxlength="50" value="<?php echo @$cleanvalues->currency_display($prod->max_contr_price); ?>" /> <?php $cleanvalues->example_currency_text(5.00,6.50); ?></div>
                                </div>
                            <?php
								*/
                                }
                            ...
                            ?>
                                <div class="<?php fs_row_color(); ?>" fsrow='1'>
                                    ...
                                    <div style="float: left;  font-size: 11px;"><strong><?php echo $mgrlang['min']; ?></strong><br /><input type="text" name="min_contr_credits" id="min_contr_credits" style="width: 70px;" maxlength="50" value="<?php echo $cleanvalues->number_display($prod->min_contr_credits); ?>" /></div>
                                    <div style="float: left;  font-size: 11px; margin-left: 5px;"><strong><?php echo $mgrlang['max']; ?></strong><br /><input type="text" name="max_contr_credits" id="max_contr_credits" style="width: 70px;" maxlength="50" value="<?php echo $cleanvalues->number_display($prod->max_contr_credits); ?>" /></div>
                                </div>
                            <?php
								*/
								}
                ...
                <div id="tab7_group" class="group">
                    <div class="<?php fs_row_color(); ?>">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
							<?php echo $mgrlang['products_f_attach']; ?>:<br />
                        	<span><?php echo $mgrlang['products_f_attach']; ?></span>
                        </p>
                        <select name="attachment" id="attachment" onchange="attachment_update();">
                        	<option value="none" <?php if($prod->attachment == 'none'){ echo "selected='selected'"; } ?>><?php echo $mgrlang['gen_none']; ?></option>
                            <option value="galleries" <?php if($prod->attachment == 'galleries'){ echo "selected='selected'"; } ?>><?php echo $mgrlang['attach_sel_gal']; ?></option>
                            <option value="media" <?php if($prod->attachment == 'media'){ echo "selected='selected'"; } ?>><?php echo $mgrlang['attach_media_in_gal']; ?></option>
                            <option value="both" <?php if($prod->attachment == 'both'){ echo "selected='selected'"; } ?>><?php echo $mgrlang['attach_both']; ?></option>
                        </select>
                    </div> 
                    <div class="<?php fs_row_color(); ?>" id="gals_div" style="display: <?php if($prod->attachment == 'none' or $_GET['edit'] == 'new'){ echo "none"; } else { echo "block"; } ?>">
                        <img src="images/mgr.ast.off.gif" class="ast" />
                        <p for="name" onclick="support_popup('<?php echo $supportPageID; ?>');">
                ...
                <?php $row_color = 0; ?>
                <div id="tab8_group" class="group" style="padding: 20px 20px 20px 20px;">
            ...
            <div id="save_bar">
                <input type="button" value="<?php echo $mgrlang['gen_b_cancel']; ?>" onclick="cancel_edit('mgr.products.php');" /><input type="submit" value="<?php echo $mgrlang['gen_b_save']; ?>" />
            </div>
            </form>
