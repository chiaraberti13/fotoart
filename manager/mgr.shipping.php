<?php
	...
	###################################################################
		...
		//$perpage = $config['settings']['perpage_shipping'];
		...
		{
			case "save_regions":				
				save_regions($page,'shipping','ship_id');			
			break;			
			case "save_groups":				
				save_groups($page,'shipping','ship_id');				
			break;			
			case "save_sort":
				save_sort('shipping','ship_id');
			break;
			case "clear_sort":
				clear_sort('shipping');			
			break;			
			case "save_status":
				save_status($page,'shipping','ship_id');
			break;
			case "duplicate":
				$shipping_result = mysqli_query($db,"SELECT title FROM {$dbinfo[pre]}shipping WHERE ship_id = '$_GET[dupid]'");
				$shipping = mysqli_fetch_object($shipping_result);
				
				$new_id = DuplicateMySQLRecord($dbinfo[pre].'shipping', 'ship_id', $_GET['dupid'], 'title', 'uship_id');
				
				# DUPLICATE RANGES
				$shipping_ranges_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}shipping_ranges WHERE ship_id  = '$_GET[dupid]' ORDER BY sr_id");
				while($shipping_ranges = mysqli_fetch_object($shipping_ranges_result))
				{
					$sql = "INSERT INTO {$dbinfo[pre]}shipping_ranges (
						...
						'$new_id',
						'$shipping_ranges->fromrange',
						'$shipping_ranges->torange',
						'$shipping_ranges->price'
						)";
					$result = mysqli_query($db,$sql);
				}
				
				# DUPLICATE REGIONS 
				$regions_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}regionids WHERE item_id  = '$_GET[dupid]' AND mgrarea='$page' ORDER BY id");
				...
				# DUPLICATE GROUPS 
				$shipping_groups_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}groupids WHERE item_id  = '$_GET[dupid]' AND mgrarea = '$page'");
				while($shipping_groups = mysqli_fetch_object($shipping_groups_result))
				{
					mysqli_query($db,"INSERT INTO {$dbinfo[pre]}groupids (mgrarea,item_id,group_id) VALUES ('$page','$new_id','$shipping_groups->group_id')");
				}
				
				# UPDATE ACTIVITY LOG
				save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_shipping'],1,$mgrlang['gen_b_dup'] . " > <strong>$shipping->title</strong>");
				
				# OUTPUT A VERIFICATION MESSAGE
				$vmessage=$mgrlang['gen_mes_dupsuc'];
			break;
			case "del":
				if(!empty($_REQUEST['items']))
				{
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT title,ship_id FROM {$dbinfo[pre]}shipping WHERE ship_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result))
					{
						$log_titles.= "$log->title ($log->ship_id), ";
					}
					...
					# DELETE RANGES
					@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}shipping_ranges WHERE ship_id IN ($delete_array)");
					
					# DELETE GROUPS
					@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}groupids WHERE mgrarea = '$page' AND item_id IN ($delete_array)");
					
					# DELETE REGIONS
					@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}regionids WHERE mgrarea='$page' AND item_id IN ($delete_array)");
					
					# DELETE
					//@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}shipping WHERE ship_id IN ($delete_array)");
					# MARK AS DELETED
					@$sql = "UPDATE {$dbinfo[pre]}shipping SET deleted='1' WHERE ship_id IN ($delete_array)";
					@$result = mysqli_query($db,$sql);
					
					# UPDATE ACTIVITY LOG
					save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_shipping'],1,$mgrlang['gen_b_del'] . " > <strong>$log_titles</strong>");
		...
		$id_field_name = "ship_id";		
		require_once('mgr.datasort.logic.php');	
		
		# IF THIS IS AN ENTRY PAGE OR shippinggroups IS BLANK RESET THE shippinggroups SESSION	
		if($_GET['ep'] or empty($_SESSION['shippinggroups']))
		{
			$_SESSION['shippinggroups'] = array('all');
		}			
		...
		if($_REQUEST['setgroups'])
		{
			...
			{
				$_SESSION['shippinggroups'] = $_REQUEST['setgroups'];
			}
			else
			{				
				$_SESSION['shippinggroups'] = array($_REQUEST['setgroups']);
			}
		}
		
		# GET THE TOTAL NUMBER OF ROWS
		if(in_array("all",$_SESSION['shippinggroups']))
		{
			$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(ship_id) FROM {$dbinfo[pre]}shipping WHERE deleted='0'"));
		}
		else
		{
			//$shipping_result2 = "SELECT COUNT(ship_id) FROM {$dbinfo[pre]}shipping WHERE"
			$shipping_result2 = "SELECT COUNT(ship_id) FROM {$dbinfo[pre]}shipping LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}shipping.ship_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['shippinggroups']).") AND shipping.deleted='0'";
			$r_rows = mysqli_result_patch(mysqli_query($db,$shipping_result2));
		}
		...
		if(in_array("all",$_SESSION['shippinggroups']))
		{
			$shipping_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}shipping WHERE deleted='0' ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
		}
		else
		{				
			$shipping_result = mysqli_query($db,"SELECT sortorder,ship_id,active,title FROM {$dbinfo[pre]}shipping LEFT JOIN {$dbinfo[pre]}groupids ON {$dbinfo[pre]}shipping.ship_id = {$dbinfo[pre]}groupids.item_id WHERE {$dbinfo[pre]}groupids.group_id IN (".implode(",",$_SESSION['shippinggroups']).") AND shipping.deleted='0' GROUP BY {$dbinfo[pre]}shipping.ship_id ORDER BY $listby $listtype LIMIT $startrecord,$perpage"); 				
		}	
...
<head>
	...
	<script language="javascript" type="text/javascript">
		...
		function clear_sorts(){
			location.href='<?php echo $_SERVER['PHP_SELF']; ?>?ep=1&action=clear_sort&listby=ship_id&listtype=asc';
		}
		...
		function do_actions(){
			...
			switch(selecteditem){
				...
				case "install_shipping_mod":					
					workboxobj.mode = 'install_shipping_mod';
					workboxobj.page = '<?php echo $page; ?>';
					workboxobj.filename = '<?php echo $_SERVER['SCRIPT_NAME']; ?>';
					workboxobj.supportid = '<?php echo $supportPageID; ?>';
					workbox(workboxobj);
		...
		Event.observe(window, 'load', function()
			...
			if($('abutton_add_new')!=null)
			{
				$('abutton_add_new').observe('click', function()
					{
						window.location.href='mgr.shipping.edit.php?edit=new';
					});
		...
		function load_group_selector()
		{
			...
			var loadpage = "mgr.groups.actions.php?mode=grouplist&mgrarea=<?php echo $page; ?>&ingroups=<?php if(in_array('all',$_SESSION['shippinggroups'])){ echo 0; } else { echo 1; } ?>&exitpage=<?php echo $_SERVER['PHP_SELF']; ?>&sessname=shippinggroups";
		...
		// START SHIPPING REGIONS DETAILS PANEL
		var start_panel;
		function start_region_panel(id)
		{
			var reg_panel = 'more_info_' + id;
			$$('.details_win').each(function(s) { s.setStyle({display: "none"}) });
			start_panel = setTimeout("show_div_fade_load('" + reg_panel + "','mgr.shipping.actions.php?action=regwin&id="+id+"','_content')",'550');
		}
		
		// CANCEL LOAD AND CLOSE ALL PANELS
		function cancel_region_panel(id)
		{
		...
		function shiptype_boxes_old(shiptype)
		{
			if(shiptype == 'global')
			{
				hide_div('shipping_regions');
				hide_div('shipping_regions_clear');
			}
			else
			{
				show_div('shipping_regions');
				show_div('shipping_regions_clear');
				var item_id = 'new';
		...
		// INITIATE RECORD DUPLICATION
		function do_duplicate(item_id)
		{
			if("<?php echo $_SESSION['admin_user']['admin_id']; ?>" == "DEMO")
			{
				demo_message();
			...
			{
				location.href='mgr.shipping.php?action=duplicate&dupid='+item_id;
			}
		}
...
<body>
	...
	<div id="container">
        ...
        <div id="content_container">
            ...
            <div id="actions_bar">							
                <div class="sec_bar">
                    <img src="./images/mgr.badge.shipping.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_shipping']; ?></span> &nbsp; 
                </div>							
                ...
                <div style="padding-top: 9px; margin-right: 20px;">
                    ...
                    <select align="absmiddle" id="actionsdd" onchange="do_actions();">
                        ...
                        <?php if($shipping_group_rows and $r_rows and in_array("pro",$installed_addons)){ ?><option value="assign_groups">&nbsp; <?php echo $mgrlang['gen_au_itg']; ?></option><?php } ?>
                        <option value="set_status">&nbsp; <?php echo $mgrlang['gen_tostatus']; ?></option>
                        <option value="set_regions">&nbsp; <?php echo $mgrlang['gen_toregions']; ?></option>
			...
			<form name="grouplist" id="grouplist" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
				...
				<div style="<?php if(in_array('all',$_SESSION['shippinggroups'])){ echo "display: none;"; } else { echo "display: block;"; } ?>" class="options_area" id="group_selector"></div>
			</form>
            <?php
                ...
                <div id="content">						
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <tr>
							...
							<?php $header_name = "ship_id";		if($listby == $header_name){ $sel = "_sel"; } else { $sel = ""; }	$hlink = ""; $hlink = $_SERVER['PHP_SELF'] . "?listby=" . $header_name . "&listtype="; if($listtype == "asc" && $listby == $header_name){ $hlink .= "desc"; } else { $hlink .= "asc"; } ?><td class="cth<?php echo $sel; ?>" onclick="window.location.href='<?php echo $hlink; ?>'" align="center" width="100">	<div class="content_table_header<?php echo $sel; ?>"><div><a href="<?php echo $hlink ?>"><?php echo $mgrlang['gen_t_id']; ?></a></div></div></td>
                            ...
                            <?php $header_name = "region";		if($listby == $header_name){ $sel = "_sel"; } else { $sel = ""; }	$hlink = ""; $hlink = $_SERVER['PHP_SELF'] . "?listby=" . $header_name . "&listtype="; if($listtype == "asc" && $listby == $header_name){ $hlink .= "desc"; } else { $hlink .= "asc"; } ?><td class="cth<?php echo $sel; ?>" onclick="window.location.href='<?php echo $hlink; ?>'" align="center">				<div class="content_table_header<?php echo $sel; ?>"><div><a href="<?php echo $hlink ?>"><?php echo $mgrlang['gen_t_region']; ?></a></div></div></td>
                        ...
                        <?php								
                            ...
                            while($shipping = mysqli_fetch_object($shipping_result))
							{
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onmouseover="cellover(this,'#<?php echo $color_fade; ?>',32);" onmouseout="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"><?php if($shipping->sortorder) { echo $shipping->sortorder;} else { echo "--"; } ?></td>
                                <td align="center"><a name="row_<?php echo $shipping->ship_id; ?>"></a><?php echo $shipping->ship_id; ?></td>
                                <td onclick="window.location.href='mgr.shipping.edit.php?edit=<?php echo $shipping->ship_id; ?>'"><a href="mgr.shipping.edit.php?edit=<?php echo $shipping->ship_id; ?>" class="editlink"><?php echo $title; ?>&nbsp;</a></td>
                                <td align="center" nowrap>
									<?php 
										if($shipping->region == 2)
										{
											echo "<strong>".$mgrlang['shipping_f_regions_op2']."</strong>";
										}
										else
										{
											echo "<strong>".$mgrlang['shipping_f_regions_op1']."</strong>";
										}
										
										/*
									?>
                                        <div id="rw<?php echo $shipping->ship_id; ?>" style="font-weight: bold;" onmouseover="start_region_panel(<?php echo $shipping->ship_id; ?>)" onmouseout="cancel_region_panel(<?php echo $shipping->ship_id; ?>);"><?php echo $mgrlang['shipping_f_regions_op2']; ?></div>                                            
                                        <div id="more_info_<?php echo $shipping->ship_id; ?>" style="display: none; margin: -30px -10px -10px 80px;" class="details_win">
                                            <div class="details_win_inner">
                                                <img src="images/mgr.detailswin.arrow.gif" style="position: absolute; margin: 16px 0 0 -9px;" />
                                                <div id="more_info_<?php echo $shipping->ship_id; ?>_content" style="overflow: auto; padding: 10px; text-align: left; border: 1px solid #fff"><img src="images/mgr.loader.gif" style="margin: 40px;" /></div>
                                            </div>
                                        </div>
                                    <?php
										*/
                                ...
                                <td align="center"><div id="ac<?php echo $shipping->ship_id; ?>"><a href="javascript:switch_status('ac','<?php echo $shipping->ship_id; ?>');"><img src="images/mgr.small.check.<?php echo $shipping->active; ?>.png" border="0" /></a></div></td>
                                <td align="center" valign="middle" nowrap>
                                    <?php if($shipping->calc_module == 'custom'){ ?><a href="javascript:do_duplicate(<?php echo $shipping->ship_id; ?>);" class='actionlink'><img src="images/mgr.button.small.dup.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_b_dup']; ?></a><?php } ?>
                                    <a href="mgr.shipping.edit.php?edit=<?php echo $shipping->ship_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a> 
                                    <a href="javascript:deleterec(<?php echo $shipping->ship_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a>
                                </td>
                                <td align="center" valign="middle" nowrap>                                                
                                    <input type="checkbox" name="items[]" value="<?php echo $shipping->ship_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
                            }
