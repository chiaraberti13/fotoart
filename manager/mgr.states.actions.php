<?php
	...
	###################################################################
		...
		switch($_REQUEST['action'])
		{
			# SET ACTIVE STATUS
			case "ac":
				$state_result = mysqli_query($db,"SELECT active FROM {$dbinfo[pre]}states where state_id = '$_REQUEST[id]'");
				$state = mysqli_fetch_object($state_result);
				
				# FLIP THE VALUE
				$new_value = (empty($state->active) ? 1 : 0);	
							
				$sql = "UPDATE {$dbinfo[pre]}states SET active='$new_value' where state_id = '$_REQUEST[id]'";
