	...
	###################################################################
		...
		switch($_REQUEST['pmode'])
		{
			...
			case "galleries":
				...
				# PULL subscription DETAILS FROM DB	
				if($_GET['id'] != "new"){
					$sub_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}subscriptions WHERE sub_id = '$_GET[id]'");
				...
				echo "<input type='checkbox' name='all_galleries' id='all_galleries' value='1' class='radio' style='margin: -4px 0 0 4px; vertical-align: middle'";
				if($id == 'new' or $sub->all_galleries == 1){ echo "checked"; }
				echo " /> <label for='all_galleries' class='gallery_label'>{$mgrlang[gen_dis_in_all]}</label></div>";
			...
			case "display_ip_list":
				//sleep(1);
				$ip_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}item_photos WHERE item_id = '$_GET[id]' AND mgrarea = 'sub' ORDER BY ip_id DESC");
				$ip_rows = mysqli_num_rows($ip_result);
				while($ip = mysqli_fetch_object($ip_result))
				{
					$sub = zerofill($_GET['id'],4);
					$ip_id = zerofill($ip->ip_id,4);				
					$src_img = "sub".$sub."_ip".$ip_id."_small.jpg";				
					...
					*/
					...
					echo "<div class='ip_div' id='ip_$ip->ip_id'>";
						...
						echo "<div class='ip_div_inner' style='width: ".$newsize[0]."px;'><img src='mgr.subscriptions.actions.php?pmode=display_ip&sub=$_GET[id]&ip_id=$ip->ip_id' /></div>";
			...
			break;
			case "delete_ip":
				delete_item_photo('sub',$_GET['sub'],$_GET['ip_id']);
				echo "<script>load_ip();</script>";
			break;
			# SET ACTIVE STATUS
			case "ac":
				$sub_result = mysqli_query($db,"SELECT active FROM {$dbinfo[pre]}subscriptions where sub_id = '$_REQUEST[id]'");
				$sub = mysqli_fetch_object($sub_result);
				
				# FLIP THE VALUE
				$new_value = (empty($sub->active) ? 1 : 0);	
							
				$sql = "UPDATE {$dbinfo[pre]}subscriptions SET active='$new_value' where sub_id = '$_REQUEST[id]'";
			...
			# SHOW DETAILS
			case "details":
				$sub_result = mysqli_query($db,"SELECT notes,description FROM {$dbinfo[pre]}subscriptions where sub_id = '$_REQUEST[id]'");
				$sub = mysqli_fetch_object($sub_result);
				
				echo "<div style='padding: 20px;'>";
					if($sub->notes){ echo "<strong>{$mgrlang[gen_notes]}:</strong> $sub->notes<br /><br />"; }
					if($sub->description){ echo "<strong>{$mgrlang[gen_description]}:</strong> $sub->description"; }
					if(!$sub->notes and !$sub->description){ echo $mgrlang['gen_no_notes']; }
				echo "</div>";
			break;
		}	
?>
