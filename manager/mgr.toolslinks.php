		...
		switch($_REQUEST['action']){
			...
			case "del":
				if(!empty($_REQUEST['items'])){
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT tl_name,tl_id FROM {$dbinfo[pre]}toolslinks  WHERE tl_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result)){
						$log_titles.= "$log->tl_name ($log->tl_id), ";
					}
					if(substr($log_titles,strlen($log_titles)-2,2) == ", "){
						$log_titles = substr($log_titles,0,strlen($log_titles)-2);
					}
					
					@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}toolslinks WHERE tl_id IN ($delete_array)");
					
					# UPDATE ACTIVITY LOG
					save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_toolslinks'],1,$mgrlang['gen_b_del'] . " > <strong>$log_titles</strong>");
		...
		Event.observe(window, 'load', function()
			...
			if($('abutton_add_new')!=null)
			{
				$('abutton_add_new').observe('click', function()
					{
						window.location.href='mgr.toolslinks.edit.php?edit=new';
					});
...
<body>
	...
	<div id="container">
        ...
        <?php
            ...
            # SELECT ITEMS
            $tl_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}toolslinks WHERE tl_owner = '0' OR tl_owner = '".$_SESSION['admin_user']['admin_id']."' ORDER BY $listby $listtype");
        ...
        ?>
            ...
            <div id="actions_bar">							
                <div class="sec_bar">
                    <img src="./images/mgr.badge.links.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_toolslinks']; ?></span> &nbsp; 
                </div>							
                ...
                <div id="content" style="border-top: 0px;">						
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        <?php								
                            ...
                            while($tl = mysqli_fetch_object($tl_result)){
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onMouseOver="cellover(this,'#<?php echo $color_fade; ?>',32);" onMouseOut="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"><a name="row_<?php echo $tl->tl_id; ?>"></a><?php echo $tl->tl_id; ?></td>
                                <td onclick="window.location.href='mgr.toolslinks.edit.php?edit=<?php echo $tl->tl_id; ?>'"><a href="mgr.toolslinks.edit.php?edit=<?php echo $tl->tl_id; ?>" class="editlink"><?php echo $tl->tl_name; ?></a>&nbsp;</td>
                                <td align="center" valign="middle" nowrap>
                                    <a href="mgr.toolslinks.edit.php?edit=<?php echo $tl->tl_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a>
                                    <a href="javascript:deleterec(<?php echo $tl->tl_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a> 
                                </td>
                                <td align="center" valign="middle" nowrap>
                                    <input type="checkbox" name="items[]" value="<?php echo $tl->tl_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
                            }
