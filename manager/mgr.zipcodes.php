<?php
	...
	###################################################################
		...
		{		
			case "del":
				if(!empty($_REQUEST['items']))
				{
					...
					# GET TITLES FOR LOG
					$log_result = mysqli_query($db,"SELECT zipcode,zipcode_id FROM {$dbinfo[pre]}zipcodes WHERE zipcode_id IN ($delete_array)");
					while($log = mysqli_fetch_object($log_result))
					{
						$log_titles.= "$log->zipcode ($log->zipcode_id), ";
					}
					...
					# DELETE TAXES
					@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}taxes WHERE region_type = '3' AND region_id IN ($delete_array)");
					
					# DELETE
					//@mysqli_query($db,"DELETE FROM {$dbinfo[pre]}zipcodes WHERE zipcode_id IN ($delete_array)");
					# MARK AS DELETED
					@$sql = "UPDATE {$dbinfo[pre]}zipcodes SET deleted='1' WHERE zipcode_id IN ($delete_array)";
					@$result = mysqli_query($db,$sql);
					
					# UPDATE ACTIVITY LOG
					save_activity($_SESSION['admin_user']['admin_id'],$mgrlang['subnav_zipcodes'],1,$mgrlang['gen_b_del'] . " > <strong>$log_titles</strong>");
			...
			case "save_status":
				save_status($page,'zipcodes','zipcode_id');
			break;
		}
		...
		if($_GET['region'])
		{
			...
			$_SESSION['zipcodes_region_type'] = $zregion[0];
			$_SESSION['zipcodes_region_id'] = $zregion[1];
		}
		
		if($_SESSION['zipcodes_region_id'])
		{
			$zipcodes_region_id = $_SESSION['zipcodes_region_id'];
			$zipcodes_region_type = $_SESSION['zipcodes_region_type'];
			if($zipcodes_region_type == 'c')
			{
				$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(zipcode_id) FROM {$dbinfo[pre]}zipcodes WHERE country_id='$zipcodes_region_id' AND deleted='0'"));
			}
			else
			{
				$r_rows = mysqli_result_patch(mysqli_query($db,"SELECT COUNT(zipcode_id) FROM {$dbinfo[pre]}zipcodes WHERE state_id='$zipcodes_region_id' AND deleted='0'"));
			}
		}
		else
		{
	...
	<script language="javascript" type="text/javascript">
		...
		function switch_status(item_type,item_id)
		{
			...
			else
			{
				...
				var loadpage = "mgr.zipcodes.actions.php?action=" + item_type + "&id=" + item_id;
				var pars = "";
		...
		function set_region()
		{
			var selecteditem = $('regiondd').options[$('regiondd').selectedIndex].value;
			switch(selecteditem)
			{
				case "editc":
				...
				break;
				case "edits":
					location.href='mgr.states.php?ep=1';
				break;
				default:
					location.href='<?php echo $_SERVER['PHP_SELF']; ?>?ep=1&region=' + selecteditem;
				break;
			}		
		}
		...
		Event.observe(window, 'load', function()
			...
			if($('abutton_add_new')!=null)
			{
				$('abutton_add_new').observe('click', function()
					{
						window.location.href='mgr.zipcodes.edit.php?edit=new&region=<?php echo @$_SESSION['zipcodes_region_type'] . "," . $_SESSION['zipcodes_region_id']; ?>';
					});
...
<body>
	...
	<div id="container">
        ...
        <?php
			...
			$id_field_name = "zipcode_id";		
			require_once('mgr.datasort.logic.php');	
			
			//echo $zregion[0] . ":" . $zregion[1]; exit;
			...
			if($zipcodes_region_type == 'c')
			{
				$zipcode_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}zipcodes WHERE country_id='$zipcodes_region_id' AND deleted='0' ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
			}
			else
			{
				$zipcode_result = mysqli_query($db,"SELECT * FROM {$dbinfo[pre]}zipcodes WHERE state_id='$zipcodes_region_id' AND deleted='0' ORDER BY $listby $listtype LIMIT $startrecord,$perpage");
			}
        ...
        ?>
            ...
            <div id="actions_bar">							
                <div class="sec_bar">
                    <img src="./images/mgr.badge.zip.png" align="absmiddle" /><span><?php echo $mgrlang['subnav_zipcodes']; ?></span> &nbsp; 
                </div>                
                ...
                <div style="float: right; padding-top: 9px; margin-right: 20px;">
                    <select align="absmiddle" id="regiondd" onchange="set_region();">
                        <option value="#"><?php echo $mgrlang['zipcodes_region']; ?>:</option>
                        <?php
							...
							{
								echo "<option value='editc' style='background-color: #eeeeee;'>$mgrlang[zipcodes_edit_countries]</option>";
							}
							if(in_array("states",$_SESSION['admin_user']['permissions']))
							{
								echo "<option value='edits' style='background-color: #eeeeee;'>$mgrlang[zipcodes_edit_states]</option>";
							}
						?>
                    </select>
                    <?php /*echo "session: $_SESSION[zipcodes_by_country]  | get: $_GET[country]";*/ ?>
                </div>
                </form>
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                        ...
                        ?>
                            ...
                            <tr class="<?php echo $row_class; ?>" onmouseover="cellover(this,'#<?php echo $color_fade; ?>',32);" onmouseout="cellout(this,'#<?php echo $color_fade; ?>');">
                                <td align="center"><a name="row_<?php echo $zipcode->zipcode_id; ?>"></a><?php echo $zipcode->zipcode_id; ?></td>
                                <td onclick="window.location.href='mgr.zipcodes.edit.php?edit=<?php echo $zipcode->zipcode_id; ?>'"><a href="mgr.zipcodes.edit.php?edit=<?php echo $zipcode->zipcode_id; ?>" class="editlink"><?php echo $zipcode->zipcode; ?></a>&nbsp;</td>
                                <td align="center"><div id="ac<?php echo $zipcode->zipcode_id; ?>"><a href="javascript:switch_status('ac','<?php echo $zipcode->zipcode_id; ?>');"><img src="images/mgr.small.check.<?php echo $zipcode->active; ?>.png" border="0" /></a></div></td>
                                <td align="center" valign="middle" nowrap>
                                    <a href="mgr.zipcodes.edit.php?edit=<?php echo $zipcode->zipcode_id; ?>" class='actionlink'><img src="images/mgr.icon.edit.png" align="absmiddle" alt="<?php echo $mgrlang['gen_edit']; ?>" border="0" /><?php echo $mgrlang['gen_short_edit']; ?></a> 
                                    <a href="javascript:deleterec(<?php echo $zipcode->zipcode_id; ?>);" class='actionlink'><img src="images/mgr.icon.delete.png" align="absmiddle" alt="<?php echo $mgrlang['gen_delete']; ?>" border="0" /><?php echo $mgrlang['gen_short_delete']; ?></a>
                                </td>
                                <td align="center" valign="middle" nowrap>                                                
                                    <input type="checkbox" name="items[]" value="<?php echo $zipcode->zipcode_id; ?>" class="atitems" style="padding: 0px; margin: 0px 0px 0px 2px;" />
                                </td>
                            </tr>
                        <?php
                            }
