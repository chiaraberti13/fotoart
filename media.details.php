<?php
	...
	******************************************************************/
	...
	require_once BASE_PATH.'/assets/includes/config.php'; // FP
				...
				{	
					while($product = mysqli_fetch_array($productsResult))
					{
						...
						*/
						...
						{
							/*FP*/	
							$ppzl = $product['item_code'] == PUZZLES_CATEGORY ? TRUE : FALSE; 
							...
							else
							{
								if($config['EncryptIDs'])
								{
									...
									if($product['item_code'] == PUZZLES_CATEGORY) $productsArray[$product['prod_id']]['linktopuzzle'] = "{$siteURL}/photo.puzzle.php?mediaID={$media[encryptedID]}";
								}
								else
								{
									...
									$productsArray[$product['prod_id']]['addToCartLink'] = $cartLink; // Direct to cart
									if($product['item_code'] == PUZZLES_CATEGORY) $productsArray[$product['prod_id']]['linktopuzzle'] = "{$siteURL}/photo.puzzle.php?mediaID={$media[media_id]}";
								}
