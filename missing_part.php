...
$websitename="Modulo richiesta pezzi mancanti"; 
...
// post_max_size, upload_max_filesize, file_uploads, max_execution_time!
// 2048kb = 2MB,       1024kb = 1MB,     512kb = 1/2MB etc..
$max_file_total="12288";

// Value for the Submit Button
$submitvalue=" Invia Richiesta "; 
...
$thanksmessage="<script type=\"text/javascript\">window.location=\"https://www.fotoartpuzzle.it/conferma.html\";</script>"; 
...
If($_POST['submit']==true) {
	extract($_POST, EXTR_SKIP);
		...
		If(trim($yourname)=="") { 
			$error.="Non hai inserito il nome completo!<br />";
		}
		
		If(trim($youremail)=="") { 
			$error.="Non hai inserito la tua email!<br />";
		...
		If(trim($yourmessage)=="") { 
			$error.="Devi inserire un messaggio!<br />";
		}
		...
		If($allowattach > 0) {
			...
			If((array_sum($_FILES['attachment']['size'])) > ($max_file_total*1024)) {
				...
				$error.="La grandezza massima dei file deve essere ".$max_file_total."kb<br />";
				...
				For($i=0; $i <= $allowattach-1; $i++) {
					...
					If($_FILES['attachment']['name'][$i]) {
						...
						If(!in_array(get_ext($_FILES['attachment']['name'][$i]), $allowtypes)) {
							...
							$error.= "Tipo di file non valido: ".$_FILES['attachment']['name'][$i]."<br />";
							
						//Check the size of each file
						} Elseif(($_FILES['attachment']['size'][$i]) > ($max_file_size*1024)) {
							
							$error.= "Il tuo file: ".$_FILES['attachment']['name'][$i]." è troppo grande.<br />";
...
<title><?php echo $websitename; ?> - Assistenza Puzzle</title>
...
<style type="text/css">
	...
	.button,.button-reset{width:200px;
	border:none;
	margin-bottom:6px;}
...
function Checkit(theform) {
	if(theform.yourname.value=="") {
		error+="Devi inserire il nome completo\n";
	}
	
	if(theform.youremail.value=="") {
		error+="Non hai inserito la tua email\n";
	} else if(!e_regex.test(theform.youremail.value)) {
		error+="Indirizzo email non valido\n";
	}
		
	if(theform.yourmessage.value=="") {
		error+="Non hai inserito un indirizzo di spedizione\n";
	}
...
<center>
<a href="/missing_part_en.php"><img src="assets/images/flag_duble.jpg" alt="" border="0" align=""></a>
<br><br>
Modulo per la richiesta di pezzi mancanti o difettosi.<br> (utilizzabile entro e non oltre 60gg da data di consegna ordine)<br><br></center>
Seguire queste semplici istruzioni per ricevere le parti mancanti o difettose:<br><br>

 1. Il puzzle deve essere completato, anche in presenza di parti mancanti o difettose;<br>
 2. Nel caso di tessere difettose/deteriorate, queste devono essere eliminate dal puzzle prima delle foto;<br>
 3. Inserire sotto i spazi vuoti lasciati dalle tessere asportate, un foglio bianco per evidenziare la foto;<br>
 4. A questo punto devi scattare e caricare nel modulo 2 immagini:<br><br>
 
 <font color="ff0000">una foto dell'intero puzzle + una foto del particolare dove mancano il pezzo/i (area cm 15x15)<br><br>
 nota: non serve indicare coordinate delle tessere o altre descrizioni, SERVONO SOLO 2 FOTO BEN DEFINITE !!</font><br><br>

Con le foto del puzzle pronte, ora puoi compilare la richiesta:<br><br>

Inserire numero ordine e codice puzzle nel formato: 35257 - 61011 <small>(per fotopuzzle solo il numero ordine)</small><br><br>
I pezzi vengono inviati entro 2 settimane e spediti con posta prioritaria.<br><br>
<body>
<?If($display_message) {?>
...
<table align="center" class="table">
	...
	<?If($allowattach > 0) {?>
		<tr>
			<td width="100%" class="attach_info" colspan="2">
				<b>Tipo di file valido:</b> <?=implode($allowtypes, ", ");?><br />
				<b>Dimensione massima per immagine:</b> <?=$max_file_size?>kb.<br />
				<b>Dimensione massima per invio:</b> <?=$max_file_total?>kb.
			</td>
		</tr>
	<?}?>
	
	<tr>
		<td width="30%" class="table_body">Nome completo:</td>
	...
	<tr>
		<td width="30%" class="table_body">Email:</td>
	...
	<tr>
		<td width="30%" class="table_body">Numero Ordine e Codice Puzzle:</td>
		<td width="70%" class="table_body">
			...
			<?} Else {?>
				...
				<input name="emailsubject" type="text" size="30" value="<?=stripslashes(htmlspecialchars($emailsubject));?>" /><span class="error_message">*</span></td>
	...
	<?For($i=1;$i <= $allowattach; $i++) {?>
		<tr>
			<td width="30%" class="table_body">Immagine:</td>
			<td width="70%" class="table_body"><input name="attachment[]" type="file" size="30" /></td>
		</tr>
	<?}?>
	
	<tr>
		<td colspan="2" width="100%" class="table_body">Inserire qui un indirizzo postale dove inviare le parti richieste:<span class="error_message">*</span><br /><br />
			<div align="center">
	...
	<tr>
		<td colspan="2" width="100%" class="table_footer">
			<input type="hidden"  class="button" name="submit" value="true" />
			<input type="submit"  class="button" value="<?=$submitvalue;?>" /> &nbsp;
			<input type="reset" class="button-reset" value="<?=$resetvalue;?>" />
		</td>
	</tr>
</table>
</form>
...
<?} Else {?>
	...
	<div align="center" class="button-reset"><?=$thanksmessage;?></div>
	<br />
	<br />
