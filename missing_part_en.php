...
$websitename="Request Form missing pieces"; 
...
$thanksmessage="<script type=\"text/javascript\">window.location=\"https://www.fotoartpuzzle.it/conferma_en.html\";</script>"; 
...
If($_POST['submit']==true) {
	extract($_POST, EXTR_SKIP);
		...
		If(trim($yourname)=="") { 
			$error.="You have not entered the full name!<br />";
		}
		...
		If(trim($yourmessage)=="") { 
			$error.="You must enter a shipping address!<br />";
		}
		...
		If($allowattach > 0) {
			...
			If((array_sum($_FILES['attachment']['size'])) > ($max_file_total*1024)) {
				...
				$error.="The maximum file size must be ".$max_file_total."kb<br />";
				...
				For($i=0; $i <= $allowattach-1; $i++) {
					...
					If($_FILES['attachment']['name'][$i]) {
						...
						If(!in_array(get_ext($_FILES['attachment']['name'][$i]), $allowtypes)) {
							...
							$error.= "Invalid file type: ".$_FILES['attachment']['name'][$i]."<br />";
							
						//Check the size of each file
						} Elseif(($_FILES['attachment']['size'][$i]) > ($max_file_size*1024)) {
							
							$error.= "Your file: ".$_FILES['attachment']['name'][$i]." it is too big.<br />";
...
<title><?php echo $websitename; ?> - Puzzle Service</title>
...
function Checkit(theform) {
	...
	if(theform.yourmessage.value=="") {
		error+="It did not provide a shipping address\n";
	}
...
<center>
<a href="/missing_part.php"><img src="assets/images/flag_duble.jpg" alt="" border="0" align=""></a>
<br><br>
Form for the request for missing or defective parts.<br> (Used no later than 60 days from delivery date order)<br><br></center>
Follow these simple instructions to get the missing or defective parts:<br><br>

 1. The puzzle must be completed, even with missing or defective parts;<br>
 2. In case of defective/non-performing cards, they must be eliminated from the puzzle, first of the photos;<br>
 3. Enter below the empty spaces left by removing tiles, a white paper to highlight the photo;<br>
 4. At this point, you have to take and upload 2 images into the module:<br><br>
 
 <font color="ff0000">a photo of the whole puzzle + a photo of the missing particular where the piece/s (area 15x15 cm)<br><br>
 note: do not need to indicate coordinates of the tiles or other descriptions, NEED ONLY 2 PHOTOS WELL DEFINED !!</font><br><br>

With photos of the puzzle ready, now you can complete the form:<br><br>

Enter order number and puzzle code in the format: 35257 - 61011 <br><br>
The pieces are sent within 2 weeks by priority mail.</center><br><br>
<body>
<?If($display_message) {?>
...
<table align="center" class="table">
	...
	<?If($allowattach > 0) {?>
		<tr>
			<td width="100%" class="attach_info" colspan="2">
				<b>Valid file type:</b> <?=implode($allowtypes, ", ");?><br />
				<b>Maximum image size:</b> <?=$max_file_size?>kb.<br />
				<b>Maximum size for sending:</b> <?=$max_file_total?>kb.
			</td>
		</tr>
	<?}?>
	
	<tr>
		<td width="30%" class="table_body">Full name:</td>
	...
	<tr>
		<td width="30%" class="table_body">Order number and Puzzle Code:</td>
	...
	<?For($i=1;$i <= $allowattach; $i++) {?>
		<tr>
			<td width="30%" class="table_body">Image:</td>
			<td width="70%" class="table_body"><input name="attachment[]" type="file" size="30" /></td>
		</tr>
	<?}?>
	
	<tr>
		<td colspan="2" width="100%" class="table_body">Here enter a postal address where to send the parts required:<span class="error_message">*</span><br /><br />
			<div align="center">
