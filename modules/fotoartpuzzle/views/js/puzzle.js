(function () {
    ...
    if (initialSummary && Object.keys(initialSummary).length) {
        ...
        if (initialSummary.boxFont) {
            state.boxFont = initialSummary.boxFont;
        }
        if (initialSummary.formatData && typeof initialSummary.formatData === 'object' && Object.keys(initialSummary.formatData).length) {
            state.format = initialSummary.formatData;
    ...
    function renderFormatStep() {
        ...
        config.formats.forEach(function (item) {
            ...
            if (normalizeFormatName(state.format) === item.name) {
    ...
    async function finalizeCustomization() {
        ...
        try {
            ...
            payload.append('box_color', state.boxColor || '');
            payload.append('box_font', state.boxFont || '');
            payload.append('format', normalizeFormatName(state.format));
            payload.append('format_payload', JSON.stringify(state.format || {}));
            payload.append('file_name', state.fileName || '');
    ...
    function updateSummaryPanel() {
        if (state.customizationId) {
            ...
            appendSummaryItem(list, translate('Font'), state.boxFont);
