<?php
	...
	******************************************************************/
	...
	define('PAGE_ID','photoPuzzle'); // Page ID
	...
	$smarty->display('photo.puzzle.tpl'); // Smarty template
	...
	//{	
		...
		{	//$smarty->assign('noAccess',1);
			// Parto con il classico upload della foto
		}
		else
		{
			...
			if($mediaInfo->getRows())
			{
				...
				{	
					while($product = mysqli_fetch_array($productsResult))
					{
						...
						{
							//FP	
							$ppzl = $product['item_code'] == "PPZL" ? TRUE : FALSE; 
							//if($product['item_code'] == "PPZL") $productsArray[$product['prod_id']]['linktopuzzle'] = "{$siteURL}/photo_puzzle.php?mediaID={$media[media_id]}";
							...
							//FP
							if(mysqli_num_rows($optionsResult) && !$ppzl)
							{
							...
							else
							{
								if($config['EncryptIDs'])
								{
									...
									if($product['item_code'] == "PPZL") $productsArray[$product['prod_id']]['linktopuzzle'] = "{$siteURL}/photo.puzzle.php?mediaID={$media[encryptedID]}";
								}
								else
								{
									...
									$productsArray[$product['prod_id']]['addToCartLink'] = $cartLink; // Direct to cart
									if($product['item_code'] == "PPZL") $productsArray[$product['prod_id']]['linktopuzzle'] = "{$siteURL}/photo.puzzle.php?mediaID={$media[media_id]}";
								}
