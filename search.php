	...
	*/
	...
	{	
		...
		if($_REQUEST['searchPhrase'] != $lang['enterKeywords']) // Make sure the phrase 'Enter Keywords' is not included in the search
		{
			...
			if($langset['id'] == 'russian')
				$searchPhrase = mb_convert_case($searchPhrase, MB_CASE_LOWER, "UTF-8");
			else
				$searchPhrase = strtolower($searchPhrase);
	...
	if($_REQUEST['galleries'])
	{
		...
		else
		{
			...
			$_SESSION['searchForm']['galleries'][] = $_REQUEST['galleries']; // Single gallery passed // $_SESSION['searchForm']['galleries'][$_REQUEST['galleries']]
		}
	...
	{	
		...
		{	
			...
			{				
					...
					foreach($_SESSION['searchForm']['keywords'] as $key => $batch)
					{
						...
						{							
							...
							while($keywordDB = mysqli_fetch_assoc($keywordsResult))
							{
								...
								//echo 'mid'.$keywordDB['media_id'];
								
								$runningKeywords[] = $keywordDB['media_id']; // Added in 4.5.2
				...
				*/
				...
				{	
					@$resultsFlat = implode(',',$runningKeywords); // 4.5
					...
					if($totalRowsFound > 0) // Only do this if previous rows have been found
					{
						...
						if($dspLicensesCount = getRows())
						{
							...
							if(!$resultsFlat) $resultsFlat = 0;
							
							$mediaFromDSPLicenseResult = mysqli_query($db,"SELECT SQL_CALC_FOUND_ROWS media_id FROM {$dbinfo[pre]}media_digital_sizes WHERE (license IN ({$licListFlat}) OR ds_id IN ($validDigitalSizesFlat)) AND media_id IN ({$resultsFlat}) GROUP BY media_id");
					...
					//print_r($_SESSION['searchForm']['licenses']);					
					//$licensesSQL = "SELECT SQL_CALC_FOUND_ROWS license,media_id FROM {$dbinfo[pre]}media WHERE active = 1 AND license IN ({$licListFlat})"; // pre 4.5
					$licensesSQL = "SELECT SQL_CALC_FOUND_ROWS license,media_id FROM {$dbinfo[pre]}media WHERE active = 1 AND license IN ({$licListFlat})"; // AND media_id IN ({$resultsFlat}) removed in 4.5.4				
					...
					if($totalRowsFound > 0 and $licensesCount > 0) // Only do this if previous rows have been found
					{
						...
						if($mediaFromDSPLicenseArray) // Merge with the previous array
							$resultsArray = $licenseResultsArray + $mediaFromDSPLicenseArray; // 4.5 //$resultsArray = array_merge($licenseResultsArray,$mediaFromDSPLicenseArray); // pre 4.5
						else
			...
			//{
				...
				*/
					...
					if($_SESSION['searchForm']['keywords'])
					{
						...
						{							
							$titleFieldName = ($selectedLanguage != $config['settings']['default_lang']) ? "title_{$selectedLanguage}" : 'title';
							...
							foreach($keywords as $keyword)
							{
								...
								$titleSQL.= "{$dbinfo[pre]}media.{$titleFieldName} {$titleSubSQL}";
						...
						{
							$descriptionFieldName = ($selectedLanguage != $config['settings']['default_lang']) ? "description_{$selectedLanguage}" : 'description';
							...
							foreach($keywords as $key => $keyword)
							{
								...
								$descriptionSQL.= "{$dbinfo[pre]}media.{$descriptionFieldName} {$descriptionSubSQL}";
		...
		//print_r(tagCloud(false,$config['settings']['tagCloudSort'])); exit;
		...
		*/
		...
		if(@$returnRows = mysqli_num_rows($mediaTypesResult))
		{
			while($mediaTypes = mysqli_fetch_assoc($mediaTypesResult))
			{
				...
				$mediaTypesArray[$mediaTypes['mt_id']]['name'] = ($mediaTypes['name_'.$selectedLanguage]) ? $mediaTypes['name_'.$selectedLanguage] : $mediaTypes['name'];
